 ...
 *
 * **本函数的 `import.meta.dirname` 恒等于 `<repo>/tests/test-support`，与调用方在哪无关**
...
/**
 * 〔src/test 分离〕把「我所在的 `tests/…` 目录」映射到**被守对象所在的 `src/…` 目录**。
 *
 * # 为什么需要它
 *
 * 源码级守卫过去用 `__dirname` 当「被守对象的目录」—— 那在测试与被守文件同住一个目录时
 * 成立。两棵树分开后不成立了，而 `tests/` 与 `src/` 是**严格镜像**，所以映射就是换掉路径里
 * 那一段目录名。
 *
 * # 为什么是一个函数而不是各处一行正则
 *
 ...
 * 会安静地量错东西）。⇒ 一个东西一个住址。
 *
 * ⚠ 只换**最后一个** `tests` 段，且要求它是完整一段（`/tests/` 或结尾 `/tests`）——
 * 避免把 `…/tests/latest/…` 里的 `latest` 误伤。
 *
 * ⚠ 那个 `(.*)` **刻意是贪婪的**：上一轮写的是非贪婪 `(.*?)`，取的其实是**第一个**
 * `tests` 段，与这段注释说的「最后一个」相反。今天的路径里只有一段 `tests` 所以两者
...
export function srcDirOf(testDir: string): string {
  ...
  if (!m) {
    throw new Error(
      `srcDirOf 期望一个 tests/ 下的目录，拿到的是 ${testDir}` +
