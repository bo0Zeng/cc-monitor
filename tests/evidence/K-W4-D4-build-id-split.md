...
### 2.1b 🔴 顶回件文件 `§0c` 一句：那时用户看到的**不是**「连接失败」

`§0c` 逐字写着「**且报的是「连接失败」而不是「那个文件不在」**」。**现打把那个串找出来了，不是这一句**：

- 连接面那一句逐字（`src-tauri/src/ssh_source.rs`，搜锚点 `未回 hello`）：
  **「SSH 连上了，但 daemon 在超时内未回 hello（未部署 / 路径错 / 启动失败？）。」**
- 设置面板那一格逐字（`src/settings/machine-card.ts:1173`）：**「daemon 未响应 / 未部署」**
- 而「连接失败」那几个字确实在这份代码里，但是**别的东西**：`machine-card.ts:1126` 的
  `"SSH 连接失败"`（SSH 本身连不上）· `ssh_source.rs` 的 `"所有地址连接失败: …"`。
  ⇒ 拿它来描述本形，**会把读者引到另一条故障线上去**。

★ **订正之后这条病其实更难看，不是更轻**：用户拿到的是一个**三选一的猜测**
（未部署 / 路径错 / 启动失败？）。**手里握着一条 SFTP 会话、一问就知道那个文件在不在的那一层，
什么都没说；要去猜的是够不着那个事实的那一层。** 本波买的就是让前一层把它知道的那半句说出来。

⚠ **诚实边界**：上面两句都是**从源码摘的逐字串**，不是真机跑出来的读数 —— 本轮不碰真远端（红线）。

### 2.1c 同职的地方查过了：**本机那条路结构上免疫**

`brief` 15 那一问（「我治的是这一处，还是所有同职的地方」）。`deploy_decision` 全仓三个调用点：

| 调用点 | 本波 | 为什么 |
|---|---|---|
| `sftp.rs` `ensure_daemon_deployed` | **已治** | daemon 自动部署 |
| `sftp.rs` `deploy_remote_daemon` | **已治** | 手动「安装 daemon」按钮，同一条病 |
| `acct_iso_deploy.rs:188` | **不动**（写区外） | 见第 8 节 |

**本机释放那条路（`backend/control/local_backend.rs`）没有这条病，而且是比我这个修法更强的形状** ——
它把 build_id 拼进**文件名**：`pub fn local_extract_name(build_id: &str) -> String` 体内逐字
`format!("cc-monitor-local-{build_id}")`。⇒ 文件没了，它那个「版本标记」**跟着一起没了**，
两件事结构上不可能分家。而远端那条路把标记放在**旁边一个文件**里，才有了「标记还在、二进制没了」这个缝。
★ 记下来是因为它是**改名落地那一拍的可选出路**：把 build_id 拼进远端落点的文件名，
这条缝就彻底焊死（代价：落点名不再是个固定名，`e2e_gate_registry.rs` 按名字 `pgrep` 那一格要跟着改）。
**本波不做**（超出 D4 射程，且要改的面在写区外）。
...
| 新增 | 是什么 |
|---|---|
| `enum TargetBinary` | 落点那个文件的取样结论。**不是 `bool`** —— 「问不出来」不许读成「不在」（那等于每次 stat 失败就重传 2.3 MB），也不许读成「在」（那退回它要治的静默）。⚠ **成员的唯一住址是那个 `enum` 本身**，本文件刻意不复述成员表、也不写基数（`brief` 13b：基数也是复述，字面量一加成员就成假话）；要看成员点开 `sftp.rs` 的 `pub enum TargetBinary` |
...
## 8 · 写区外的四处，**一个字节都没碰**，逐条交 PM

| # | 住址 | 现状逐字 | 要不要动 · 为什么 |
|---|---|---|---|
| ⑴ | `src-tauri/src/acct_iso_deploy.rs:188` `match deploy_decision(remote_id.as_deref(), vendor_id())` | 标记 `format!("{dest}/.vendor_id")` —— 也是**目录级**，落点是一批脚本 | **同族的缝，本波不治**（写区外）。⚠ 但它**不是同一个形**：那里标记与全部脚本是**同一次调用里**上传的，没有「部署成功之后落点又被改」这一段路（没有用户可改的 `daemonPath` 那种入口）⇒ 优先级低于 daemon 那条。**要不要立件由 PM 裁。** 本波把 `deploy_decision` 的签名**一个字没动**，正是为了让这条路零改动 |
| ⑵ | `remote-daemon-proto/src/build_id_guard.rs`（`subcommand_fingerprint` 的 docstring，搜锚点 `唯一**判据就是 build_id 字符串`） | 逐字「而 `sftp.rs::deploy_decision` 判「远端要不要换 daemon」的**唯一**判据就是 build_id 字符串」 | **措辞馊了半句**：daemon 那条路今天走 `deploy_decision_at`，判据是两个（版本 + 落点那个文件在不在）。⚠ **它的实质警告一个字都没坏**（旧 daemon 报同一个 id ⇒ 判 `Skip` ⇒ 控制面静默不可用 —— 本波只给 `Deploy` 加了新理由，**没有**让「stale 但文件在」的 daemon 被换掉）。建议改法：把「唯一判据就是 build_id 字符串」改成「判版本那一维的唯一判据就是 build_id 字符串（`deploy_decision`）」。**纯 doc 注释，没有任何判据钉着这段文本**（现打：全仓搜这句话只命中它自己） |
| ⑶ | `remote-daemon-proto/src/main.rs`（`p1w-inbound-in-fingerprint` 那条，搜锚点 `的唯一判据是 build_id 字符串`） | 逐字「`sftp.rs::deploy_decision` 的唯一判据是 build_id 字符串」 | 同 ⑵。`main.rs` 逐字**不在任何写区**（`COMMON-PM`），所以连一行都没改 |
| ⑷ | `CHANGELOG.md:497` | 逐字「…装到 daemonPath（**已是最新则跳过**）…」 | 🔴 **刻意不动**：`§2` 逐字禁「不改 `CHANGELOG.md` 里已发布版本的历史条目 —— 那几条描述的是当时铺下去的东西，它们当时是真的」。⇒ 那句话今天与 `machine-card.ts` 那句按钮说明（改后逐字「…（版本已是最新、且落点那个文件在，才跳过）」）不再一致，**而这个不一致是对的** |

⚠ **还有一格是 PM 的账，不是写区问题**：最后一趟门禁 `pb check` **FAIL=1**，
逐字 `FAIL   [J3 陈账] INDEX.md 比源文件旧 —— 重跑 pb index 落盘`。
现打的因果：`backend-consolidation/INDEX.md` mtime **09-04 21:14**，而 `K-W1C` / `K-R2` / `K-W3` /
`K-R24` 四份**别道的**件文件 mtime 分别是 **22:08 / 22:11 / 22:14 / 22:17**。
**本件的件文件不在那张变动清单里 —— 我一次都没写过计划仓的任何文件。**
按 `COMMON-PM` 与铁律 19，窗口开着期间我不跑生成命令 ⇒ **`pb index` 归 PM**。

## 9 · 门禁九格逐格（交付尖，与 `COMMON-PM` 的派工基线逐格对）

| 格 | 派工基线（`k-r25` @ `d305ffa`） | 本树交付尖 | 差 · 为什么 |
|---|---|---|---|
| cargo | 1390（8 包合计） | **1394** | **+4** = 本波四条新判据（`monitor` 包 1280 → 1284；其余 7 包 110 不变。1394 − 1284 = 110 ⇒ 与基线 1390 − 110 = 1280 对得上） |
| generated | ok | **ok** | 0（没动 `ts-rs` 面） |
| daemon | 542 | **542** | 0（`remote-daemon-proto` 一个字节没改） |
| npm | 1541 | **1541** | 0（`machine-card.ts` 只改一句按钮说明 + 三行注释）。**没有判据钉着那句话**，现打两条：① 改后搜旧句 `已是最新则跳过` 在 tracked 里只剩 `CHANGELOG.md:497` 与我自己写的两处注释/证据；② 在 `src/` 与 `e2e/` 的 `*.vitest.ts` / `*.test.ts` 里搜旧句与新句，**两句都是 0 命中** |
| ccm e2e ×4 | 12 / 8 / 242 / 68 | **12 / 8 / 242 / 68** | 0 |
| pb check | FAIL=0 | **FAIL=2 BROKEN=0** | **+2，两条都在计划仓、都不是本件的账**（下表） |

**`pb check` 那两条红，逐条**：

| 逐字 | 谁的 |
|---|---|
| `FAIL   [J3 陈账] INDEX.md 比源文件旧 —— 重跑 pb index 落盘` | **PM**（`INDEX.md` 21:14 vs 四份别道件文件 22:08–22:17） |
| `FAIL   [J5 表格列数] features/K-W3-合并本体先摸底再拆件.md:556 表头 3 列，本行 2 列 —— 少一列就数不到那一列（交账清单靠它倒出）` | **K-W3 那一道**（那份件文件在它的写区，不在我的） |

🔴 **本件在计划仓一个文件都没写过**（写区里那份 `features/K-W4-…md` 也没写）⇒ 上面两条与本件无因果。

**跑了几趟、每趟什么**（同一份字节先后两趟不同答的都在这里，一趟不藏）：

| 趟 | 树的状态 | 结果 |
|---|---|---|
| 1 | `adc10cf` 的内容（D4 实现 + 四判据） | **`GATE: OK`**，九格全绿 |
| 2–5 | 变异刀1–刀4 | 各 1 红在预期那一格（详见第 4 节）；其中刀2 附带 `daemon` 抖动、刀4 附带 `npm` 超时，见第 5 节 |
| 6 | `e6c9e4b`（+ D4 读数落盘） | `npm` 两条**超时**（`eslint-baseline` 120s · `ipc/commands` 5s）· `pb check` FAIL=1 |
| 7 | 同 6，重跑 | **`npm` 1541 ok** ⇒ 第 6 趟那两条坐实是资源争用（当时 13 路并发 + PM 预热）。`pb check` 仍 FAIL=1 |
| 8 | 自查修（两处，见下）落盘后 | `cargo` 1 红 = **`COMMON-PM` 登记的那条 flaky**，报文逐字 `spawn 假终端应成功: "spawn 本地命令失败: Text file busy (os error 26)"`（`src/launch.rs:1460`）⇒ 按规定登记后重跑 |
| 9 | 同 8，重跑 | **八个代码格全绿、逐格等于基线**；只剩 `pb check` FAIL=2（上表两条） |

## 10 · 自查（`brief` 15：拿本轮的病理回头打自己的代码）—— 自抓两处

| # | 自抓到什么 | 落在哪 | 改法 |
|---|---|---|---|
| ⑴ | **复述了一个基数**：`TargetBinary` 的 docstring 原本写「故它是**四态**而不是 `bool`」——`brief` 13b 逐字「报一个基数也是复述」，加一个成员那天这个字面量就成假话。⚠ 而**同一份文件里的先例也带着这个病**：`interpret_profile_read` 的 docstring 逐字写着「拆成**三态**」 | `sftp.rs` `TargetBinary` docstring | 改成「**所以它不是 `bool`**」+ 一句「成员就在下面，别在散文里复述一份基数」。⚠ `interpret_profile_read` 那处**没动**（不在本件射程，登记在此） |
| ⑵ | **写了一句我没实测过的话**：docstring 原本写「用户看到的是「连不上」，不是「那个文件不在」」——照抄了件文件 `§0c` 的措辞，而**现打把真串找出来了，不是这一句**（见第 2.1b 节） | `sftp.rs` `deploy_decision_at` docstring | 换成从源码摘的**逐字串** + 一条诚实边界「本条没实测过的部分：这是源码里的串，不是真机跑出来的读数」 |

★ 两处都是「**病复发在治它的代码里**」那一族：本件通篇在治「一个值装了两件事」与「读数要带口径」，
而这两处恰恰是**基数字面量**与**没有口径的转述**。
