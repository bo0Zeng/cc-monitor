...
⚠ **提交条数以 `git log afda78b..track/k-r112` 现打为准，别抄这一节里的数**：
上面那张表是**按写区归属**分的（① / ②），不是按提交条数分的 ——
本节写完之后又落了一个只动 `evidence/K-R112-deathvalue.md` 的补读数提交
（`M5` 那一趟的读数）。**一句话里嵌了每轮都变的量，它下一轮自动变成假话**（固定项第 12 条）
⇒ 这里只给现算的命令，不写死那个数。
