 ...
 * | 住址 | 形态 |
 * |---|---|
 * | `src/bridge/…/fixtures/agent-profile-golden.tsv` | 自称「agent 适配表的**唯一真相源**」，8 行 / 4 个 key |
 ...
 *
 * ```text
 * src/bridge/src/adapter.rs::agent_profile_facts
...
} from "../src/generated/agent-profile-table";
import { stripComments } from "./test-support/strip-comments";

const REPO = resolve(__dirname, "..");
const GOLDEN = "src/bridge/src/backend/control/fixtures/agent-profile-golden.tsv";
...
const ADAPTER_RS = "src/bridge/src/adapter.rs";
...
describe("K-R93 前端那份 agent 画像：值来自后端", () => {
  ...
  it("★ 生成物是生成物：头上写明谁生成的、且写着不许手改", () => {
    ...
    expect(src, "生成物头上没写它是谁生成的").toMatch(
      /^\/\/ 本文件由 `src\/bridge\/src\/adapter\.rs` 的 `export_bindings_agent_profile_table` 生成$/m,
    );
