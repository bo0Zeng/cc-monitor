...
# 而 `tests/e2e/` 里那几套**跑在一台没装后端的机器上**（沙箱容器 / CI runner），
...
command -v jq >/dev/null 2>&1 || {
  echo "tests/e2e/fake-daemon.sh: 需要 jq —— 它要解析 --launch/--list-accounts 那条 JSON。" >&2
...
case "$_sub" in
  ...
  --launch)
    ...
    [ -n "${FAKE_DAEMON_TMUX_SOCK:-}" ] || {
      echo "tests/e2e/fake-daemon.sh: --launch 需要 FAKE_DAEMON_TMUX_SOCK=<私有 socket 名>。" >&2
