#!/usr/bin/env bash
# 建干净 Linux 容器台架的镜像。**本目录唯一允许出现宿主网络的一步**（同 `tests/e2e/weak-net/build-image.sh`）。
#
...
# 这一段与 `tests/e2e/weak-net/build-image.sh` 是同一条现实，不是抄来的措辞。
#
# ## 为什么这在红线之内
#
...
#
# 用法：
#   bash tests/e2e/local-backend-container/build-image.sh            # 镜像已在就跳过
#   bash tests/e2e/local-backend-container/build-image.sh --force    # 强制重建
set -uo pipefail
