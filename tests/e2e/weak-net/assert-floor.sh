#!/usr/bin/env bash
# 跑弱网台架，并断言它的运行期 PASS 数不低于地板 —— `tests/e2e/assert-pass-floor.sh` 的同族。
#
# ## 为什么另起一份，而不是直接用 `tests/e2e/assert-pass-floor.sh`
#
# 那一份的第一件事是 `npm run --silent "test:$SUITE"` ⇒ **要在 `package.json` 里有一条
# `test:weak-net`**。而 `package.json` **不在件 `W-F1` 的写区里**（写区只有 `tests/e2e/`、
...
#
# 用法：bash tests/e2e/weak-net/assert-floor.sh <地板>
set -uo pipefail
