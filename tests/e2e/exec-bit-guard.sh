...
# 已经踩过两次：
#   ① R00：`src/shared/ccm` 记成 644 → CI 上 `-bash: …/shared/ccm: Permission denied`，
...
# `src/shared/` 一旦改名/移动，或 git 在 CI 容器里因任何原因失败（如 dubious ownership），
...
# `tests/e2e/` 下**约 20 个**带 shebang 的 .sh 是 100644 且**故意的**——它们一律经 `bash "$X"`
# 调用（见 tests/e2e/resume-suite.sh:52、tests/e2e/graylight-suite.sh:63），不依赖自身可执行位。
# 所以这里**不能**把 glob 放宽到 `tests/e2e/`（会误伤那 20 个），而是显式白名单：
...
ALLOWLIST=(
  ...
  # 自己在干净检出上跑不起来。
  tests/scripts/verify-committed-state.sh
)
...
#
# **为什么这里必须是显式白名单，不能沿用 src/shared/ 那套「有 shebang 就该 755」**：
...
fail=0
checked=0
# **按作用域分开计数**：只统计总数不够——白名单那 2 个恒在，会把"src/shared/ 整个消失"
...
check_one() {
  ...
  # （审计 S1：目前 src/shared/ 无 symlink，属潜伏；宁可显式跳过，也不给出一条无效的修复建议。）
  if [ "$mode" = "120000" ]; then
...
list="$(mktemp)" || exit 1
trap 'rm -f "$list"' EXIT
if ! git ls-files -z -- src/shared/ > "$list"; then
...
while IFS= read -r -d '' f; do
  ...
  # 今天 src/shared/ 下文件都小于管道缓冲区所以碰不到，但不能把正确性寄托在文件大小上。
...
if [ "$checked_shared" -eq 0 ]; then
  echo "===== 守卫自检失败：src/shared/ 下 0 个带 shebang 的受检文件 —— 该目录改名/移动了？ ====="
  echo "      （白名单条目仍在，故总数非 0；但 src/shared/** 这个主作用域已失守）"
  exit 1
fi
...
if [ -d "$UPSTREAM" ] && [ -d src/shared/cc-bus ]; then
  if ! diff -r -q --exclude='*.bak*' "$UPSTREAM" src/shared/cc-bus > /dev/null 2>&1; then
    echo
    echo "警告（不阻断）：src/shared/cc-bus 与 $UPSTREAM 已漂移。"
    echo "  差异： diff -r --exclude='*.bak*' \"$UPSTREAM\" src/shared/cc-bus"
