...
vi.mock("../../src/account-chip", () => ({ pickPrimaryOrigin: () => "aya" }));
...
vi.mock("../../src/account-usage", async (orig) => ({
  ...(await orig<typeof import("../../src/account-usage")>()),
  fetchAccountUsage: vi.fn(async () => outcome.value),
}));

import { UsageView } from "../../src/views/usage-view";
