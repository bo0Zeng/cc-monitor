 ...
 *
 * 换成**「不指定账号」**：它说的正是用户在做的选择。
 * 选这个说法而不是新造词，还有一条现成依据 —— `machine-card.ts` 里那条本来就写着
 * 「不指定（…）」，跟着既有措辞走比再发明一个词好。
 *
 * ⚠ **本段原本还有一句「（不指定，用登录时已有的那个）」，已删** —— audit-0805 F12 实测
 * 那句话在主路径上**是假的**：`kind:"base"` 经 `ACCOUNT_DIMENSION`（`applies` 恒真）必发
 * `--base`，而 `shared/ccm` 收到 `--base` 是 **`unset CLAUDE_CONFIG_DIR`**（`:674` + `:709`），
 * 落 `~/.claude` 而**不是**「登录时已有的那个」。
 * ⇒ **本条规则不变**（「基座」仍不许出现在用户文案里）；变的是它当初用来举例的那句话。
 * 「说什么词」由本条管，「说的是不是真的」由 `tests/account-base-semantics.vitest.ts` 管，
