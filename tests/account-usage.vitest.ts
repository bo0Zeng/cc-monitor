...
import { fetchAccountUsage, invalidateAccountUsageCache } from "../src/account-usage.ts";
...
describe("KR101D2 生产路零解析，而解析层三样都还在盘上", () => {
  ...
  it("★ 阳性对照：这把尺子真的逮得住 —— 合成一份含调用的生产段样本必须被认出来", () => {
    const synthetic = stripComments(
      'import { parseUsageCapture } from "../src/account-usage-parse.ts";\nconst r = parseUsageCapture(raw);\n',
      "ts",
    );
  ...
  it("★ ③ 退役不是删除：解析器 ＋ 它的 vitest ＋ 冻结夹具，三样都得在盘上", () => {
    for (const rel of [
      ...
      "tests/account-usage-parse.vitest.ts",
      "tests/__fixtures__/usage-capture-2026-07-31.txt",
    ]) {
    ...
    // 而且它真的在跑那个函数（不是一份空壳）。
    const t = readFileSync(resolve(REPO_ROOT, "tests/account-usage-parse.vitest.ts"), "utf8");
