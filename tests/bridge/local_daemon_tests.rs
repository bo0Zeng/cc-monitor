...
///   monitor ↔ daemon，而 `src/bridge/crates/` 与 `src/bridge/src/` 同属这一半。
...
fn relay_running_really_reads_the_handle_table() {
    ...
    let prod = guard_core::production_code(include_str!("../../src/bridge/src/local_daemon.rs"));
...
fn the_relay_has_a_named_starter_and_it_runs_before_the_detached_branch_returns() {
    ...
    let me = include_str!("../../src/bridge/src/local_daemon.rs");
...
/// 而 **CI 的 `cargo test` 之前一步都不铺 `src/bridge/embedded-daemons/`**
...
/// 在**同一个 `CARGO_TARGET_DIR`** 里把 `src/bridge/embedded-daemons/` 从「无」加成「有」，
/// **`build.rs` 不重跑**，第一趟读到 `1208 / 0 / 10`，`touch build.rs` 之后才是 `1211 / 0 / 11`。
/// ⚠⚠ **这个坑看不出来**：两档的输出面长得一模一样，读到的是「无 emb」那一档的数。
///
/// ⇒ **纪律**：换 emb 状态之后 `touch src/bridge/build.rs`；
...
fn the_stop_command_really_calls_this_module() {
    let dc = guard_core::production_code(include_str!("../../src/bridge/src/daemon_control.rs"));
    ...
    let me = guard_core::production_code(include_str!("../../src/bridge/src/local_daemon.rs"));
...
fn detached_reads_the_path_that_was_taken_not_a_guess() {
    ...
    let remote = crate::daemon_control::daemon_status("某台远端".into()).expect("查远端状态");
    assert!(
...
fn the_two_resolution_paths_still_agree_on_the_order() {
    ...
    let mine = guard_core::production_code(include_str!("../../src/bridge/src/local_daemon.rs"));
...
///      `src/bridge/build.rs`（`Path::new("embedded-daemons")`，**是代码不是散文**）·
///      `src/bridge/src/tool_registry.rs`（登记表数据）· `src/bridge/src/sftp.rs` ·
///      `src/bridge/src/write_site_registry.rs` · `src/backend/main.rs` ·
...
///    〔08-31 实打：第一版就是这么红的，`left: [("src/bridge/src/local_daemon.rs", 2), …]`。〕
...
///    全部量于 `4eb271f`、本树未铺 `src/bridge/embedded-daemons/`、跑在沙箱里**：
...
///
/// ★ 这撞的是 `lib.rs` 自己写下的分档标准〔逐字，引的是 **2026-09-10 之前**那一版原话，
/// 保着不改 —— 这一段说的是当时那次判断〕：「**拒绝**（口上有东西、接不上）=
...
/// 「这一份产物里没带 sidecar」；**分档标准本身、本条判据的靶，两样都一格没动。**
...
fn the_user_actionable_start_failures_all_reach_the_user() {
    ...
    assert_eq!(
        ...
        "`StartOutcome::Failed` 的构造点不是 6 处了 —— \n\
             ...
             诚实降级（**这一份产物里没带 sidecar** 那种 —— 裸 exe / 开发树 / 释放内嵌也失败）\n\
             ⇒ 仍走日志。\n\
             ⇒ 改完把这个数改对，**别只改数**。"
    );
    ...
    let enclosing = |needle: &str, skip_def: bool| -> Vec<String> {
        ...
        for l in prod.lines() {
            ...
            if l.contains(needle)
                && !(skip_def && t.starts_with("fn "))
                && cur != needle.trim_end_matches('(')
            {
                out.push(cur.clone());
            }
        }
...
///
/// 现打（09-04，本工作树，**未铺** `src/bridge/embedded-daemons/`）：
/// `src/bridge/src` 下 **105 份 `.rs`，走兜底 0 份**。
/// 地板 100 与 monitor 全树那条「剥得干不干净」的自检同职：**扫到的份数低于它说明遍历坏了**，
/// 不是代码变干净了。⚠ 棘轮纪律：只许升不许降。
///
/// # ★★ 09-04 落定拍：**两个数复打成立；而上面那句「所有判据一个数都不动」被打穿了一半**
///
/// 现打（量于 `4eb271f`，本树未铺 `src/bridge/embedded-daemons/`，跑在沙箱里）：
/// **105 份 / 走兜底 0 份**，与上一拍逐字相同 —— 但这是量出来的，不是抄的
/// （分母 = `find src/bridge/src -name '*.rs'`，与本条的遍历同语义）。daemon 侧 **73 / 0** 同样成立。
///
...
/// （`guard_core::block_comment_model_holds`）按 `#[test]` 切块普查 `src/bridge/src`
...
impl E2eSandbox {
    fn demand() -> Self {
        ...
        let work =
            std::path::PathBuf::from(std::env::var("CCM_E2E_WORK").expect("要 CCM_E2E_WORK"));
        Self {
...
fn e2e_a_second_host_adopts_the_running_daemon_instead_of_starting_a_second_one() {
    #[cfg(target_os = "linux")]
    {
        ...
        *DETACHED.lock().expect("锁") = None;
        ...
        expect_started(
            start_detached(&|| Ok(bin.clone()), &sb.envs()),
            "第一个宿主",
        );
        ...
        expect_adopted(
            start_detached(&|| Ok(bin2.clone()), &sb.envs()),
            "第二个宿主",
        );
