...
vi.mock("../../src/format", () => ({ formatBytes: (n: number) => `${n}B` }));
vi.mock("../../src/error-toast", () => ({ showActionFailureToast: vi.fn() }));
vi.mock("../../src/remote-launch", () => ({ buildOpenTerminalCmd: (c: string) => `cd '${c}'` }));

import { SftpPanel } from "../../src/sftp/panel";
import type { SftpEntry } from "../../src/sftp/paths";
...
describe("F54 open(revealPath) 定位高亮", () => {
  ...
  it("★ P6a-D：每条写远端的路都先 `sftp_stat`，例外要登记理由", () => {
    ...
    const src = readFileSync(resolve(srcDirOf(__dirname), "panel.ts"), "utf8");
