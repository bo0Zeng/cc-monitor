...
//!   **曾经就在 `plugin/discover.rs` 活着，而人群够不着它**。
//!   ⚠ 〔`K-R52` 09-11 订正，**同轮摘登记**〕那个**活体今天没了**：那一处已改成保守方向的
//!   `false`，而它够不着的那部分人群由 [`super::cfgless_guard`] 补上了（`src/` 全树）。
//!   ⇒ 「不构成保护」这个定性**对本护栏自己仍然成立**（人群还是只有 `platform/` 一个目录），
//!   但它今天只剩 [`g6_reach::counterexample_b_the_shape_still_passes_outside_the_population`]
//!   那份**合成**证据 —— 活体证据与合成证据不是一回事，别把这一行读成前者。
...
/// - **乙** 证「**不构成保护**」：本护栏来历里那个地雷的**逐字同形**，
///   放在 `platform/` 之外就**今天通过** —— 人群够不着它。
///   ⚠ 〔`K-R52` 09-11〕这一条**从活体退成了合成**：原先的活体是
///   `plugin/discover.rs` 那个裸 `true`，它已被修掉（同轮摘登记）。
///
/// ★★ 合成一条会让下一个人以为**一条反例就够** —— 而那正是本轮要治的病。
#[cfg(test)]
mod g6_reach {
    ...
    /// ★★ **反例乙**（证「不构成保护」）：本护栏来历地雷的**逐字同形**，
    /// 放在 `platform/` 之外就今天通过 —— 人群够不着它。
    ///
    /// # 〔`K-R52` 09-11〕这一条**从活体退成了合成**，两件事一起记
    ///
    /// 原先它钉的是一个**活体**：`plugin/discover.rs::is_executable` 的 `#[cfg(not(unix))]`
    /// 块体是一个裸 `true`。那一处 `K-R52` 已经修掉（改成保守方向的 `false`），
    /// 于是本条当场红了 —— **那是它干对了活**（它的失败文案逐字写着「要么它被修好了
    /// （好事 —— 同轮摘登记）」）。这里就是那一次摘登记。
    ///
    /// 🔴 **摘掉活体之后，这条反例买到的东西变小了，写明别读大**：
    /// 它现在证的是「**同一个形状，换个住址就不红**」，用的是合成样本；
    /// 它**不再**证「今天真有一处这样的代码活着」—— 那句话今天是假的。
    /// （本仓的纪律 14r：「断某个对账今天该是空的」是**空真** ⇒ 合成样本是它的正确形状，
    ///  但也仅此而已。）
    ///
    /// # 三个断言合起来才是这条反例
    ///
    /// ① 合成样本喂给**判据本体**，它**会红** ⇒ **形状对得上**（不是「假想的坏写法」）；
    /// ② `plugin/` 不在人群里 ⇒ 换个住址就没人看 ⇒ **不构成保护**；
    /// ③ 那个曾经的活体今天**真的被修好了**（不是被绕开、也不是判红条件被改松）——
    ///    ⇒ 断言它那一臂现在是一个**诚实**取值。少了③，本条会退化成
    ///    「我换了个合成样本，所以不红了」，而那与「判据被改松」在输出上一模一样。
    ///
    /// ⚠ **人群那一格今天有人管了**：[`super::cfgless_guard`] 的 B 族扫 `src/` 全树
    /// （扣掉 `platform/` —— 那一格归本护栏，同一条判红条件不许两处各写一份）。
    ///
    /// 🔴 **这一行 `#[test]` 是 `K-R52` 当轮重新补上的，别再弄丢**：改写本条的头注那一拍
    /// 把它连同旧头注一起换掉了，于是本条**静默地不再运行** —— 判定行从 643 掉到 642，
    /// 而全表**一片绿**。逮住它的不是任何一条判据，是本轮那张变异表（`M2` 明明该打红它，
    /// 读数却只有一条新红）。⇒ 「跑完先核判定行数」这条纪律买到的就是这一次。
    #[test]
    fn counterexample_b_the_shape_still_passes_outside_the_population() {
        // ① 形状对得上：那个地雷的逐字同形喂给判据本体，它红。
        assert!(
            !fabricates("{\n    true\n}").is_empty(),
            "连合成的同形样本都不红了 —— 那就不是人群问题，是判据本身坏了"
        );
        // ② 而人群按**目录**画：`plugin/` 不在里面 ⇒ 同一段代码换个住址就没人看。
        assert!(
            !platform_sources()
                .iter()
        ...
        assert!(
            body.contains("false"),
            "`plugin/discover.rs` 的非 unix 臂不再是那个保守取值 `false` 了 —— \n\
             它现在是：{body}\n\
             ⇒ 要么换了别的**诚实**取值（那就回来把这条断言改成新的那个），\n\
             要么判红条件被改松了（坏事）。别让这一格靠「什么都不写」变绿。"
        );
    }
}
...
mod g6_scope_pins {
    ...
    /// # 它治的是一次已经发生的「只修一半」
    ///
    /// `src/doc/INVARIANTS.md` §41.6 早就把那句标成**原措辞**并给了现措辞，
        ...
        for word in &forbidden {
            assert!(
                ...
                "`readonly_guard.rs` 里又出现了 `{word}` —— 那是 `D1` **收窄前**的说法，\n\
                 而那份文件里的判据兑现的是**收窄后**那句（它只认本 crate 源码文本里\n\
                 那三个命名空间的调用，既不认起进程、也不认依赖 crate）。\n\
