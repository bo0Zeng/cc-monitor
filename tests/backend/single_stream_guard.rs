...
//!      逼判据自己印出真值，是复跑这个分母的权威路子（`tests/evidence/kg5-c3-cuts.py` 的 `M1`）。
...
//!   6. 剥法的边界在 `src/bridge/crates/guard-core/src/lib.rs`。
...
mod tests {
    ...
    /// `src/backend` 下把「不是整份文件」的文本交进剥法的调用点 **0 处**
    /// —— 也就是说那两刻这一侧**没有**块级的活体洞，本条的块那一半在这一侧是**预防**，不是修补。
    /// ⚠ **这是那两刻的读数，不是不变量** —— 引用前重跑量具。
