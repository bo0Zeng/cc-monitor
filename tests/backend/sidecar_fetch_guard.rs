...
mod tests {
    ...
    ///
    /// ⚠ 〔09-10 接线那一拍〕**那一拍做了三跳的本体，而本条仍然绿，这是对的**：
    /// 第一个生产调用方只能长在 `inbound.rs` / `main.rs` 上，而那一拍的写区不含它们。
    /// ⇒ 本条今天钉住的仍是同一句话：**这一层还没被接上**。
    #[test]
    ...
    /// ⑧ 🔴 那一行 `allow` **只许静默死码这一种，而且只许有这一处**〔`K-W2D` 09-10〕。
    ///
    /// # 它治的是什么（现打的读数，不是设想）
    ///
    /// 判据 ⑦ 是「什么时候**摘**掉它」的闹钟，而在摘掉之前，那一行本身是一个**没人看着的洞**：
    /// 把它放宽成 `unused`（或在层里某个文件上再挂一个），编译器从此对这一层的
    /// **未用导入 · 未用变量 · 不可达代码** 全部闭嘴，而 ⑦ 一个字都不会说
    /// —— 它数的是调用点，不是静默面。
    ///
    /// 09-10 那一拍现打过这一行的份量：把它拿掉，`cargo build` 从 **0 条**警告变成 **45 条**
    /// `never used`。⇒ 这一行今天**盖着 45 条**；它多盖一类，就是多藏一类。
    ///
    /// # 它认不出什么（如实登记，**这一段是本轮自查订正过的**）
    ///
    /// 🔴 初版这里写的是「认不出 `#[allow(…)]` 写在函数或语句上的那一形」——**那是假的**：
    /// 采集面是「生产段里**任何一行**含 `allow(`」，item 上的 `#[allow(…)]` 照样进人群
    /// （`W20` 那一刀打的正是这一形，一刀就红）。写下那句话时我没打它，**先写后核就是这么错的**。
    ///
    /// 它真正认不出的是这几形：`#[allow (dead_code)]`（名字与括号之间有空格）·
    /// `#[expect(…)]`（另一个词）· `#[cfg_attr(…, allow(…))]` 里那半（它含 `allow(`，**会**被认出来，
    /// 但会被当成一处普通站点报出来，说的话不准）。
    /// ⇒ 本条买到的是「**盘上今天那一处静默不许变形、不许变多**」，
    /// **不是**「整层没有任何静默」。别把它读大一格。
    #[test]
    fn the_layer_silences_only_dead_code_and_only_in_one_place() {
        // 拼出来，免得本文件里这几行自己被当成语料（本条读的是 `sidecars/` 那棵树，
        // 而拼字符串这一手是这个仓的通行做法：让判据在自己的文本里找不到自己）。
        let want = format!("#![{}(dead_code)]", "allow");
        ...
        for (rel, code) in &files {
            for line in code.lines() {
                if line.contains(needle.as_str()) {
                    sites.push((rel.clone(), line.trim().to_string()));
                }
            }
        }
        assert_eq!(
            sites.len(),
            1,
            "这一层里静默编译器的地方有 {} 处（应恰好 1 处）：{sites:?}\n\
             多一处 = 静默面在扩散，而判据 ⑦ 数的是调用点、对这件事一个字都不会说；\n\
             零处 = 那一行被摘了 —— 若同轮 ⑦ 也红了，那是接线，本条跟着改；\n\
             若 ⑦ 仍绿而它没了，构建里会多出几十条 `never used` 噪音，真警告从此藏在里面。",
            sites.len()
        );
        assert_eq!(
            sites[0].0, "sidecars/mod.rs",
            "那一行搬家了：现在住 `{}`。它该住层的 `mod.rs` —— 挂在某个文件上就变成了\
             「这个文件被豁免」，而它要表达的是「整层还没被接上」。",
            sites[0].0
        );
        assert_eq!(
            sites[0].1, want,
            "那一行被改写了：现在是 `{}`，而它只许是 `{want}`。\n\
             ★ 放宽成 `unused` 那一族，会把未用导入 / 未用变量 / 不可达代码一起盖掉，\n\
             而这一层今天正盖着几十条死码 —— 多盖一类就是多藏一类。",
            sites[0].1
        );
    }
}
