...
| **mcp.rs** (F87 issue #50+#51) | MCP 管理（SS-14 读写分界）：**读**跨 scope 宽容展示（用户 `~/.claude.json` / local / 项目 `.mcp.json`，缺/坏一律跳过、server 原样保留）；**写只**项目 `<dir>/.mcp.json`（`mcp_json_path` 硬编码，绝不写 `~/.claude.json`/`settings.json`） | `read_mcp_servers / list_mcp_project_dirs / write_project_mcp_server / remove_project_mcp_server` |
