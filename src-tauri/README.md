...
| **tmux.rs** (B14-F51) | tmux 反查（tab 右键 attach）：`parse_tmux_ls`（**真 TAB 分列**，调研 03 §3.1 坑）+ `list_remote_tmux`（`command -v tmux` 门控→哨兵 NO_TMUX 返 None；`tmux ls -F` 走 `connect_and_exec_cmd` 只消费不改形）。前端按 cwd+`pane_current_command==claude` 反查。F52/F58 续挂管理类命令 | `parse_tmux_ls()` + IPC `list_remote_tmux` |
