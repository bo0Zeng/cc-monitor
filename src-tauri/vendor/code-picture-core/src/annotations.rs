...
fn dir(repo: &Path) -> PathBuf {
    repo.join(".codepicture").join("annotations")
}
...
pub fn list(repo: &Path) -> Vec<Annotation> {
    let entries = match fs::read_dir(dir(repo)) {
        Ok(e) => e,
        Err(_) => return vec![],
    };
    let mut out = Vec::new();
...
pub fn get(repo: &Path, id: &str) -> Option<Annotation> {
    if !valid_id(id) {
        return None;
    }
    let content = fs::read_to_string(dir(repo).join(format!("{}.json", id))).ok()?;
    serde_json::from_str(&content).ok()
}

/// 写一条批注(**原子**:temp + rename,防半截丢批注);首写建目录 + `.gitignore`。
pub fn write(repo: &Path, ann: &Annotation) -> std::io::Result<()> {
    if !valid_id(&ann.id) {
...
pub fn remove(repo: &Path, id: &str) -> std::io::Result<bool> {
    if !valid_id(id) {
        return Ok(false);
    }
    let p = dir(repo).join(format!("{}.json", id));
    if p.is_file() {
        fs::remove_file(p)?;
        Ok(true)
    } else {
...
pub fn ensure_gitignore(repo: &Path) -> std::io::Result<()> {
