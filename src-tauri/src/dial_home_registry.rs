...
//! 两处都是读数逼出来的，不是口味：
//!
//! 1. **家今天是二元闭集，不是一元。** 现打（生产段，`production_code` 剥过注释与测试段）：
//!    锚点住 `ssh_source.rs` 与 `port_forward.rs` 两份文件。
//!    要把它写成一元，只有两条路 —— **搬代码**（本件明写不做）或者
//!    **把锚点集收窄到只剩住在 `ssh_source.rs` 里的那两个**（`client::connect(` /
//!    `client::connect_stream(`，丢掉 `channel_open_direct_tcpip(`）。
//!    🔴 后者正是件计划点名的失效方向：**把住址定在方便的地方，而不去问它该不该是那个家**。
//!    ⇒ 锚点集与 daemon 那份**逐字相同**，家诚实地记成两条。
...
//!
//! **它会不会随 `K-P6` 那三拍搬走 —— 会，而且是按这个次序：**
//!
//! | `K-P6` 那一拍 | 这个家会怎么动 |
//! |---|---|
//! | 第 2 步（C 类：SFTP + 端口转发整个动作搬进后端） | `port_forward.rs` 那一行**整行消失** ⇒ 家收成一元 |
//! | 第 3 步（B 类：`connect_and_exec_cmd` 那 35 个调用点 → 一条后端命令） | `SITES` 不动，[`unmoved_dial_sites_in`] 那个数往下走 |
//! | 第 4 步（A 类剩余：跳板 · 测试连接跟代理走） | `ssh_source.rs` 那一行也消失 ⇒ **家变成空集** |
//!
