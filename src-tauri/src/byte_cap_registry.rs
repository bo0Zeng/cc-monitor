...
mod tests {
    ...
    const CAPS: &[(&str, &str, u64, &str, &str)] = &[
        ...
        // 以为它只管请求那一侧：
        //   `relay/server.rs:97`  读**下游请求**头 ⇒ `read_head` 返回 `None` ⇒ 回 `400 Bad Request`
        //   `relay/server.rs:134` 读**上游响应**头 ⇒ 同一个 `read_head`  ⇒ 回 `502 Bad Gateway`
        // ⚠ 同一份代码里 `relay/http1.rs:39` 的头注写的是「回 431」，与两个调用点都不符 ——
        //   那是那边的旧注，本表登记的是**调用点实测的处置**，不是那句头注。
        (
