...
mod tests {
    ...
    // ② **把真调用留成多行块注释、换个桩值** —— 内层那一行剥不掉、照旧被读成一条合法绑定
    ...
    ///   （`:1359`，它经上面那个 `const` 取，所以**不在**刚才那 7 行里）
    ///   ＋ 两条 `let _ = std::env::var(..)` 的**纯读点名行**（`:4046` / `:4113`，
    ...
    ///   🔴 **`C` 第七拍（09-01）第三次兑现了这一条，而且是自己撞的**：本拍在 `:1259` 一带
    ///   与守卫头注里加了注释 ⇒ 三个数全部推后（`:1343`/`:3911`/`:3978` →
    ///   `:1359`/`:4046`/`:4113`，文件 4002 → **4137** 行）。⚠ **前三遍写的 `:1354` / `:1358` /
    ///   `:4042` 一族都还是错的** —— 每写一句订正就又往上面加了行，**这一段三次把自己推后**。
    ///   那 7 处 / 5 · 2 的分布**重打后没变**（本拍新加的字全在 `//` 与 `///` 行上，进不了这个分母）。
    ...
    ///    **不跟踪块注释的开合状态** ⇒ 多行块注释的**内层行**一个字都不会被剥掉。
    ...
    fn every_test_that_starts_the_real_daemon_demands_a_private_tmux() {
        ...
        const MISS_NO_BINDING: &str = "含取 shim 那个口的那一行**读不出绑定名**\
             ...
             （`/*` 与 `*/` 各独占一行）—— 剥法是**按行前缀**的，内层那几行一个字都剥不掉。\
             内层行**不以 `let ` 打头**时就落到这里，而上面那句「实得那一行」\
