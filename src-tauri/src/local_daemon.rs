...
mod tests {
    ...
    ///    | 6h | ㈢ 换成**正向数写位**之后新出现的那一族：写位形状不是「`"PATH"` 后紧跟方法调用」的写法。我枚举出四种 —— ㈠ `(format!("PATH"), v)`（键名是字面量，但被 `format!` **裹了一层**）· ㈡ `("PATH", v)` 这种 `&str` 元组 · ㈢ `cmd.env("PATH", v)` · ㈣ `map.insert("PATH", v)` | 正向数只认那一种形状；**代价方向与减法相反**：减法只多算（假红），正向数只少算（假绿）—— 这四种**旧的减法口径都数得到**，是这次换口径**新掉出来的** | **0**（四种今天一处都没有）。⚠ 分母话，逐种交代：㈡ **`envs: Vec<(String, String)>` 的类型过不了**；㈢㈣ **根本不走 `envs` 这条路**（自己造 `Command` / 别的容器，属 6a 那一族）；**只有 ㈠ 是「类型过得了、也真进 `envs`」的** ⇒ **真能进 `envs` 又数不到的，今天是 6d（键名拼出来）＋ 本格的 ㈠ 这两格**，不是一格 |
    ///
    ...
    fn every_test_that_starts_the_real_daemon_demands_a_private_tmux() {
        ...
        //       **真正新掉出来的那一格**，登记进 `6h`。
        //       ⇒ 合起来：㈢ 今天的**假绿面 = 6d（键名拼出来）＋ 6h 的那一格**，**两格，不是一格**。〕
        //
        ...
        //   **只剩两格**（6d ＋ 6h 的 `format!` 那一格），而假红面在减法那边是**开放**的
