...
mod tests {
    ...
    ///   （`:1333`，它经上面那个 `const` 取，所以**不在**刚才那 7 行里）
    ///   ＋ 两条 `let _ = std::env::var(..)` 的**纯读点名行**（`:3898` / `:3965`，
    ...
    ///   **别当常量抄**。要重打就跑这一条（站在工作树根，与本段口径逐字相同）：
    ...
    ///    **机制一个字没变**。
    ///
    ///    **⇒ 这道闸买到的是「防手滑」，买不到「挡住存心绕」。**
    ///    🔴 **别再用「保证」「买断」「fail-closed 就还在」这类词描述它**（PM `§0s` 五 ③ 逐字）。
    ///
    ///    | | 说得准的话 | 依据（没重打的写明） |
    ///    |---|---|---|
    ///    | **买到** | 口里那句 `panic!` 被改成降级 ⇒ 门禁当场红 | `D5-M1` 新红 1、只点那条测试（**`C` 第六拍没重打**） |
    ///    | **买到** | 反空真那半格不是仪式（「恒炸」的实现过不了） | `D5-M9` 新红 1（**没重打**） |
