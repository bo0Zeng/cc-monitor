...
mod tests {
    ...
    ///    〔★ 09-04 `K-R9` 就地批注：**下面这条裁定后来被 PM 自己推翻了** —— `K-R9` 立件做的
    ///     恰恰就是「把剥法做成块注释开合配对」，而且是**在共用原语这一层**做的。
    ///     两者不矛盾：原裁定否的是「**在这条守卫里**再钉一个形状」（每钉一个，下一拍来下一个），
    ///     `K-R9` 改的是**所有源码形态判据共用的那一份剥法** —— 一处改，252 个调用点一起受益，
    ///     不是「钉形状」而是「换尺子的种类」。⇒ 别拿下面这句去否掉 `K-R9`。〕
