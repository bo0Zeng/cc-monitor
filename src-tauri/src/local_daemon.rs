...
mod tests {
    ...
    //   ⚠⚠ **它仍然管不到「调用排在 spawn 之前还是之后」** —— 那一格没钉，登记在
    ...
    ///       ⚠ **㈠ 买到 / 买不到，写准**：买到的是「**只要本体真的调到了那个口，缺变量就一定炸**」
    ///       —— 炸在口里，与调用处怎么写无关（`unwrap_or_else(|_| demand_tmux_shim(..))`
    ///       照样 fail-closed）。**买不到**的是「这条测试**走不走**那个口」：那仍然是一条
    ...
    ///       · ② fail-closed 取变量 —— **09-01 起也是机器守的**（`§0q` 裁一 · 出路乙）：
    ///         取 shim 收敛到 `demand_tmux_shim` 一个口，而
    ///         [`the_one_shim_gate_really_fails_closed`] 在**默认门禁里真跑那个口**。
    ///         ⚠ 仍是文本钉的是「**谁进了那个口**」（见上面 ㈠ 那段的「买不到」）。
    ...
    ///    停机条件**一个字都盖不到它们**。
    ///    ★ **09-01 就兑现过一次**：`D4` 找到了 ② 的绕过，`§0q` 裁一没有走登记，
    ///      而是**换了做法**（唯一入口 + 一条真跑的 fail-closed 测试）。这条豁免不是纸面上的。
    #[test]
