...
mod tests {
    ...
    ///    被测对象 = 本工作树 `track/k-r7` 尖 `aee8b9f`）：两个文件里 `trim_start()` 后以
    ///    `*` / `/*` 打头的行共 **19** 条（本文件 **17** = 10 条解引用赋值 ＋ 7 条多行字符串里
    ///    以 `**` 打头的续行 · `local_backend.rs` **2** 条，都是解引用赋值），
    ///    其中落在本条人群那 6 个块里的 **6** 条（三条测试各 2 条 `*XXX.lock()… = …;`）。
