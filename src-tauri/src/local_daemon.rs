...
mod tests {
    ...
    ///    | 6h | ㈢ 换成**正向数写位**之后新出现的那一族：写位形状不是「`"PATH"` 后紧跟方法调用」的写法 —— `cmd.env("PATH", v)`（自己造 `Command`，根本不走 `envs`，属 6a 那一族）· `map.insert("PATH", v)`（同上）· `("PATH", v)` 这种 `&str` 元组 | 正向数只认那一种形状；**代价方向与减法相反**：减法只多算（假红），正向数只少算（假绿） | **0**。⚠ 分母话：`("PATH", v)` 那种 **`envs: Vec<(String, String)>` 的类型过不了**；另两种**不走 `envs` 这条路** ⇒ **真能进 `envs` 又数不到的，今天只剩 6d 那一格**（键名不是字面量）|
    ///
    ...
    ///    ② **真正买到安全的是 ①②，不是 ③**：默认 `cargo test` **够不到**这些测试（`#[ignore]`）；
    ///       `--ignored` 裸跑**当场炸**（fail-closed，排在任何 spawn 之前）。
    ...
    fn every_test_that_starts_the_real_daemon_demands_a_private_tmux() {
        ...
        // ★ **为什么这个形状就是「写位」，而且它的分母给得出**：`envs` 的类型是
        //   `Vec<(String, String)>`（`local_backend::supervise_with_stdio` 的签名）
        //   ⇒ 一个 `&'static str` 字面量要当键放进去，**必须先走一次 `&str → String` 的转换**，
        //   而这样的转换只有四种写法：`.to_string()` / `.into()` / `.to_owned()`（都是「后面紧跟点」）
        //   ＋ `String::from(..)`（上面那一步归一掉）。**「读」是开放的，「写位」是这个类型封死的。**
        //
        ...
        //     · **写位探针**（分母 = 我枚举的 8 种写法）：正向数**数不到 4 种**。
        //       逐条核过之后，**真能进 `envs` 的只剩一种**：键名不是字面量
        //       （`(format!("PAT{}", "H"), v)` 这类）—— 那**正是已登记的 6d**，`M-δ2` 实证过。
        //       另三种（`("PATH", v)` 这种 `&str` 元组 · `cmd.env("PATH", v)` · `map.insert(..)`）
        //       **要么类型过不了，要么根本不走 `envs` 这条路**（后者属 6a 那一族）。⇒ 登记 `6h`。〕
        //
        // 🔴 **代价说清楚：这一换把出错的方向翻了个个儿。**
        //   减法那一版 PM 09-01 验算过：`path_writes = W + R₂ ≥ W` ⇒ **只会多算、永不少算**
        //   （错在**假红**这一侧，看得见）。正向数反过来：**只会少算、不会多算**
        //   （错在**假绿**这一侧，悄悄过去）。之所以仍然买它：假绿面被上面那个类型封死到
        //   **只剩 6d 一格**，而假红面在减法那边是**开放**的，且假红会**训练人绕过判据**
        ...
        assert!(
            ...
            "`E2eSandbox::demand()` 里 `PATH` 这个 env 键的**写位形状**数到 {} 处（要的是恰好 1 处）——\n\
             ...
             ⇒ 下面是 `demand()` 起 20 行剥完注释的原文，**自己核对哪一行是写、哪一行是读**：\n{demand}",
            path_writes(&demand)
        );
