...
mod tests {
    ...
    fn every_real_daemon_e2e_demands_a_private_tmux_dir() {
        ...
        //   （编辑器对齐块注释的默认产物）今天**剥得掉** ⇒ 两条守卫一起红（实打，新红 2）。
        ...
        for c in real_e2e {
                ...
                .unwrap_or_else(|| {
                    panic!(
                        "`{name}` 会起一个**真** daemon，却没走取 shim 的那个唯一入口 \
                         ...
                         （**静默的假绿**，逐条登记在 `local_daemon.rs` 正题守卫头注\
                         「诚实边界 9」，归跟进件 `K-R8`）。\n\
                         ⚠ **别把上一句读成全称**：内层行写成 ` * let …`（编辑器对齐块注释的\
                         默认产物）今天**剥得掉** ⇒ 整行消失 ⇒ 本条找不到它 ⇒ **红**\
                         （`C8` 09-01 实打，新红 2）。\
                         ⚠ **分母**：这里只登记我量过的这两形，\
                         「块注释这一族一共有几种走法」这个分母给不出。"
                    )
                });
            ...
            assert!(
                ...
                "`{name}` 里出现 `{GATE}` 的那一行（**剥掉「`//` / `*` / `/*` 打头的整行」之后**）\
                 ...
                 ③ 🔴 **块注释的内层行，只要自己不以 `//` / `*` / `/*` 打头就剥不掉** ——\
                 这一形是已知的、今天开着的：`/*` 与 `*/` 各独占一行时，那几行 \
                 `trim_start()` 之后仍是普通代码行 ⇒ 本条与下游 ㈡ 都会把它当真代码读。\
                 **下面是我量过的三形**（分母 = 这三形；\
                 「块注释这一族一共有几种走法」那个分母给不出）：\n\
                 ㋐ 内层行以 `let ` 打头 ⇒ **一次静默的假绿**（`D6` `M-D6-3`：一条起真 daemon 的 e2e，\
                 `PATH` 里一点 shim 都没有，全量门禁 `GATE: OK`、新红 0）；\n\
                 ㋑ 内层行既不以 `let ` 打头、也不以上面那三种前缀打头 ⇒ 落到本条与 \
                 `MISS_NO_BINDING`，那时上面那一行「实得那一行」印出来的**就是一条注释** ——\
                 那不是报文说假话，是这把尺子**本来就只剥整行**；\n\
                 ㋒ 内层行以 `*` 打头（` * let …`，编辑器对齐块注释的默认产物）\
                 ⇒ **今天剥得掉**、整行消失 ⇒ 落回上面那句 `panic!`（「没走那个唯一入口」）\
                 与下游 `MISS_LOCK2`，两条守卫一起红（`C8` 09-01 实打，新红 2）。\n\
                 一行式 `/* … */` 会被剥掉（它以 `/*` 打头），旧剥法下它曾是一形假阳（`D6-M1`）。\n\
                 ⚠ **分母**：本条人群今天 **1** 条、下游人群 **6** 条（两个文件切出 **58** 块；\
                 量于 09-01，量具 `scratchpad/kr7-c7-starline.py`，被测对象 = 工作树尖 `aee8b9f`）。\
                 **「块注释这一族还能怎么走」的分母给不出** ⇒ 这里只登记实打过的 ③ 那三形；\
                 逐条见 `local_daemon.rs` 正题守卫头注「诚实边界 9」，归跟进件 `K-R8`。",
                line.trim()
            );
        }
    }
