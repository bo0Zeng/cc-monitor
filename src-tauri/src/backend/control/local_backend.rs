...
mod tests {
    ...
    fn every_real_daemon_e2e_demands_a_private_tmux_dir() {
        ...
        //   🔴 **它剥不掉块注释的内层行** —— `/*` 与 `*/` 各独占一行时中间那几行原样留着。
        //   两个方向今天都实打过：**多行式是一次静默的假绿**（`D6` `M-D6-3`，全量门禁 `GATE: OK`）、
        ...
        for c in real_e2e {
            ...
            let line = code
                ...
                .unwrap_or_else(|| {
                    panic!(
                         ...
                         **多行块注释的内层行剥不掉** —— `/*` 与 `*/` 各独占一行时，\
                         中间那几行 `trim_start()` 之后仍是普通代码行，照旧算数。\
                         那一形是一次**静默的假绿**，逐条登记在 `local_daemon.rs` \
                         正题守卫头注「诚实边界 9」，归跟进件 `K-R8`。"
                    )
                });
            ...
            assert!(
                ...
                "`{name}` 里出现 `{GATE}` 的那一行（**剥掉「`//` / `*` / `/*` 打头的整行」之后**）\
                 ...
                 ③ 🔴 **块注释的内层行剥不掉，这一形是已知的、今天开着的**：\
                 `/*` 与 `*/` 各独占一行时，中间那几行 `trim_start()` 之后仍是普通代码行\
                 ⇒ 本条与下游 ㈡ 都会把它当真代码读。**两个方向今天都实打过**：\
                 内层行以 `let ` 打头 ⇒ **一次静默的假绿**（`D6` `M-D6-3`：一条起真 daemon 的 e2e，\
                 `PATH` 里一点 shim 都没有，全量门禁 `GATE: OK`、新红 0）；\
                 内层行不以 `let ` 打头 ⇒ 落到本条与 `MISS_NO_BINDING`，那时上面那一行\
                 「实得那一行」印出来的**就是一条注释** —— 那不是报文说假话，\
                 是这把尺子**本来就只剥整行**。\
                 ...
                 **「块注释这一族还能怎么走」的分母给不出** ⇒ 这里只登记实打过的这两形；\
