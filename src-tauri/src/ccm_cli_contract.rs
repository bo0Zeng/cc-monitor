...
mod tests {
    ...
    fn the_backend_unreachable_failure_face_has_exactly_one_home() {
        ...
        // ③b ★ **调用点数 == 登记为「没有退路」的后端路条数**〔`F` 拍 09-04 第二半加的〕。
        //    只判「非空」的话，两条无退路的路里**有一条悄悄不走这个失败面**（比如改回 `return 1`）
        //    它一个字都不说 —— 而那正是「唯一失败面」这个性质要守的东西。
        //    ⚠ 两个数**都现算**：一个数生产段里的调用行，一个数那张表里的 `NoFallback`。
        //    手抄任何一个就又多一份要同步的字面量（`brief` 13b）。
        let no_fallback_paths = BACKEND_BACKED_PATHS
            .iter()
            .filter(|(_, s)| matches!(s, FallbackShape::NoFallback { .. }))
            .count();
        assert!(
            callers.len() >= no_fallback_paths,
            "`backend_unreachable` 的调用点只有 {} 处，而登记为**没有退路**的后端路有 {} 条。\n\
             ⇒ 至少有一条登记为无退路的路**走不到**这个失败面（它自己另有一套说法，或者干脆静默）。\n\
             实得调用行：{callers:?}",
            callers.len(),
            no_fallback_paths
        );
        // ③c **总处数是个读数，带分母**〔`F` 拍 09-04 后半〕。
        //    ⚠ 它比 ③b 多管一件事：**「后端不可达」这句话不许在别处另起一套说法**。
        //    今天 3 处，逐个点名（分母 = `shared/ccm` 生产段里所有调用行）：
        //      · `acct_table_load` —— 账号解析（登记在表里，`NoFallback`）
        //      · `launch_via_daemon` —— 建会话（登记在表里，`NoFallback`）
        //    ⚠⚠ **`L5 身份`那段 tmux 内前置检查今天不在这个数里，而那是一格如实登记的欠账**：
        //      它报的是**同一件事**（这台机器上没有后端），却走 `die`（码 **2**），
        //      而这两条走 `backend_unreachable`（码 **4**）—— 同一个根因两个码。
        //      `F` 拍试过把它收进来，被**写区外**两条判据拦下（`polling_registry.rs` 逐字钉
        //      `die "找不到 daemon`，它的头注又钉着 e2e 那边的 `rc=2`）⇒ 提案交回 PM。
        //      ⇒ 这个数**今天是 2**；PM 裁完那条提案之后它该变 3，**同拍**改这里。
        assert_eq!(
            callers.len(),
            2,
            "`backend_unreachable` 的调用点从 2 处变成了 {} 处。\n\
             多了 = 又有一条路开始报「后端不可达」而没登记（它是不是也该进 `BACKEND_BACKED_PATHS`？）；\n\
             少了 = 有一条路不再走唯一失败面（多半是有人给它单独写了一句话 + 一个码）。\n\
             ⇒ 真加/删了这样的路就来改这个数，并在 `K-P2` 件文件里说清增量归谁。\n\
             实得调用行：{callers:?}",
            callers.len()
        );
    ...
    fn the_local_launch_recipe_is_reachable_only_from_print() {
        ...
        //   照样过 —— 那是**一个子串装了两件事**。
        // ⚠⚠ **而只排掉 `!=` 仍然不够**〔本轮自查第二处，`F` 拍后半逮到〕：
        //   `if [ "$do_print" = 1 ] || [ "${CCM_FORCE_LOCAL:-}" = 1 ]; then` 也含着那个子串、
        //   也不含 `!=` ⇒ **子串判放它过去**，而它正是件计划 `§6-3` 逐字排除过的那种买法
        //   （「一个环境变量就能走回本地 ⇒ 旧住址还在。**不许这么买**」）。
        //   ⇒ 改成**逐字钉整条守卫**。代价如实写：改守卫的**排版**（空格 / 写法）也会红，
        //   而那正是这把尺子要的 —— 这一行是「exec 路走不到本地那条」的**唯一**结构依据。
        assert_eq!(
            guard.trim(),
            "if [ \"$do_print\" = 1 ]; then",
            "本机 `new-session` 那段编排**不在 `--print` 那一支里**了，或者那一支的守卫被放宽了。\n\
             它上方最近的分支头是：{guard}\n\
    ...
    const BACKEND_BACKED_PATHS: &[(&str, FallbackShape)] = &[
        (
            // 〔`K-C1` 08-24 接线；`K-P2` `F` 拍 09-04 **退路删掉**〕账号解析。
            "--list-accounts",
            FallbackShape::NoFallback {
                // 与 `--launch` **同一个失败面**（同一个函数、同一句文案、同一个码）——
                // 那正是 `KP2C` 要的：两处各写一句就是两份措辞，而调用方按码判失败种类。
                face: "backend_unreachable \"$why\"",
                // 退路删掉之前的锚点：走了哪条路曾经是**可观测**的（`K-C1` 的 `_ccm_acct_src`）。
                // 只剩一条路之后这个变量没有存在的理由了 ⇒ 连它一起删。
                gone: "_ccm_acct_src=file",
            },
        ),
        (
            "--resolve",
    ...
    fn every_backend_backed_path_in_ccm_keeps_an_observable_fallback() {
        ...
        //   `--launch`），其中还留着退路的 **1** 条。
        //   ⚠ **两拍的读数**：09-04 之前 **3** 条（三条都有退路）；`F` 拍前半删了 `--launch`
        //   那条 ⇒ **2**；`F` 拍后半按 PM 裁定删了 `--list-accounts` 那条 ⇒ **1**。
        // ⚠ 剩下这 1 条**不是漏做**：`--resolve` 的降级是静默且**正确**的 ——
        //   `resume` 的 argv 只是**建议**，本地那条同样正确（拿不到就走本地 resume 串）。
        //   它是 ④ 那条棘轮登记的那笔欠账，有名有姓。
        assert!(
            has_fallback.len() <= 1,
