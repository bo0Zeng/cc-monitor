...
mod tests {
    ...
    fn the_backend_unreachable_failure_face_has_exactly_one_home() {
        ...
        //      · `L5 身份`那段 tmux 内前置检查 —— **它不发子命令、只查二进制在不在**
        //        ⇒ 它**不进** `BACKEND_BACKED_PATHS`（那张表的键是「发出去的子命令」），
        //        但它报的是同一件事，所以必须走同一个失败面。
        //        〔`F` 拍之前它是 `die`（码 2）—— 同一个根因给两个码，调用方就得维护两份判法。〕
        assert_eq!(
            callers.len(),
            3,
            "`backend_unreachable` 的调用点从 3 处变成了 {} 处。\n\
