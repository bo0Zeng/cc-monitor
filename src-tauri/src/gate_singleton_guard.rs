...
//! # 它查什么、查不了什么
//!
//! 查**判定形状**的字面量。实测（F03 落地时）：全仓 Rust 侧各只有 **1 处**，就在 `gate-core`。
//!
//! ⚠ **08-06 订正：原来只查两种形状**（`cc-` 前缀 与 `-cc-<N>` 撞名中缀），
//! 而 `is_ccm_tmux_name` 认的是**三种** —— 漏掉的恰好是 `<X>-cc`，
//! 也就是 S4b-3b 把命名反转之后**今天真正在用的那个主形态**。
//! 实测后果：一份只实现现行形态的第二判定
//! （`fn is_ours(n) { n.ends_with("-cc") && n.len() > 3 }`）四条判据全绿。
//! ⇒ 指纹是**判定的投影**，而这次投影恰好丢掉了活着的那一支、留下了旧的那两支。
//! **判定改过形状，指纹没跟** —— 与本区反复记的停滞式腐坏同族。
//!
