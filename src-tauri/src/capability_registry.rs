...
mod tests {
    ...
    /// 一次 commit 就落地**的文件照样够不着本条，只要它不叫那 6 个名字之一
    ...
    /// 包装层对 stdout/stderr 与退出码一个字节都不贡献 ⇒ 下面三形表里那四个数**必然**不变。
    /// **这是论证，不是读数 —— 别把它读成我打过了。**
    ///
    /// ⚠⚠ **一条本条自己盖不住的 —— 而「盖不住哪一形」必须说准**
    /// 〔`D4` 08-29 逮到：上一版逐字写「`src-tauri/` 那一格上**真生效**的 `runner`
    /// 会让本条**自己不被执行**」。那是一句**没有限定语的全称**，而它对下面的形 2 是假的；
    /// 更早那一版还带着限定语，本件删限定语时把它删宽了。〕
    /// **让本条「自己不被执行」的只有不透明的 `runner`**（不真跑测试的那一类，如 `/bin/echo`）——
    /// 08-29 现打形 1：包数 **0**、一条 `test result: ok` 都产不出。
    /// 🔴 **反例现打（3 分钟一刀，别再省它）**：**透明代理 `runner` 也是「真生效」的**，
    /// 而那一形**本条照样跑起来，也照样逮到它** —— 08-29 现打形 2：代理真起了 **6** 个二进制、
    /// `rc=101`、本条逐字 `… stays_registered ... FAILED`、报文点名 `["runner"]`。
    /// ⇒ **「真生效 ⇒ 本条不被执行」这个全称一刀就打穿了**，正确的说法在下面三形表里。
    /// ⚠⚠ **别把上面那句读成「本条盖住了透明那一侧」** —— 它只管「本条会不会被执行」这一格。
    /// 下面的**形 3**（透明代理 + 判据看不见那个 `runner`）里本条照样跑起来、照样看不见，
    /// **那一形今天没人兜得住** —— ⚠ **这也是一句全称，它的分母就是下面那张表量过的三形
    /// （1/2/3，另加一行基线）**：形 1 门禁兜住、形 2 本条自己兜住、**形 3 两边都没兜住**。
    /// ⚠ 形 3 那一行**我这一拍没重打**（住址：本文件 `§9.2` 与 `audits/K-G2-D4.md#§3`
    /// 各有一次独立读数）；而它今天的活体不在这 6 格里 —— 见本节末尾「形 3 今天要怎么才复现得出」。
    ///
    ...
    ///
    /// 正确的说法要**分三形**〔PM 08-28 裁定的表；`D3` 逐形实打、`D4` 08-29 独立复核。
    /// ⚠ **哪一行是哪一拍量的，说清楚**：**形 0 / 1 / 2** 是 08-29 `D4` 回修那一拍
    /// **我用 `/home/zbl/.cache/kg2r5/gatedoor.sh` 重打的**（逐字复刻 `scripts/gate.sh:105`
    /// 与 `run_gate_sum`，⚠ **开了 `set -uo pipefail`**，被测对象是本工作树、
    /// `CARGO_TARGET_DIR` 独立）；**形 3 那一行我这一拍没重打** ——
    /// 它是 08-29 上一拍（`kg2r4-gatedoor.sh`）与 `D4` 各打过一次的读数，
    /// 住址是本文件 `§9.2` 与 `audits/K-G2-D4.md#§3`〕：
    ///
    /// | 形 | 谁兜得住 · 靠哪一支 | 08-29 复打的四个数 |
    /// |---|---|---|
    /// | **0 · 基线**：6 格全缺席 | —— | `rc=0` · 包数 **8** · 合计 **1303** · 判定**绿** · 代理 marker 0 |
    /// | **1 · 不透明 `runner`**（不真跑测试，如 `/bin/echo`） | ✅ **门禁兜得住** —— 一条 `test result: ok` 都产不出 ⇒ 采集面自检当场红 | `rc=0` · 包数 **0** · 合计 **`0`** · 判定**红**（只跑到 0 个包，应当 8） |
    /// | **2 · 透明代理 `runner`**（起真二进制、原样透出） | ✅ **本条自己兜得住** —— 透明代理让本条**真的跑起来**，而它看见 `runner` 就红 | `rc=101` · 包数 5 · 合计 83 · 判定**红**；本条逐字 `… stays_registered ... FAILED`，报文点名 `["runner"]`；代理真起了 6 个二进制 |
    /// | **3 · 透明代理 + 判据看不见那个 `runner`** | 🔴 **没人兜得住** —— 而这正是「净变宽」那一族，三次被逮到的都是这一形 | `rc=0` · 包数 **8** · 合计 **1303** · 判定**绿** · 代理 marker **8 条** ⇒ 与形 0 **逐个相等** |
    ///
    ...
    /// 它今天的**活体不在这 6 格里**，在上面「不守什么」栏 `rust-toolchain.toml` 那一行。
    ///
