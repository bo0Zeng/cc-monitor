...
mod tests {
    ...
    ///
    /// 正确的说法要**分三形**〔PM 08-28 裁定的表；`D3` 逐形实打；下面是 08-29
    /// 回修那一拍**我自己逐形复打**的四个数。量具 `/home/zbl/.cache/kg2r4-gatedoor.sh`
    /// **逐字复刻** `scripts/gate.sh:105` 那条命令与 `run_gate_sum` 的算法，
    /// 被测对象是本工作树，`CARGO_TARGET_DIR` 独立〕：
    ///
    /// | 形 | 谁兜得住 · 靠哪一支 | 08-29 复打的四个数 |
    /// |---|---|---|
    /// | **0 · 基线**：6 格全缺席 | —— | `rc=0` · 包数 **8** · 合计 **1303** · 判定**绿** · 代理 marker 0 |
    /// | **1 · 不透明 `runner`**（不真跑测试，如 `/bin/echo`） | ✅ **门禁兜得住** —— 一条 `test result: ok` 都产不出 ⇒ 采集面自检当场红 | `rc=0` · 包数 **0** · 合计 `<找不到>` · 判定**红**（只跑到 0 个包，应当 8） |
    ...
    ///
    /// 🔴 **形 1 那一格必须点名是哪一支，因为它是独木桥**〔`D3` 加的，PM 的表里没有〕：
    /// 形 1 里 **`rc = 0`** —— cargo 自己**成功退出**（`/bin/echo` 的退出码是 0）⇒
    /// `run_gate_sum` 的**退出码那一支完全没起作用**，而「合计为 0」那一支排在它**后面**、
    /// 根本没轮到 ⇒ **唯一逮到形 1 的是 `elif [ "$pkgs" -ne "$want_pkgs" ]` 那一支**。
    /// 把它松成 `>=`、或把那个 8 调错，形 1 立刻变成第二个形 3。
    /// ⚠ 那一支住 `scripts/gate.sh`，**不在本条的射程内**，本条只如实记这一句 ——
