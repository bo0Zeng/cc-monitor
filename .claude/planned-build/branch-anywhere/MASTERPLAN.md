...
### §0.3ter TUI 的 branch 到底做了什么（顺带查清）

`project/asd` 那份：**单一 `sessionId`、5 个根、84 个分叉点**（分叉度 2 的 61 个、3 的 21 个、4 和 5 各 1 个）。
⇒ **TUI 的 branch 是「在同一个文件里长出一条新路」，不产新会话文件。**

这正是用户说的「得 ESC 回去，这样就破坏当前会话」的机制：旧路径变成 off-main，
当前会话的活跃指针挪走了。**我们这条路（复制成新文件 + 另起一个进程）与它本质不同**，
「两条都活着」只有我们做得到——这条值得写进产品说明。

### §0.4 一个**刻意不跟**官方 SDK 的地方：切片语义
...
7. **`logicalParentUuid` 是唯一没查清的一条**（§0.3bis-5）。真语料里有 4 处，是
   **compact 边界的回指**。开放问题：**在一个 compact 之后的节点上分叉时，
   前缀里会不会缺掉 compact 之前的上下文？** 祖先回溯只跟 `parentUuid`，不跟这条边。
   两份原生 fork 样本都不含它 ⇒ 无从对照。**不猜**：G0 只把现象与问题记进注释，
   真要答需要一份「在 compact 之后 fork」的原生样本。
