...
| **E75** | **`stripComments` 不认 Rust 的字符字面量 ⇒ 一个 `'\"'` 就让它此后不再剥注释** | G3b-1 写 `const SHELL_META: &[char] = &['\'', '\"', …]` 时被 C04a 守卫当场打红 | **未修（已绕开，但缺口还在）**。`test-support/strip-comments.ts` 的状态机跟踪 in-string，但**不认字符字面量** ⇒ `'\"'` 里的双引号被当成字符串起点，从此**整个文件后半段的注释都不再被剥**。<br><br>**实测后果**：`history.rs` 后面一段 doc 注释里有 `` `#[tauri::command]` `` 字样，于是 C04a 把它当成真属性，报出一个不存在的命令 `build_resume_ps_command`「声明了却没注册」。<br><br>**与该文件头注记的那次是同一类、方向相反**：那次是 `"…/*\/bin"` 里的 `/*` 被当成块注释开始，**吞掉 521 行真代码**（假阴性、危险）；这次是假阳性（fail-closed、会叫）。**两次都源于同一个根因：状态机不认某类字面量。**<br><br>**本轮绕法**：把 `&[char]` 改写成 `&str`（字符串里的 `\\\"` 是转义引号，状态机认得），并在那行上方写明为什么不能写成字符数组。**改措辞不改守卫**。<br><br>**真修法**：给状态机加字符字面量识别（`'x'` / `'\\''` / `'\\u{…}'`）。它被 6+ 个守卫共用，改它要连 `strip-comments.vitest.ts` 一起补例。|
