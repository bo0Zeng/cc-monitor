...
| **E37** | **`CLAUDE_CONFIG_DIR` 零官方文档，而整套多账号隔离压在它上面** | 2026-07-30 查官方 settings 页与环境变量页均无此项 | **已并入 `account-zero` Z07**（版本钉 + 漂移检测）。它失效的话多账号整体崩，而今天**没有任何东西会响**。最坏形态不是报错而是**静默共用身份**（以为在用 z、实际烧 b 的额度）⇒ Z07 的验收必须是「改成假位置 ⇒ `verify` 当场红」的变异 |
