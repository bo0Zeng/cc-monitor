...
| E23 ✅ **2026-08-02 U8c-2a 结案** | IR 之外的第 6 个手拼 builder | 同上（重要 2） | **已解决**：`buildUsageProbePayload` 已删除，用量探针载荷改由 Rust `launch_core::usage_probe_payload` 编译（`account_usage` 的 IPC 从「收渲染好的串」变成「收结构化账号表态」）。原文：`src/remote-launch.ts:70-76` 的 `buildUsageProbePayload` 手拼 `export CLAUDE_CONFIG_DIR=…; unset <nested>; claude`，即 `account` + `nested-env-reset` 两个维度的手工复刻，由 F10（晚于 F03）新增 |
...
| **E59** | **机器详情页上还留着 origin 选择器 —— 「进去 = 选定一个实例」这条地基判据没兑现** | Phase G 整体设计审计（2026-07-31） | **✅ 已闭合（2026-08-01，用户拍板「删掉」）**。<br><br>**订正条目原文的「4 处」**：真正该删的是 **3 处** —— `accounts-section`（`<select>`）· `mcp-section`（一整行机器按钮）· `cc-bus-hooks-section`（`<select>`）。第四处 `cc-bus-section` 的下拉**保留且必须保留**：它住在**顶层 cc-bus 驾驶舱视图**（`views/cc-bus-view.ts`），**不是** `panel.ts` 的 per-machine 块 —— 那儿没有页上下文，它的选择器就是上下文本身。删了那个视图就没法选机器了。<br><br>**为什么选删不选藏**（用户原话「删掉」）：这次重做的整条论证是「机器是中心对象、上下文由页面给」，留一个能绕过页面上下文的入口，等于把地基判据降级成约定。删之后 `origin` **只能**来自共用 store。<br><br>**顺带删掉一次没人用的 IPC**：`mcp-section` 的 `list_remote_mcp_origins` 原本只为渲染那排按钮，按钮没了它就没有消费者 —— 而**它是要连 SSH 的**。<br><br>**三块的降级形态各不相同，因为它们表示能力不同**：`accounts` 只对远端有意义（本机是 L3b 欠账）⇒ 收到 `null` 原地不动；`mcp` **本机与远端都有意义**且两种模式读写面不同 ⇒ 保留「机器：xxx」那一行做**只读显示**；`cc-bus-hooks` 远端诊断在没有页上下文时**按钮置灰**并写明原因，而不是默默挑一台。<br><br>门禁：`cc-bus-hooks-section.vitest.ts` 新增 E59 三条（DOM 里没有任何可选机器的控件 / 没有页上下文时按钮不可点 / 跟着 store 走且诊断目标真的变）；`machine-sync.vitest.ts` 那两条**改写**——原来用 hooks 的下拉既当驱动又当观察点，现在驱动只剩 cc-bus 驾驶舱与 store 本身（后者才是机器详情页切页的真实路径）。<br><br>原始记录：**要先定「藏还是删」**。主计划 §3 账本第 4 行的最终形态是「4 处 origin 选择器**由页上下文取代**」；S4a/S4b 只做到「四份共用一个 store」，**选择器一个都没删/没藏**。`accounts-section.ts:115` 恰在 `hosts.length > 1`（本计划的目标场景）时显示下拉；`mcp-section` 有一整行机器按钮。<br><br>**后果不止难看**：写动作按 store 定目标（`mcp-section.ts:699` `const startOrigin = this.origin`、`accounts-section.ts:182` `launch_remote_terminal({origin: this.origin})`）⇒ **在标着 A 的页面上把东西写进 B**，而 `router.activeId` 仍是 A、界面上看不出来。<br><br>做法建议：给四个分节加 `setPageMode(inPage)`（照 `MachineCard` 已有的那个契约），页内隐藏自己的选择器 —— 页头就是选择器。**横跨 4 个文件，且「隐藏 vs 删除」是个真决定**，故不在 Phase G 顺手做 |
| **E60** | **`makeInfoIcon` 仍无 `destroy()` —— 自己写死的硬前置被跳过了** | Phase G（两个视角独立命中）；原始登记在 `settings-ia/STATUS.md`「必须做但刻意延后的」第 1 条 | **✅ 已闭合（2026-08-01），但修法与标题写的不一样 —— 没加 `destroy()`。**<br><br>**补 `destroy()` 是错的修法**：那要 24 个调用点**每一个**都记得调，而它们今天全都只是 `append(makeInfoIcon(...))`。一个靠 24 处自觉维持的不变量迟早会破，破了照样没人知道 —— 与原问题同构，只是把责任转嫁了。<br><br>⇒ 改成**结构上不可能泄漏**：tooltip **只在显示期间存在**（显示时才 `appendChild`，隐藏时 `remove()`）。图标被销毁时 tooltip 本就不在 DOM 里，无需任何人做任何事。唯一兜不住的时序（**正显示着**时图标被销毁 ⇒ `mouseleave` 永不到来）由 `sweepOrphanTooltips()` 在下一次显示前扫掉，**残留上限恒为 1 条**。<br><br>门禁：新 `info-icon.vitest.ts` 8 条，含「造 24 个不悬停 → 零 tooltip」「重建 100 次 → 零残留」「悬停中被销毁 5 轮 → 不累积」。两条变异实测：回到「构造即 append」红 · `hide` 只 `display:none` 不摘 DOM 红。<br><br>原始记录：那条前置逐字写着「**必须先于任何页面化**（16 个调用点、无销毁路径，页面级构造/销毁会稳定泄漏）」，而 S4b-1/S4b-2 的页面化**已经做完了**，调用点也从 16 涨到 **24**。`info-icon.ts:47` 把 tooltip `appendChild(document.body)`，全文件无任何回收路径；`rebuildCards()` 每次重建全部 `MachineCard`，每开一次设置窗跑两遍。<br><br>**窗口关闭会兜住**，所以今天不是用户可感缺陷 —— 但判据是「页面化之前」，不是「泄漏够不够大」。**门是自己立的，越过去了且没有任何门禁会红**，这一点比泄漏本身值得记 |
| **E61** | **`SettingsRouter` 方向键走注册序、导航按 DOM 序显示 —— 机器页一出现就错位** | Phase G 整体设计审计 | **✅ 已闭合（2026-08-01）**：`onNavKeydown` 改走新的 `navOrderIds()`（读 DOM），不再走 `routeIds`（注册序）—— 键盘导航的语义本来就是「按你看见的顺序走」。<br>**按登记时的要求补了那个构型的测试**：父 + **多个**子项 + 子项**后注册**（生产里唯一真实的形状），并先钉住「视觉序 ≠ 注册序」这个前提，否则测的是一个不存在的差异。变异实测：改回 `routeIds` 即红。<br><br>原始记录：`addRoute` 用 `anchor.after(navButton)` 把子项插到父项之后（视觉序：应用/机器/aya/nano/改动足迹），而 `onNavKeydown` 遍历 `this.routeIds` = **Map 注册序**（应用/机器/改动足迹/aya/nano）—— 机器页是 `RemoteSection` 异步 `refresh()` 之后才注册的，永远排在 footprint 后面。⇒ 焦点在「机器」上按 ↓ 跳到「改动足迹」，`End` 落到最后一台机器而不是视觉最后一项。<br><br>`router.vitest.ts` 三条测试（扁平三页 / 只查 DOM 序 / 父+**一个**子项）互补但从不组合，恰好漏掉生产里唯一的真实构型（父 + **多个**子项 + 后注册）。修的时候要**同时**补那个构型的测试 |
| **E62** | **「有改动需重启」条活不过关窗口，而模块头注声称它只能靠真重启消掉** | Phase G 整体设计审计 | **✅ 已闭合（2026-08-01）**：`reasons` 落 `localStorage`，并随存一个**本次进程的 `bootId`** —— 读回来时 `bootId` 对不上就整条丢弃。于是「关窗再开」条还在（对：确实还没重启），「真重启」条消失（对：改动已生效）。<br>**为什么不用 `sessionStorage`**（看起来更贴切）：设置窗是**另一个** webview，`sessionStorage` 不跨窗口共享 —— 那正好把「关窗即忘」原样搬了个家。<br>**顺带补齐两处供货**：`panel.ts` 的「Claude 数据目录」与 `showBgSessions`（后者的字段注释逐字写着「重启生效」）。`showBgSessions` 只在**真的改了**才标，否则每次 toggle 都标会把噪音变回来。<br>两条变异实测：不落盘红 · 去掉 `bootId` 比对红（真重启后条不消）。<br><br>原始记录：`restart-notice.ts` 的 `reasons` 是**模块级内存变量**；windowMode 下 `close()` 是 `getCurrentWindow().close()`，下次 `open_settings_window` 是 `WebviewWindowBuilder::new` 建**全新窗口**。⇒ 改完远端配置 → 关设置窗（这正是改完之后的标准动作）→ 再打开 → **条没了，而 monitor 根本没重启**。<br><br>头注却写着「刻意不给『知道了』按钮…要让它消失只有一个办法：真的重启」—— 那句话现在是假的。同一批代码里 `machine-status` 已经论证过「跨重启保留是对的」并落了 localStorage，这里把一个**进程级**状态放进了**最短命的那个窗口**的内存里。<br><br>另：`panel.ts:404`（Claude 数据目录）与 `panel.ts:797`（showBgSessions）两处也需重启、也没给条子供货（诊断日志那处 Phase G 已补）|
| **E63** ✅ **= E80，已随 E80 一并闭合（2026-08-01）** | **`machine-card` ↔ `remote-section` 双向值导入 —— 全仓唯一一个真运行期循环依赖** | Phase G 代码工程审计（对 175 个前端模块跑 Tarjan，3 个 SCC 里只有这个两侧都是值导入） | **未修（低风险但该断）**。`machine-card.ts:25` 从 `remote-section` 取 `describeStage`，`remote-section.ts:52` 又 import `MachineCard`。S4b-3b-3 抽 `MachineCard` 时把 `describeStage`（`ConnectStage` → 文案的纯函数）落在了旧文件里 ⇒ 两个 1100 行的文件在模块层面又粘回一块。<br><br>今天靠「顶层只有声明、没有求值」侥幸不炸，且 `eslint.config.js` 里没有 `import/no-cycle`。修法很轻：`describeStage` + `ConnectStage` 转口一起挪到第三个小模块。顺带 `machine-card.ts:26` 从 `remote-section` 转口 `type ConnectStage`，而生成物就在 `../generated/ConnectStage` |
...
| **E75** | **`stripComments` 不认 Rust 的字符字面量 ⇒ 一个 `'\"'` 就让它此后不再剥注释** | G3b-1 写 `const SHELL_META: &[char] = &['\'', '\"', …]` 时被 C04a 守卫当场打红 | **✅ 已闭合（2026-08-01）**：状态机认 Rust 字符字面量了。**判据刻意窄**——右引号必须**紧跟**在一个字符或一个转义之后（`'x'` / `'\\n'` / `'\\''` / `'\\u{…}'` / `'\\x41'`），于是 `'static` / `'a>` / `&'_` 一概不匹配 ⇒ 本仓 114 处生命周期**零误伤**（这正是当初刻意不把 `'` 当定界符的理由，现在两者都成立）。<br>补 5 条测试（含「`'\"'` 之后的注释仍要被剥」「幻影字符串不许把后面的 `#[tauri::command]` 字样带成真属性」「生命周期不误伤」「TS 侧 `'…'` 仍是定界符」），两条变异实测：去掉识别红 · 放宽成定界符红。<br>`history.rs` 里当初那条绕法的告诫已改成历史注记（约束不再成立）。<br><br>原始记录：`test-support/strip-comments.ts` 的状态机跟踪 in-string，但**不认字符字面量** ⇒ `'\"'` 里的双引号被当成字符串起点，从此**整个文件后半段的注释都不再被剥**。<br><br>**实测后果**：`history.rs` 后面一段 doc 注释里有 `` `#[tauri::command]` `` 字样，于是 C04a 把它当成真属性，报出一个不存在的命令 `build_resume_ps_command`「声明了却没注册」。<br><br>**与该文件头注记的那次是同一类、方向相反**：那次是 `"…/*\/bin"` 里的 `/*` 被当成块注释开始，**吞掉 521 行真代码**（假阴性、危险）；这次是假阳性（fail-closed、会叫）。**两次都源于同一个根因：状态机不认某类字面量。**<br><br>**本轮绕法**：把 `&[char]` 改写成 `&str`（字符串里的 `\\\"` 是转义引号，状态机认得），并在那行上方写明为什么不能写成字符数组。**改措辞不改守卫**。<br><br>**真修法**：给状态机加字符字面量识别（`'x'` / `'\\''` / `'\\u{…}'`）。它被 6+ 个守卫共用，改它要连 `strip-comments.vitest.ts` 一起补例。|
| **E76** | **P5 之后 `@ccm_sid` 被写入不触发任何重探 ⇒ daemon 的 tmux 快照可能长期带着空 sid** | 修 E67③ 时读代码查出（2026-07-31） | **未修，先登记**。P5（`67653e2`）删掉 8s ticker 后，tmux 快照只在三个 hook 上刷新：`tmux_hook.rs:49 HOOK_EVENTS = [session-created, session-closed, session-renamed]`（→ SIGUSR1 → `watcher.rs:798 Poke`），另加 pidfd 与 socket inotify。**tmux 没有「设置了用户选项」这种 hook** ⇒ `shared/ccm` 通道 B 回填 `@ccm_sid`（`shared/ccm:654`）、以及 `/branch` 漂移改写它，**都不会让 daemon 重探**。<br><br>**时序上这不是边角**：`ccm` 建会话 → `session-created` 触发重探，而此刻 `@ccm_sid` **还没写**（通道 B 要先独立读会话文件确认，见 `shared/ccm:616-654` 的通道 A/B 设计）⇒ 快照里那一列是空的，且要等到该 server 上**下一次**会话生/死/改名才会被修正。<br><br>**影响面（未实测，按代码推断）**：daemon 帧驱动的灰灯判定（INVARIANTS §24bis）与按 sid 的会话定位。tabs.ts 右键菜单那条走的是**另一条**路（`list_remote_tmux` 独立 ssh 现查），不受影响。<br><br>**为什么不顺手修**：daemon 侧加轮询撞零定时器铁律（§41）；让 `ccm` 写完 `@ccm_sid` 后调 `--tmux-notify` 要它知道 daemon 的 pid+starttime（今天不知道），且要动 `shared/ccm`。**两条都是设计决定，不该夹在一次「修 CI 红」里做。**<br><br>**已知的一个观测点**：`e2e/resume-daemon-frames.sh` 之所以一直没暴露它，是夹具 `gen-idle-tmux.sh` 在 daemon 启动**之前**就设好了 `@ccm_sid`。<br><br>**2026-08-01 实测：最便宜的那个候选被排除了。**<br>想过让 `ccm` 写完 `@ccm_sid` 之后做一次**同名 rename**（`rename-session -t X X`），借 `session-renamed` 这条已有 hook 把 daemon 敲醒。隔离 socket 上实测（tmux 3.x）：<br>· 同名 rename → hook **触发 0 次**（tmux 短路了，名字没变就不发）<br>· 真改名 → 触发 1 次<br>· `set-option @ccm_sid` → 不触发（如预期）<br>⇒ **这条路走不通**，别再试。<br><br>**剩下三个候选，各有代价，需要一次设计决定**：<br>① `ccm` 从 `tmux show-hooks -g` 里**扒出** daemon 装的那条 `--tmux-notify <pid> <start>` 命令串并执行它 —— 不用知道 pid（现读），但要解析 hook 字符串，脆。<br>② **两步改名**（`X` → `X-tmp` → `X`）：会短暂改变会话名，而会话名是 A5 那条跨语言契约，**风险最大**。<br>③ **monitor 侧自愈**：收到 `session_added` 后若最近一份 tmux 快照里那个 sid 是空的，就发一次 `list_remote_tmux`（一次性、事件触发，**不是轮询**，也不碰 daemon 的零定时器铁律与 `shared/ccm`）。**我倾向 ③** —— 它把修正放在感觉到知识缺口的那一侧，且用的是既有命令。代价是那条路今天由 §24bis 的**单写者**把着，动它要先看清 emitter 的并发面。|
| **E77** | **daemon 加了子命令却没 bump `BUILD_ID`，靠记性不靠机检 —— 本仓第三次栽** | Phase G 审计当场抓出（2026-08-01） | **✅ 已闭合（2026-08-01）**：bump 到 `p1u-fork-session` + **机检落地** `remote-daemon-proto/src/build_id_guard.rs`（子命令集指纹 ↔ BUILD_ID 的**追加式**历史表；变异实测：加一个子命令不 bump 即红，错误信息直接列出「多了哪个」并写明为什么不能只改那张表）。<br>**两条挡不住的已在模块头注写明**：① 有人可以就地改最后一行而不 bump（表在同一个文件里，读不到 git 历史；真堵死要 CI 比 `git show HEAD~1`，需要 `fetch-depth: 0`，成本高于收益 ⇒ **如实登记为未做**）；② 「子命令没变但行为变了」它不管（p1r 那次就是）。它覆盖的是 p1t / G2 那一类 —— 而那恰好是栽过的两次里的两次。<br><br>原始记录：<br><br>G2 给 daemon 加了 `--fork-session`（main.rs 的 match 多一条臂），`BUILD_ID` 没动。后果是**整轮远端分叉能力在已部署的机器上休眠**：`ssh_source.rs` 判 stale 的判据是 `reported != EXPECTED_DAEMON_BUILD_ID`，两边同 id ⇒ 不判 stale ⇒ 不自动重装 ⇒ 用户点远端 `⑂` 永远只拿到「版本过旧，请重新部署」。<br><br>**加重情节**：`remote-daemon-proto/src/main.rs` 的版本谱系里，p1r 与 p1t **两段各自逐字写着这一课**（「P5 漏做、P7 补上的那一课」），`doc/INVARIANTS.md` §41.5 也写着。**写了三遍仍然漏第三次** ⇒ 这件事不该靠记性。<br><br>**机检怎么做（未做）**：`main.rs` 的子命令 match 臂集合 + `EMITS` 集合做成一个指纹，钉在一条测试里；指纹变了而 `BUILD_ID` 没变就红。难点是「什么算行为变化」没有廉价判据 —— 光看 match 臂会漏掉「臂没变但实现变了」（p1t 那次恰好是加臂，p1r 那次不是）。**折中方案**：先只钉「子命令集合变了必须 bump」这一半，它至少能挡住本轮这种。|
| **E78** | **「分叉完怎么起」在两个调用点各复制了一份（含逐字重复的 toast 文案）** | Phase G 架构视角审计（2026-08-01） | **✅ 已闭合（2026-08-01）**：查事实 + 起会话 + 成功反馈**全部收进 `runForkFlow`**，两个调用点只剩「我是谁 + 分叉结果」一句。加了结构性守卫（`fork-flow.vitest.ts` 的 E78 组：成功 toast 文案只许出现在 `fork-flow.ts`、调用点不许自己调 `collectForkSource`，另带反向自检）—— 行为测试证明不了「没有第二份」，只能按源码结构判。<br>**订正审计的一处措辞**：两个调用点取源 sid 的方式不同（`tab.sessionId` vs `sidFromJsonlPath`）**不是重复**，那是两种不同的输入（实时 tab 手里就有 sid，历史查看器只有路径），已在 `ForkFlowInput` 的字段注释里写明。<br><br>原始记录：`fork-flow.ts` 自称「唯一生产接线」，但真正共享的只有 `runForkFlow` 中段：`tabs.ts::startForkedSession` 与 `views/session-viewer.ts::startForkedSession` 各自写了「`collectForkSource` → `runForkFlow` → 成功 toast」约 15 行，**连 toast 文案都是逐字重复的双写点、无守卫**；而且取源 sid 的口径不同（tabs 用 `tab.sessionId`，viewer 用 `sidFromJsonlPath(jsonlPath)`）。<br><br>**修法**：把 collect 与成功 toast 一并收进 `runForkFlow`，两个调用点只剩「我是谁 + 分叉结果」。<br><br>**为什么不在本轮做**：本轮已经在这条路上改了三处阻塞（tmux 名净化 / 失败不再被吞 / 账号三态），再叠一次调用契约重构会让「哪一处改动导致了什么」不可分辨。**这是排期理由，不是技术理由。**|
| **E79** | **「这个会话怎么起的」只在起会话那一侧收成了一处，显示那一侧仍各读各的；本机则根本没有探针** | Phase G 主计划终账（2026-08-01） | **部分闭合（2026-08-01）：探针那一半做完了，显示侧收敛仍未做。**<br><br>**✅ 本机探针**：新命令 `list_local_session_accounts`（`local_accounts.rs`）—— 远端 `--session-accounts` 的对侧实现。扫 `<claude_dir>/sessions/*.json` → pid → `/proc/<pid>/environ` **只抠 `CLAUDE_CONFIG_DIR` 一个键**（边界照抄 daemon 侧那套：绝不回传整个环境快照；`configDir` 过 `is_safe_config_dir` 白名单后才回）。`fork-flow.ts::collectForkSource` 的本机分支接上了 —— 分叉一个**正跑着的**本机会话不再白弹一次追问小窗。<br><br>**平台**：**Linux 有，Windows 没有**，且在返回值里**说清楚**（`available:false` + 原因），不是静默空表。Windows 上读另一个进程的环境块要 `NtQueryInformationProcess` + 跨进程读内存（多半还要提权），代价与收益不成比例。上层本来就有「不知道就问一次」这条路。<br><br>**顺带**：`accounts.session-accounts` 这条平价欠账**还清**（对账表 122→123 命令 · 不对称 19→18 · 欠账 10→9）。**平台限制不等于欠账** —— 对账表问的是「这条能力我们做没做」，不是「它在每台机器上都答得出吗」，这条判断写进了那一行的注释。`SessionAccount` / `SessionAccountsResult` 加 ts-rs 导出，前端手抄那份换成生成物。<br><br>**❌ 仍未做：显示侧收敛。** tab 徽章走 `TabManager.sessionAccountsByS`，右键菜单走 `list_remote_tmux` 现查 —— 账本第 4 行要的「起会话与显示都读同一处」还差这一半。它要动 tab 徽章的取数路径（主平台主路径），**该单独排一次**，不适合夹在这一批里。<br><br>原始记录：<br><br>✅ 起会话侧已收拢：`inferForkLaunch`（推断）+ `deriveForkSource`（取数），且两个信号**不许互相顶替** —— tmux 清单答「活没活 / 在哪个 tmux」，pidfile 答「哪个账号」，tmux 清单里根本没有账号信息。<br>❌ 显示侧仍各读各的：tab 徽章走 `TabManager.sessionAccountsByS`，右键菜单走 `list_remote_tmux` 现查。<br><br>**另有一条实测约束**：本机侧**没有对侧探针** —— daemon 的 `--session-accounts` 是远端专属，`local_accounts.rs` 只枚举账号、不认会话。所以本机分叉一律按「查不出来」处理、问一次。**这不是偷懒，是没有信息源**；要收敛得先给本机加一个「某 sid 现在跑在哪个账号下」的查询。|
| **E80** | **`src/settings/` 里有一条真的运行期 import 环，而本仓没有任何门禁能发现环** | Phase G 代码工程视角审计（2026-08-01，独立写了 import 图 + DFS 复核） | **✅ 已闭合（2026-08-01）**：`describeStage` 与 `ConnectStage` 的再导出**搬进 `machine-card.ts`**（它唯一的消费者），`machine-card` 的 `ConnectStage` 改为直连生成物 ⇒ 环断。另建 **`src/import-cycle-guard.vitest.ts`**：自建 import 图 + DFS，**只算运行期边**（`import type` 擦除、不算；否则会逼人为一条不存在的依赖做重构 —— 本仓那条 type-only 的 `cards/index ⇄ cards/subagent` 就该放过）。带两条反向自检（图非空 / 判据喂人造环会红），变异实测：把环加回去即红。<br>**没装 `eslint-plugin-import`**：红线不装包，且它多出来的能力本仓用不上；模块头注写明「哪天真装了，这条守卫应当被它取代而不是并存 —— 两条判据并存必然漂」。<br><br>原始记录：`machine-card.ts:25` 值 import `describeStage` ← `remote-section.ts`；`remote-section.ts:49` 又 import `MachineCard` ← `machine-card.ts`。`machine-card.ts` 是从 `remote-section.ts` 抽出来的（其头注自陈），却回头 import 上层的**值** —— 正是「叶子反向依赖上层」。<br><br>**今天不炸**：两边的用点都在方法体里，模块求值期不触发，属 TDZ 型隐患。<br>**门禁侧**：`eslint.config.js` 只有 recommended 两套，**没有 `eslint-plugin-import` / `import/no-cycle`** ⇒ 环在本仓结构性不可见。审计实测全仓 114 个生产 .ts：值 import 环 1 条（就是这条），含 type-only 2 条（另一条 `cards/index.ts ↔ cards/subagent.ts` 是 type-only、运行期被擦掉）。<br><br>**顺带**：`machine-card.ts:26` 从 `remote-section` 绕道拿 `ConnectStage`，而它本来就是 `../generated/ConnectStage` 的生成物，直连即可断掉类型侧那条边。|
| **E81** | **覆盖率棘轮从没被真正棘过，裕度是宣称值的 5 倍** | Phase G 代码工程视角审计（2026-08-01，实跑 `vitest --coverage`） | **✅ 已闭合（2026-08-01）**：地板 40/34/36/41 → **52/43/46/53**（实测 S54.44 / B45.25 / F48.85 / L55.96 的下方 2-3%）。并把「棘紧记录」写成 `vitest.config.ts` 里的**追加式清单**（每次棘都写实测值 + 日期），让「过期没过期」一眼可见 —— 原来的写法是只记一次「当前值」，然后它就静静地过期了。<br><br>原始记录：`vitest.config.ts` 的注释写着「设在当前值下方 ~2-3%……当前 S48.98 / B41.15 / F44.60 / L50.41」，阈值 `40/34/36/41`。**实测 S54.39 / B45.20 / F48.80 / L55.92** ⇒ 裕度 11-15 个点，约 **2000 条语句可以变成无覆盖而门禁不响**；注释里记的「当前值」本身也已过期。<br><br>这是「基线名义下长期挂着」的典型形状，只不过挂的是覆盖率不是告警。**修法**：把阈值棘到实测值下方 2-3%，并在注释里写上棘紧的日期与实测值（而不是只写一次就不管）。|
| **E82** | **几处地板 / 基线数字低于实际，方向安全但都失去了灵敏度** | Phase G 代码工程视角审计（2026-08-01，逐条实跑） | **✅ 已闭合（2026-08-01）**：shellcheck 覆盖面地板 37 → **39**（就地实跑同一表达式核过，**不是照抄审计给的数** —— 我第一版按审计的描述写了 41，实跑是 39，当场改回；这正是「别把推断当结论」）· stylelint 注释 57 → **50** · `ci.yml` 那段散文里的套数与断言数订正（地板值本身一直对，有逐个 grep 的反向自检兜着，是散文没跟）· `e2e/README.md` 的地板表订正并写明「单一事实源是 `ci.yml` 的调用行，本表是给人读的副本，改地板两处都要动」。<br><br>原始记录：① shellcheck 覆盖面地板写 37，实测同一表达式是 **39**（e2e 已 21 个）⇒ 现在可以少扫 2 个文件而不报警。② stylelint 注释写「57 项基线」，实测 **50**。③ `ci.yml` e2e-tmux job 的散文注释写「8 套 / 共 152 条 / ccm-acceptance 15 / usage-probe 7」，而实际调用行是 19 / 9 —— **地板值本身是对的**（有逐个 grep 的反向自检兜着），只是散文没跟。④ `e2e/README.md` 的表同病。<br><br>**为什么值得修**：这些数字是「门禁失效时唯一会叫的东西」，写下之后没人回来棘紧，等于把灵敏度慢慢交出去。与 E81 是同一类病的不同实例。|
| **E83** | **`npx eslint .` 有 39 个告警永远不会被任何人看到** | Phase G 代码工程视角审计（2026-08-01） | **✅ 已闭合（2026-08-01）**：给 `e2e/**/*.mjs` 补 globals（node + mocha + browser —— `browser.execute(() => …)` 的函数体**在页面里跑**，那些 DOM 全局是真实存在的，不是漏声明）并把同一条 `_`-前缀「有意不用」约定挂上去（约定该跟着仓走、不跟着目录走）；`npm run lint` 从 `eslint src` 放开到 **`eslint .`**。补完**全仓实测仍是 7 项**，基线数字不变、覆盖面变大。<br><br>原始记录：`npm run lint` = `eslint src` ⇒ 7 个（与 ci.yml 注释一致，**诚实**）。但同一份 `eslint.config.js` 特意 ignore 了 `dist/node_modules/src-tauri/remote-daemon-proto/coverage`—— 那是**仓级意图**，而脚本只扫 `src`。`npx eslint .` 是 **46** 个，多出的 39 个全在 `e2e/restart-shims/*.mjs` 与 `e2e/tier2/*.mjs`，全是 `no-undef: process/console/it`（缺一段 node globals 配置）。<br><br>**修法**：给 e2e 那几个 .mjs 补 globals 配置，然后把脚本从 `eslint src` 改成 `eslint .`（基线随之从 7 变成 7）。|
| **E74** | **短命 claude 进程会让 tab 生灭：每条消息一对 SessionAdded/SessionRemoved** | aterm 实测 `claude -p` 的 pidfile 行为（2026-07-31，claude 2.1.220） | **未做，要排正式计划（动 tab 生命周期 = 主平台主路径）**。<br><br>**对方实测**：`claude -p … --output-format stream-json` **会**写 `sessions/<PID>.json`，进程活着时存在、**退出即清**（事后 diff 会假阴，得边跑边轮询才抓得到）。⇒ 若有人按「每条消息起一次 CLI」的架构跑，daemon 会为**每条消息**发一对 added/removed，monitor 侧闪出 tab 又消失、attach 菜单跟着闪。<br><br>**两个字段都过滤不掉**（对方核过，非推测）：`kind` 是 **`"interactive"`**（不是 print/oneshot）；`entrypoint` 是 **`"sdk-cli"`**，而**真实的 SDK 交互会话也是 `sdk-cli`** ⇒ 拿它当判据会误伤真会话。**顺带更正我方一处伪证据**：我曾以「本机 10 个 pidfile 全是 `entrypoint:"cli"`」佐证，那只是单机样本、不覆盖 SDK 启动路径 —— 普查的**边界**也要说出来，否则它是个伪装成证据的巧合。<br><br>**修法必须落 monitor 侧，不能落 daemon**：「最短存活门槛」本质是「等一会儿再决定」= 让线程自己醒来，**撞 `INVARIANTS §41` 零定时器铁律**（`no_timer_guard` 钉住，是 zero-poll-liveness P0-P7 整轮的收益锚点）。而且落 monitor 本来就更对：daemon 的职责是如实报告内核事件，「短命的别建 tab」是**呈现决策**，不该污染事实层。monitor 侧已有同类设施可复用（`ssh_source.rs` 的 sid 去抖 retire、`tabs.ts` 的 in-flight 防抖）。<br><br>**与 E73 刻意分开**：E73 是「attach 该不该给」，本条是「短命会话该不该建 tab」，两个不同的轴 —— 合并会重蹈 `kind` 把两件事塞进一个判据的覆辙。<br><br>**待查**：`name`（cwd 派生 + 递增后缀，每条消息一个新名）会不会进 tab 标题 —— **我的推测，未验**。<br><br>**2026-08-01：定了做法，但**没做**，理由在最后。**<br><br>**做法（monitor 侧，一条事件驱动的宽限期）**：`remote-added` 到达时**不立刻建 tab**，而是挂一个 grace（约 400ms）——期间若收到同 sid 的 `removed`，两条一起丢弃、tab 从不出现；没收到就照常建。`ssh_source.rs` 的 sid 去抖 retire 已经是同一形状，可复用其教训。<br><br>**为什么不做**：<br>① 它给**每一个新会话**的出现都加了 400ms 延迟，而那是主平台主路径上最高频的一件事 —— 为一个**今天没有人在跑**的架构（对方测过但没上）给所有人加延迟，收益方向是负的。<br>② 真要做，宽限期的值必须有实测依据（多快算「短命」），而那需要先有真在跑的那种负载。<br>③ 条目自己写着「动 tab 生命周期 = 主平台主路径，要排正式计划」——**这条判断仍然成立**。<br><br>⇒ **等它真的发生（或对方确定要上那套架构）再做**，届时按上面那个做法排一次 planned-build。|
| **E73** | **`↗` 拉前对「没有终端后端」的会话报错归因错误；`kind` 把两件事合成了一个轴** | aterm 要让 SDK bridge 会话出现在 cc-monitor 里，问显式写 `kind:"interactive"` 有没有副作用 | **✅ 两半都做完了（2026-08-01）**。<br><br>**① 归因**（`tabs.ts::explainBringFrontFailure`）：`↗` 失败之后按远端 tmux 实况分四档 —— 有交互终端只是没 marker（原文案在这一档才对）· tmux 里有它但前台不是 claude（**明说「装 ccm 不会好」**）· 压根不在 tmux 里 · 查不到清单（**如实说查不了，不乱归因**）。只在**失败路径**上补查一次，happy path 零开销；判据取自与 attach 同一个 `findClaudeTmuxMatches`，不另造。4 条测试 + 变异（去掉第③档即红）。<br><br>**② 字段**：新增 **`attachable`**（pidfile 布尔 → daemon `session_added` 帧 additive → monitor）。契约写进 `doc/IPC-PROTOCOL.md` **§9.3** 并**冻结**；daemon `BUILD_ID` → **`p1v-attachable`**。名与值按条目里记的对方承诺（「你定名定值、我无条件跟随、一次写对不二次迁移」）由本仓定。<br><br>**订正条目里那句「光加 pidfile 字段修不了」**：它的理由是「idle-tmux 误判是从 tmux 侧做的，两个数据源」——但 monitor **能把两边 join 起来**（idle-tmux 按 `@ccm_sid` 匹配 sid，`attachable` 也是按 sid 记的）。所以**不需要**在 tmux 侧加标记，`TMUX_LS_FMT` 那条红线也就不用碰。monitor 侧用它做两件事：拦掉 `↗`（**在发 IPC 之前**，因为这一档我们已经知道答案）+ **把这些 sid 从 idle-tmux 判定里排除**（否则 monitor 会把跑着脚本的会话当空壳，给出「杀死空 tmux」）。<br><br>**缺席 = true**，存量零迁移；**只认真正的布尔**（字符串 `"false"` 当没写 —— 宁可少一次门控，也不要把拼错的值读成「不可 attach」而把功能吞掉）。<br><br>原始记录：**未修**。核了 tab 上四个终端向动作（`tabs.ts:2244-2326`）：**attach / kill / preview 自门控正确** —— 先查远端 tmux 清单，`findClaudeTmux` 查不到就把菜单项摘掉。**唯独 `↗` 没兜住**：`bring_remote_terminal_to_front` 靠扫窗口标题里的 `ccm-rbind-<sid>`，没有终端后端的会话必然失败，而文案是「未绑定窗口（**远端会话需在远端启用 ccm wrapper**）」—— **把用户引向一个不存在的问题**（装 ccm 也不会好）。<br><br>**结构上的病**：`kind` 今天把两件事合成一个轴 —— ①「该不该在 UI 出现」（`interactive`/缺席=出现，其余=隐藏）② 「是不是终端后端撑着的」（决定 `↗`/attach 该不该给）。SDK bridge 正好是「①要②不要」，现有字段表达不了。attach 那三个是**碰巧**靠自查 tmux 兜住的，不是设计。<br><br>修法：给 `↗` 接上同一套「有没有终端后端」的探测（与 attach 复用 `findClaudeTmux`），或在 pidfile 里加一个正交布尔（要跟 aterm 约新字段）。**至少先把文案按分档改掉**——错误归因比失败本身更贵。<br><br>**2026-07-31 补：前提被 aterm 纠正，结论不变但依据换了，且多出一个更急的坑。** 我原以为 bridge「没有 tmux」所以 attach 自门控住了 —— 实际它**有 tmux**，真正的门控是 `findClaudeTmux` 那个**两条件与**：`s.sid === sid` **且** `isClaudeTmuxCommand(s.command)`（比 `pane_current_command`）。bridge 的前台是 `python3` ⇒ 不匹配。**所以 attach 不出现是「前台命令不是 claude」这个巧合撑住的，不是设计** —— 对方那句「能用不等于有契约」正中这里。<br><br>**更急的坑**：`findIdleTmux`（`tabs.ts:304`）的判据是「`@ccm_sid` 精确命中 + 前台命令**不是** claude」= idle-tmux ⇒ **bridge 的形态精确落在里面**。只要它给 tmux 设了 `@ccm_sid`，monitor 就会提供「**杀死会话（kill 空 tmux）**」和 attach/就地 resume—— 它以为那是空壳，实际里面跑着 bridge。已让对方**暂时别设 `@ccm_sid`**。<br><br>**字段设计因此变形**：判据不是「有没有终端后端」（它有 tmux，这个问法答不出），而是**「attach 进去对人有没有意义」**（对方的决定性事实：`stdin=DEVNULL`，用户敲的字会被脚本吃掉）。倾向叫 `attachable`（缺席 = true，兼容存量）。**但有结构问题**：它写在 **pidfile**，而 idle-tmux 误判是从 **tmux 侧**做的（`@ccm_sid` + 前台命令），两个数据源 ⇒ 光加 pidfile 字段修不了。要么对方不设 `@ccm_sid`，要么 tmux 侧也加标记（**`TMUX_LS_FMT` 是红线不能加列**，得另走一次 `show-options`）。对方已承诺「你定名定值、我无条件跟随、一次写对不二次迁移」。|
| **E72** | **pidfile 的 `kind` 是「非 `interactive` 即隐藏」，这条隐含契约只活在 Rust 注释里** | aterm 要自己写 pidfile 时问「`kind` 是枚举还是透传」，**我第一次答反了**（说写 `bridge` 会当交互会话，实际是被排除），差点让对方按错的理解落地 | **✅ 已闭合（2026-08-01）**：`doc/IPC-PROTOCOL.md` 新增 **§9.1**（`kind` 的排他矩阵：缺席=放行 · `interactive`=放行 · `bg`=看配置 · **其它任意值=隐藏**，外加「改 kind 会走退休路径 = 让会话消失」这条副作用）与 **§9.2**（`procStart` 参与 PID 复用检测，不只是展示）。<br>条目里把「**我自己第一次也答反了**」原样写进文档，因为那正是这条契约反直觉的证据。<br><br>原始记录：`watcher.rs:1173` 的真实规则：`if kind != "interactive" && !with_bg { 排除 }`，配套注释写着「kind 字段**存在且非 interactive** 才排除；旧 CC 不写该字段 → 放行」。⇒ 矩阵是「缺席=放行 · `interactive`=放行 · **其它任意值=隐藏**」，还带一条副作用：同 key 从 interactive 翻成别的会走 `retire_sid_if_unreferenced(Gone)` 退休路径。<br><br>**为什么值得记**：这是个**排他式**契约（不在白名单就隐藏），而外部集成方最自然的直觉是「加个新 kind = 加个新类型」。它今天只写在 daemon 的 Rust 注释里，`doc/IPC-PROTOCOL.md` **一个字都没有** —— 与 E71 同一类病（隐含契约跨不出仓）。⇒ 建议：把 pidfile 的字段语义（尤其 `kind` 的排他规则 + `procStart` 参与 PID 复用检测）写进 IPC-PROTOCOL，那是跨项目消费方唯一会读的地方。|
| **E71** | **`--resolve` 的 `sessionName` / `capabilities` 看着像探测结果，其实是派生值** | aterm 接 DaemonTransport 时问「会不会退化成纯字符串拼接」，复核发现**对所有会话都已经是** | **✅ 已闭合（2026-08-01）**：走候选修法③ + 部分② —— `doc/IPC-PROTOCOL.md` 新增 **§10.1**，一张表把三个字段按可信度分开（`command` 可信 · `sessionName` 派生 · `capabilities` 典型档），并写明「要判断 tmux 会话是否真存在，用 `tmux_sessions` 帧」。`CommandPlan` 的结构体头注与**逐字段**注释同步。<br>**没做字段改名**：那是 wire 契约，aterm 已在消费，改名的收益（少一次误读）不抵一次跨仓 breaking；写清楚 + 写在**外部消费方真会读的地方**已经解决了「措辞误导」这个根因。<br><br>原始记录：`resolve_query.rs:90` 的 `run(_claude_dir, …)` 参数带下划线、`:36` 的 `claude_dir` 标着 `#[allow(dead_code)] // MVP 未用（不做 pidfile 消解）`；`sessionName` 来自 `session_name_for(sid, is_codex)` —— **纯从 sid 派生**（`cc-`/`cx-` 前缀），**没读过任何 pidfile**；`capabilities` 是 tmux/pty 的**典型档**，文件头自陈「待后续 backend 探测细化」。<br><br>**风险不在实现而在措辞**：CommandPlan 这个名字 + `capabilities` 这个字段名，会让消费方（已经有一个了）把它们当**探测出来的事实**用 —— 比如据 `sessionName` 去 attach 一个**并不存在**的 tmux 会话。今天唯一真正可信的是 `command`（= 调用方给的候选 + `--resume <调用方给的 sid>`）。<br><br>候选修法：① 字段改名/加 `Derived` 前缀 ② 加一个 `authority:"derived"|"probed"` 标 ③ 至少把这条写进 `doc/IPC-PROTOCOL.md`（目前只在 Rust 文件头，跨项目消费方读不到）。|
...
| **E67** | **main 上的 CI 已经红了很久（v3.4.0 与 v3.5.0 连红）** | v3.5.0 发版时逐条看 CI 才发现 | **✅ 六条全修完（2026-07-31 ~ 08-01；本行原写「五条」而正文有六条 —— 摘要落后正文，正是本轮 Phase G 点名的那类病）**。前端两条 `4891f84`：`host-os.vitest` 把 jsdom 的 UA 当成恒为 linux（Windows 上是 `win32` ⇒ unknown）· `paste-block-guard` 的 `walk()` 产出 `src\main.ts` 而白名单是 `src/main.ts` ⇒ **那条守卫在主平台上从来没绿过**。<br><br>**① Rust（Windows panic）**：`config_surface.rs:1490` 的 `s[0].path.ends_with("/.claude/settings.json")` —— `PathBuf::from("/h").join(".claude")` 在 Windows 上产出 `/h\.claude`（`join` 用平台分隔符），斜杠后缀恒不匹配。**订正我先前的怀疑**：本条与同文件 `:368` 那个 `cfg!(target_os = "windows")` 分支**无关**（那段只是 `WindowsProfile` 的措辞分支）——当初那句「怀疑同源」是猜的，实测证伪。改成 `Path::ends_with(Path::new(".claude").join("settings.json"))`：按**分量**比，跨平台成立且比字符串后缀更严。**本机无 Windows，不能实跑**；判据是 `Path::ends_with` 的分量语义 + Windows 上 `\` 与 `/` 都被认作分隔符，最终以 CI 为准。<br><br>**② vendored cc-acct-iso（294 断言里 184 条连锁失败）**：**不是 CI 沙箱缺 fixture**（我原来的猜测），**本机 100% 复现**。根因是 vendor 进来时**丢了可执行位**（三个脚本在 git 里全是 100644），而自测脚本用 `"$CLI" init d` **直接执行**副本 ⇒ rc=126「权限不够」。本地看不出来（`core.fileMode=false`）、远端也看不出来（`acct_iso_deploy.rs` 上传时显式给 0o755）—— 只有「干净 checkout 直接执行」这一条路会炸，也就是 CI。**恢复可执行位不违反 VENDOR.md 的镜子铁律**：上游实测就是 755（`lib.sh` 是 644，它被 source，所以**没动**），`.vendor_id` 是内容 sha256、不受模式影响。顺手把 `e2e/exec-bit-guard.sh` 扩出**第三个作用域**（vendored 白名单 + 独立计数自检）——它此前只看 `shared/**`，正是这个洞放它过去的。<br><br>**③ 两个 e2e real-machine job**：都是 **F-E2**。`resume-suite.sh` B6b 的期望值没跟上 **S4b-3b**（命名从 `cc-<X>` 反转成 `<X>-cc`，前端与 Rust 都同步了、只有 e2e 没有）；`resume-daemon-frames.sh` 等的是「kill claude 之后**新**发一帧 tmux_sessions」，而 **P5 删掉 ticker 之后那一帧永远不会来**（详见 **E76**），且那行是**裸赋值**、`set -e` 让脚本在超时那刻直接退出，连一行 FAIL 都不打。<br><br>**这条的价值不在这几个测试**，在于「Release 绿 + CI 红」这个组合已经持续两个版本没人处理 —— 红着的门禁等于没有门禁。**④（前三条修完才露出来的第四条）**：`resume-daemon-frames.sh` 第 3 步跑 `npx tsx e2e/resume-cmd-driver.ts`，而那个 driver `import "../src/accounts.ts"`（driver:25）、`accounts.ts` 顶层 import `@tauri-apps/api/core` ⇒ 那个 job 没装 node_modules，`ERR_MODULE_NOT_FOUND`。**这是既有问题、不是本轮引入**（`0b06859` 的 driver 就是这么写的），只是被②的早退挡在后面 —— 一个 bug 把另一个 bug 藏住了。修法：给该 job 加 `setup-node` + `npm ci`，并订正 ci.yml 里那句「这两套只从 Rust 侧取命令串、不跑前端，无需 node」。<br><br>**实测（CI run 30676711620，commit `5ad6f98`）**：7 个 job 里 6 个绿，含 **Rust lint + test 在 Windows 上转绿**（①的判据得到真机确认）、**E2E scripts health 转绿**（②）、**E2E real-machine (tmux, no Rust) 转绿**（③的一半）。剩下的就是这条 ④。<br><br>**剩一项**：branch protection 有没有把 CI 设成必需，未查。|
