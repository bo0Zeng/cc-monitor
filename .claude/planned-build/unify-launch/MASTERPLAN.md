...
| `src/launch-plan.ts`+`src/launch-dimensions.ts`（F03 新增） | F03,F04,F05,F06,F07 | `LaunchPlan`/`LaunchContext` 两型 IR + 维度注册表（`identity`/`env-reset`/`account`/`model`/`nested-env-reset`，`order` 定序 + 加载期断言）；**新增维度的唯一落点** | **F07 已落地架构验收**：新增第 5 个维度 `MODEL_DIMENSION`（order 25，卡在 `account`(20) 与 `nested-env-reset`(30) 之间）——`buildLaunchPlan`/`renderCli`/`canRenderCli`/`renderFallback` 的既有分支结构 diff 为零，唯一改动是 `renderEnvOps` 的 switch 加一个成比例的 `"export-model"` 分支，兑现 MASTERPLAN §0.1 成功标准②的承诺；5 个维度、4 条顺序不变量单测（含故意错序验证真 throw） | `MODEL_DIMENSION.applies` 是**条件式**（`!!ctx.modelOverride`），不是像 `ACCOUNT_DIMENSION` 那样恒真——判断依据见 `doc/INVARIANTS.md` §37（"沉默是否等价于用户期望"，不是"是不是账号相关"）；`cliFlags` 恒 `null`（`ccm` 无 `--model`，诚实降级，见 R14） |
...
| R14（**已接受，非阻塞**） | **F07 每账号模型偏好带来两个隐藏机制切换，均无应用内提示**（F07 Phase D 双 agent 审计 UX agent 发现）：① `MODEL_DIMENSION.cliFlags` 恒 `null`（`ccm` 无 `--model` flag，诚实降级）——任何配了模型偏好的账号，其**此后每一次**会话拉起都从快速的 `ccm` CLI 渲染路径永久降级到兜底 shell 渲染器，与 R13 的"隐藏机制切换"外观相似（都是"配了某设置 = CLI 路径被强制绕开"），但 F07 初版处置力度未达到 R13 的先例（R13 当时既登记了风险表、又有 `forceLegacyLaunchRenderer` 逃生口；F07 初版只在计划文档 §2 自我辩护，未登记进本表、应用内 tooltip 也未提及）；② `shared/ccm` 完全不认识这个偏好——用户在设置里配好模型、点 app 里的按钮起会话会遵循它，手动在终端敲等价的 `ccm --tmux --account z` 则完全不生效，字面违反 MASTERPLAN §0 corollary②"单一渲染目标"（app 行为 = 终端行为），且影响面比 R13 更宽（R13 只影响"自己手动维护 `CLAUDE_CONFIG_DIR`"的边缘配置用户；这里影响任何配过模型偏好又在终端手敲过 `ccm` 的用户——对一个允许直接登终端敲命令的工具，这不算边缘场景） | **判定①可接受**：教 `ccm` 认识 `--model` 是 F08（终端集成打磨）的既定分工，不属于"架构验收"这个目标该做的事；已在应用内输入框 tooltip 补一句"仅对本 app 发起的会话生效，终端里手敲 ccm 暂不识别（见 F08）"，把这道边界显式告知用户，缓解审计指出的"无提示"缺口。**②留给 F08 关闭**：F08 落地 `ccm --model` 后，`MODEL_DIMENSION.cliFlags` 可以从恒 `null` 改成真的吐 `--model <名>`，同时消灭这条风险和 R14①——F08 计划应显式对照本条验收 |
