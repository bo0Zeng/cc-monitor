...
20. **2026-07-28 — B01 完成（cc-bus 原样搬进仓）+ 独立审计闭环 + B02 范围订正。**
    B01 把 cc-bus 逐字节搬进 `shared/cc-bus/`（17 文件 1107 行；13 脚本 951 行）。
    独立对抗性审计**无阻塞项**，且它的复核比我的自证更强：不比工作树，而是把**提交对象里的
    blob** 逐一 `git cat-file blob | cmp` 盘上文件，17/17 全同——排除了"工作树对但 index/tree 错"
    这类 `diff -r` 看不见的失败；`diff -r` 的 5 类盲区（权限/隐藏文件/空目录/symlink/行尾编码）
    逐项查过全干净。**但全部发现集中在我为它新写的 37 行 exec-bit 守卫上**：两处静默恒绿
    （不校验 `git ls-files` 的 rc、不校验受检文件数 → `shared/` 改名或 git 失败就恒绿并打印
    "全部 100755"）、非 ASCII 文件名因 `core.quotePath` 被静默跳过（本仓工作目录就叫「文档/」）、
    symlink 触发不可满足的 FAIL、判据取工作树而断言取 index。全部已修并**变异验收**（4 条变异
    由绿转红）。修复中我自查还发现审计没提的一处：`checked>0` 只统计总数不够，白名单条目恒在
    会把"shared/ 整个消失"盖成绿（实测 rc=0）→ 改为**按作用域分开计数**；以及 `head -c 2` 会给
    `git cat-file` SIGPIPE，叠加 pipefail 就静默跳过 → 判 shebang 改为**内容比较**而非管道 rc。
    另修 `shared/ccm:376-379`：那段注释被这次搬家**改成了假话**（"分属不同仓库/没有测试能跨仓
    检测漂移"），而 §2.4 正引用它当 B02 的收益论据。CI shellcheck 扩到 `shared/cc-bus/scripts/*`
    （此前 1107 行新代码零 lint 覆盖）。加了一条**漂移软警告**（照抄 `build.rs::check_vendor_freshness`
    的形状：上游缺席 no-op、有差异只警告不改退出码）——它保护的不是文件，是 **B02 那张按行号
    写死的对拍表**：盘上那份是活的（12 条软链 + 本机正跑着实例），若手改而仓内不知，B02 就是在
    对一个**已非运行态**的基线论证"删这 46 行不是回归"。
    **B02 范围订正**：原计划称"B02 给 ccm 加新维度，正好验收成功标准②"——**错的**。B02 是
    cc-spawn(bash)→ccm(bash)，**全程不经 TS**（`grep CC_BUS_ID src/ --include=*.ts` 零命中），
    而 `LaunchDimension` 是 TS 侧的东西；在那里加维度 = 为假想消费者建抽象（R12/R15 同型错误），
    且会让验收变成**自己出的空题**（B02 本就没有 TS 调用点要改，"零改"必然成立）。
    → B02 只改 shell（照 `--model` 的 7 个触点加 `--bus-id`）；**TS 维度与成功标准② 的第二次
    验收挪到 B03 批二**（那时有真实 UI 调用点）。
    **流程自我批评**（审计提出，认同）：B01 的 commit 自称"只搬不改"，却混入了 CI 步骤 + npm
    script + 37 行新守卫，而风险恰好全在那部分——稀释了"搬家部分 `--numstat` 全是 `N 0`"这条
    本可一条命令确认的性质。**"搬家时顺手发现的缺陷修复"应单独 commit**，本次审计修复即照此办理。

21. **2026-07-28 — B02 完成（`cc-spawn` 收编进 `ccm`），账本「起会话被写死成 N 套实现」那一行闭合。**
    动手前撞上**计划≠现实**：`ccm --tmux` 恒 attach（`:496` 无条件接 `; tmux attach`，`--print`
    确认），而 cc-spawn 要的是「建 detached 后立刻返回」去登记总线/写台账/打印提示。停下交回
    用户，用户拍板「**ccm 为最高统一标准**，预留参数方便后面接新功能」→ 给 ccm 加 `--detach`。
    **这是拆开焊点而非新增特性**：`--tmux` 此前同时干「起容器」与「attach 进去」两件正交的事
    ——正是 §0 那个病本身。同时补 `--tmux-size`，让 cc-spawn 的 `-x 220 -y 50` 不被默默丢掉。
    两者都不静默忽略（不配 `--tmux` 直接 die），默认行为不变故既有调用方零感知。
    **`--bus-id` 加了又删**：实测 `agent_needs_bus_id` 已在 codex 下从会话名自动派生
    `CC_BUS_ID`，与 cc-spawn 手动注入的值逐字相同 → 再加 flag 就是为假想消费者建抽象。
    成功标准② 的第二次验收仍在 B03 批二（那时 TS 才有真实 UI 调用点）。
    cc-spawn 136→95 行；新增 `e2e/cc-spawn-uplift.sh`（12 项，3 条变异由绿转红）。
    **一次真实事故写进纪律**：我用 `TMUX_TMPDIR` 而非用户定的 `-L` 做隔离，隔离未生效，
    `tmux kill-server` 打在默认 socket 上，杀掉了开发机上正在跑的 `cc-claudecode-frontend`
    会话。→ e2e 固化两条硬约束：PATH 上的 `tmux` shim 强制 `-L`；**起飞前自检**断言 shim
    看不到默认 socket 的任何会话，不过就 exit 9。**"设了隔离"不算数，必须证明它生效。**
