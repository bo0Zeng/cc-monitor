...
- 2026-07-23 **F-fork-badge 完成**(feature 4,#63①):活 tab 从首条 `forkedFrom` 记录锁定血缘 → 标题加 `↳` 徽标 + tooltip(镜像 aiTitle)。Phase D 2 视角均无阻塞/无重要(字段路径对照 messages.rs/history.rs 核实、单 ↳ 合成、非 fork 字节不变);采纳建议把 applyForkedFrom 前移到 turnEndNotifier 之前 + 补 3 pin 测(单↳/远端序/tooltip,共 6 条)。**未改 styles.css**(账本对 #63① 的「大概率改」预测未命中,纯文本前缀无需 CSS)。carry-forward:远端 fork 徽标依赖 daemon 实时行透传 forkedFrom(真机验)。gate:tsc / 全套 npm test(vitest 345)。
- 2026-07-23 **F-render 完成**(feature 2,#71+#42):#71 覆盖 marked `del` tokenizer 只认 `~~`(单 `~` 返回 undefined);#42 块 `$$` 配对改「行边界规则」(开=行首|后接换行,闭=行尾|前接换行)+ CRLF 归一 + 去 `[^$]`。Phase D 2 视角:#71 SOLID;**#42 首版(line-anchored)有 3 回归(CRLF/开前有字/闭后标点)+ 新测不区分修没修** → 重写 #42 正则(离线 harness 实证 8 例全过)+ 测试改区分性(preprocess 层断言,因 DOMPurify 剥 annotation)+ #71 注释更正 + 强标 return-undefined footgun。gate:tsc / render 22 测 / 全套 333 测 / ReDoS 安全。
...
- 2026-07-23 **F-history-persist 完成**(feature 5,#46):远端来源快照 localStorage 持久化 + 构造 hydrate(loadedAt=0=首帧暖绘、首开必刷)。Phase D 视角2 报 1 重要(hydrate 不校验元素→脏 localStorage 混 null/基元→renderList deref 抛→历史打不开)→ 修:抽 loadPersistedRemoteCache 逐元素过滤(对齐惯例)+ 补 2 测。gate:vitest 350。
- 2026-07-23 **Phase G 收官**:类1 五 feature(#41 #72 #71 #42 #67 #63① #46)全完成签收。端到端 build 绿(前端 tsc+vite / monitor cargo build)、全套 350 测、无遗留、CHANGELOG [未发布] 记 7 项修复、账本落最终形态。重量级 /full-audit 全项目扫按强度裁剪留用户可选(各 feature 已 2 视角 D+E)。**loop 停**;发版/push/关 issue/真机验-close 交用户。
