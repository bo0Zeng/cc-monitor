...
> **状态列于 2026-07-29 Phase G 全面订正。** 此前 T01-T09 与 G01-G03 **全部标「待做」**，
> 而其中 5 个 T 与 3 个 G 早已交付——文档视角审阅把这条点为「MASTERPLAN 已经不是它自称的
> 单一事实来源」（重要 I1）。订正记录此前只写在 `P-READINESS.md` 里，从未回写到这张表。

| ID | 功能 | 一句话目标 | 状态 | 依赖 | 优先级 |
|----|------|-----------|------|------|--------|
| G01 | 信号修复：fmt + CI + e2e 进 CI | `cargo fmt` 28 处、分支从未跑过 CI、7 套真机 e2e 在 CI 外 | **完成**（R00；断言条数标签仍不准 → BACKLOG E12） | — | P0 |
| G02 | 伪测试扫荡 | 已确认 3 条；对每个功能的核心防线断言做变异检查 | **完成**（R02 + 之后每个功能都做变异验证；Phase G 又抓出 4 条 panel 文本扫描测试并换成真行为测试） | G01 | P0 |
| G03 | unify-launch 收尾 | F10 收口 commit；INVENTORY 重写（成功标准1 首次可验收）；账本 2 行未达成项定性 | **完成**（R06；Phase G 实测 INVENTORY 的 34 个符号锚点全部命中） | — | P0 |
| T01 | 受管工具注册表内核 | `ToolSpec` 声明 + 落点/主机申报（**装升卸没有走注册表** —— T04 审计结论：统一部署器不该建） | **完成** | G01 | P1 |
| T02 | 配置面审计视图 | 「动过你哪些文件」+ 撤销动作/指引 | **完成**（含 `HostScope`/`EitherHost`；本页不连 SSH，故远端行只能给「未确定」——见 BACKLOG 讨论） | T01 | P1 |
| T03 | 「生成待贴文本」统一组件 | `paste-block.ts`，3 个生产消费者 | **完成** | T01 | P1 |
| T04 | 五套既有机制收编 | 两步：`verified_write` + `fenced_block::find_pair`。**统一部署器经实测论证为不该建**（五件事形状不同） | **完成** | T01 | P1 |
| T05 | cc-bus 搬进本仓 | 1118 行 bash 成为仓内源；部署到 skills + symlink；钩子走「读+诊断+生成」 | **已移出本工作区**（搬家由 B01 做完、钩子诊断由 B04 做完；**只剩「部署到 skills + symlink」未做**，见 `P-READINESS.md`） | T01,T03 | P2 |
| T06 | code-picture 搬进本仓 | vendor 从「上游镜子」升格为**上游本体**（含 `tests/`）；旧 sibling 仓退役 | **已搁置**（用户 2026-07-29「code picture 先不做」）。§0 实测有时效，重启先重跑 | T01 | P2 |
| T07 | 设置面板信息架构重构 | 实测四个耦合面后**不拆 `panel.ts`**；真病是「任一 section 构造期抛 = 整页白屏」，治法是分区块隔离 | **完成** | T02,T04 | P2 |
| T08 | skill 管理泛化 | cc-acct-iso / cc-bus 已是 skill；泛化成装/卸/看版本 | 未做（P3，未开工） | T04,T05 | P3 |
| T09 | 多 agent 驾驶舱 | cc-monitor 看见/管理 bus 上的 agent、派活、读 inbox、`cc-spawn` 图形化 | **已移出本工作区**：由 unify-launch 的 **B03** 交付 | T05 | P2 |
...
| # | 日期 | 改了什么 | 为什么 |
|---|---|---|---|
| P-1 | 2026-07-29 | T01 的 `ProbeStatus` **删掉** | 与既有 `ccm_probe::CcmProbeResult` 同形、零适配、零消费者。**发明第二个同形结构不是统一，是重复**；按同轮删 `WriteVerdict::is_ok`、把 `build_online_cmd` 零调用点判阻塞的尺子，它该删。统一的结果类型已经存在 |
| P-2 | 2026-07-29 | 「探测机制不进 `ToolSpec`」的**理由换掉** | 原理由「四种机制各只有一个使用者」是**错的**——cc-acct-iso 的探测是远端 `command -v` + 解析 stdout，与 ccm **同族**，按 ≥2 判据反而够格。换成硬的那条：`ToolSpec` 是 `const` 声明式数据，探测是需要活连接/协议握手的**行为**。编译器只挡 `Box<dyn Fn>`，函数指针是 const-可构造的，所以这条边界靠结构守卫 |
| P-3 | 2026-07-29 | `TouchedFile` 拆 `path` + `note` | T02 一上手就撞到：原 `path` 混着散文（`~/.bashrc（或所选 profile）`），进不了 `Path`。第一版 glob 分支还把散文吞进 `suffix`，造出一个 0 匹配的**假缺失**——比报错更坏 |
| P-4 | 2026-07-29 | 账本新增 `ToolDestination::UserConfiguredPath` | **六条声明里三条没有任何代码支撑**：`remote-daemon` 的 `.local/bin/ccm-daemon` 全仓只出现在注册表自己里（真实是 `RemoteConfig.daemon_path`）、`cc-acct-iso` 声明成本机而实际是远端 + 前端传的 `dest_dir`、cc-bus 的落点是愿景。**声明一个不存在的常量比不声明更坏** |
| P-5 | 2026-07-29 | 注册表 ↔ 真写入方用 `pin_definition` 建**双向**耦合 | T02 审计：此前零耦合，所以这张告知页可以自信地说错而门禁不会红。现在改注册表或改 `sftp.rs` 的真常量，两边都红 |
| P-6 | 2026-07-29 | 新增 `TouchEffect::IndirectWrite` | `~/.cc-bus/` 原声明 `ReadOnly` → 渲染成「我们不写」，**假的**：驾驶舱两个按钮跑 `cc-send`/`cc-spawn`，而它们往 inbox / 名册追加。「我们只是调了别人的命令」不改变用户的文件因为在我们这儿点了一下而变了 |
| P-7 | 2026-07-29 | 「远端性从 `destination` 推导」这条**降级为无门禁的实现约定** | 原先那条守它的测试是**同义反复**（`Remote` 只由 `RemoteHomeRelative` 产生且必然产生），审计把 `ccm` 的 destination 翻掉、492 项照样全绿。删掉，不留永远不会红的钉子。真门禁要 `host` 字段 + `origin` 模型 → **T04 一起做** |
| P-8 | 2026-07-29 | T03 的消费者数从 **2 修正为 3** | MASTERPLAN 原写「两个真实消费者」。实数族 A 有 3 处，第三处（`remote-section.ts`）本身带两个真缺陷（无粘后指引 + 复制失败被 `console.warn` 吞掉）。**这是这次抽象最实在的收益，比去重重要** |
| P-9 | 2026-07-29 | `mergeNote` 做成**自由文本槽而非枚举** | 按枚举数下来 `MergeIntoKey` **只有 1 个用户**，不够 ≥2；而钩子那句"合并不是覆盖"是 load-bearing 的，通用化丢信息、套到 `.bashrc` 上又是错的。**共享的是"必须说一句"这个位置，不是那句话**（同 P-2 的形状：机制留各家，位置统一） |
| P-10 | 2026-07-29 | 族 B（5 处复制诊断）**如实登记不收** | UX 契约不同（没有"贴到哪"、没有"怎样才生效"），硬塞会让族 A 的三个必填槽在族 B 全是空的 |
