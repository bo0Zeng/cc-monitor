...
> ## 【已搁置】用户 2026-07-29 指示「code picture 先不做」
>
> 计划**原样保留**，不删——下面 §0 那份实测现状是有时效的：
> **翻转的零成本窗口在上游第一个新 commit 时关闭**（当时上游 HEAD 就是 vendored 的
> `d558e47`、领先 0 个 commit、`diff -rq` 无差异）。将来重启这件事时**先重跑 §0 那几条实测**，
> 别照抄本文件里的数字，也别照抄 `VENDOR.md`。
>
> 顺序改为：T04 第二步审计闭环 → **T07（设置面板 IA 重构）** → Phase G。
