...
- **当前功能**：无（G-B 刚签收）
- **当前步骤**：n/a
- **已完成功能**：**G-B**（vendored bash 进门禁）—— 见 `features/G-B-vendored-bash-gates.md`
- **下一个功能**：G-A（八套真机套件加断言数地板）· 之后 G-C
- **阻塞 / 待用户确认**：
  ...
  - **[待定] 地板值与 `ci.yml` 标签的同源对拍要不要做** —— 建议做（那些标签已经漂过一次：
    39 vs 44、12 vs 21）。**G-B 的两条地板刻意写在 `ci.yml` 里**（vendored 那套受 SS-10 限制，
    论证见 `features/G-B-…md` §3）⇒ 这条对拍对 **G-A 的八套自有套件**仍然适用、未被 G-B 解决
