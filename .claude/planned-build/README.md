...
| **auto-e2e/** | 给真机功能（灰灯/resume/attach/换号/账号）补 e2e 埋点 + Windows 全自动测试 harness | **✅ 全部 e2e 功能完成并随 v3.3.0 发版（2026-07-26）**：F-E0/F-E1 核心 · F-Vwin · F-E2 · F-E3 · F-E4 · F-E5。<br>**2026-08-01 订正**：本行原写「E2-E5 有完整计划、零落地记录」——那是**忠实抄了** `auto-e2e/STATUS.md` 顶部那行陈旧的「阶段」标签，而同文件正文早已记着全部完成 + 已发版。病根不在索引偷懒，在「STATUS 顶部摘要没随正文更新」。 |
...
| **account-zero/** | 把「基座」变成受管的「账号 0」（吸收破坏隔离的那个状态，而不是把它定义成违规）+ **2026-07-30 扩范围：原生身份组成的单点声明 + 版本钉/漂移检测 + 物理迁移能力** | **Phase A 已批（07-29）、07-30 按用户追加需求修订落地**。8 个功能（Z01-Z08）中 **6 个已带 commit 交付**（`Z08 60946b0` · `Z06 941e13d` · `Z07 e6674d3` · `Z01 7ad1ae3` · `Z04 f97bb76` · `Z02 部分 9cfdd38`，2026-08-01 核 `account-zero/STATUS.md` 订正；本行原写「零代码」）。**Z08 迁移能力是 P0**——它同时是 BACKLOG **E36「API key 路线乙」**的前置。**整个 cc-acct-iso 半区的硬前置是 G-B**。Z02 仍卡 `tabs.ts` 红线；动 `~/.claude/skills/` 与 z/b 真账号目录的授权**已给**，但用户说「先不要改，我在用 claude code」⇒ 动真账号那步等发话 |
| **rust-ts-boundary/** | Rust↔TS 边界从人工纪律改成生成物 + 门禁 | **✅ 八个功能全部闭环签收、本工作区主体完成**（2026-08-01 核该区 STATUS 订正；本行原写「Phase A 已落盘，等审批」）。选型订正见其 §8：`tauri-specta` 对 Tauri 2 只有 rc 版 ⇒ **不引**，改为「ts-rs 生成类型 + 手写包装层 + 守卫钉死漂移」 |
| **gate-integrity/** | 门禁不许在零断言下报绿（真机套件断言地板 + vendored bash 进 shellcheck + e2e 进 CI） | **✅ 三个功能全部完成**（G-A / G-B / G-C 均已签收；2026-08-01 核该区 STATUS 订正，本行原写「余 G-A · G-C」）。**G-B 解除了 `account-zero` cc-acct-iso 半区（Z01/Z04/Z06/Z08）的「没有网不能改那个工具」**。<br>**2026-08-01 Phase G 补**：G-A 的覆盖面地板守的是「已入网的别掉出去」，**不守「新的要进来」** —— branch-anywhere 新增的两套 e2e 就漏了一拍，已补进 CI（地板数 13 → 15）|
...
| **branch-anywhere/** | 任意对话节点分叉，**两条都活着**：远端也能 branch · 不走 CC 的 `/branch`（那是同一 pidfile 原地换 sid）而是复制 jsonl + 另起一个 · 继承原会话参数（账号/tmux/cwd） · 实时会话里也有入口 · 废弃分支保留路口但呈现区分 | **✅ 全区功能收官（2026-08-01）**：G0-G6 七个功能全部交付，各自一个 commit。**Phase G 审计抓出 3 条阻塞**（daemon `BUILD_ID` 没 bump ⇒ 整轮远端能力在已部署机器上休眠 · 追问路径的 tmux 名从 cwd 派生必被下游拒 · 远端拉起失败被吞成成功 toast），均已修并各带回归测试。**实证结论：我们的落盘格式与官方 `/branch` 一致**（两份原生 fork 的算法指纹跳过 562/52 条旁支 ⇒ 排除线性切片 ⇒ 官方与我们同为祖先回溯）⇒ **不改落盘行为**，G0 只把结论钉成测试。**不引官方 SDK**：它是 Python 而我们零 Python 依赖、daemon 是 musl 静态二进制，且它与 TUI 落盘不一致。**G2 要收窄 `readonly_guard`（撞红线，待批）**，同时解掉 E50；本区取代 E52 |
...
| **unified-backend/** | 把「后端」集成成一个（用户 2026-08-01「我们应该把 ccm 和 daemon 集成，即把后端集成」）。承 BACKLOG **E53** + **issue #77**，并处理 **#82** 的前置 | **主计划 v3 已定稿并获批，正在跑 U-1（第零梯队）**。架构由用户定框：**把 monitor 分解成 frontend / backend 两件，backend 再分 observe（读）与 control（起停接）** ——「decomposed the monitor … read and control … the same at local」。<br>⚠ **v1 的「决策内核 crate + 三宿主」已被用户否决**（「别搞成三个宿主」「把所有控制交给 daemon」），本行原先记的就是那个被否决的形态，2026-08-01 订正。<br>摸底实测：**19 条「起/停/接会话」的路，无一经 daemon**；5 个会话名生成器算法互不相同；13 个跨语言双写点（4 处无守卫）；`shared/ccm` **662 行**（不是 1300）里只有 **6 处**真搬不走。**顺带抓出两个非本区制造的真 bug**：Linux 宿主上远端终端拉起 100% 失败（`launch.rs:304`）· daemon `--resolve` 发的会话名与 monitor 已漂移（对 aterm 是现网 bug）。<br>待拍板从 6 条收敛到 **1 条**（D1「间接写算不算违反 §41.6」，不阻塞 U-1/U0/U2/U3） |
