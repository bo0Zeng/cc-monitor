...
| **branch-anywhere/** | 任意对话节点分叉，**两条都活着**：远端也能 branch · 不走 CC 的 `/branch`（那是原地换 sid）而是复制 jsonl + 另起一个 · 继承原会话参数（账号/tmux/cwd） · 实时会话里也有入口 · 废弃分支保留路口但呈现区分 | **Phase A 已落盘，等审批（2026-07-31）**。7 个功能 G0-G6。技术核心是对照官方 `claude-agent-sdk 0.2.128` 的 `fork_session` **真源码**照出我们六个缺口（uuid 不 remap 致溯源自指 · 不过滤 `isSidechain` · `logicalParentUuid` 全仓 0 命中 · 泄漏字段不清 …）。**不引 SDK**：它是 Python，而我们零 Python 依赖、daemon 是 musl 静态二进制。**G2 要收窄 `readonly_guard`（撞红线，待批）**，同时解掉 BACKLOG E50；本区取代 E52 |
