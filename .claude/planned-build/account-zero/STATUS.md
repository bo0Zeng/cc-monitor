...
- **已完成功能**：**Z08**（`isolate` 迁移，`60946b0`）· **Z06**（`NATIVE_IDENTITY` 单点声明，`941e13d`）
  · **Z07**（版本钉 + 漂移检测，`e6674d3`，销 E37）· **Z01**（账号 0 登记 + 可见，2026-07-30 签收）
- **当前功能**：—（Z01 已签收；按 README「四区连续执行顺序」下一个是 **#8 Z04**）
- **当前步骤**：Z01 Phase F 完成
- **下一个功能**：Z04 → Z02（**卡 `tabs.ts` 红线，跳过并标注**）→ Z03（承 Z02，同样跳过）→ Z05
- **阻塞 / 待用户确认**：
  ...
  - ~~**[待确认] 账号 0 的显示名**~~ → **Z01 已定**：manifest 里就叫 `"0"`，且 `"0"` 是**保留名**
    ...
    只动 UI 文案即可，不会牵动判据。
  - **[Z01 登记的后续] 账号 0 暂不可选**：从 UI 起它需要「**显式 unset** `CLAUDE_CONFIG_DIR`」这种
    注入形态（**只是「不注入」是错的**——远端 rc 里的 `export CLAUDE_CONFIG_DIR=<默认>` 会让它落到
    别的账号上）。要动 `launch-plan` + `tabs.ts`（**红线未松**）⇒ 排在 Z02/Z03 之后。
