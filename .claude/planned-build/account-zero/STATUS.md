...
- **已完成功能**：**Z08**（`isolate` 迁移，`60946b0`）· **Z06**（`NATIVE_IDENTITY` 单点声明，`941e13d`）
  ...
  · **Z04**（守卫，`f97bb76`）· **Z02 部分**（`--base` 跨语言契约守卫，`9cfdd38`；三态化卡红线）
  · **Z03 部分**（(a) 用量探针支持账号 0，`2b73c13`；(b) 按会话切号卡红线）
  · **Z05**（rc 片段一键生成，2026-07-30 签收）
- **当前功能**：—（**account-zero 区能做的全部做完**；下一个是 gate-integrity 区 **#12 G-A**）
- **当前步骤**：Z05 Phase F 完成
- **下一个功能**：**#12 G-A**（gate-integrity 区）→ #13 G-C。**本区余下三项全卡 `tabs.ts` 红线**
  （Z02 三态化 / Z03(b) 按会话切号 / Z01-follow 账号 0 可选），七步顺序见 `features/Z02-PARTIAL.md` §6。
