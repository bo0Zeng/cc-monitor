...
| Z05 | **rc 片段一键生成**（独立） | 把 `cc-acct-iso shellinit` 接进 T03 的「生成待贴文本」组件，一键复制 | **✅ 已交付**（features/Z05-rc-snippet-generation.md） | — | P3 |
...
**Z05 为什么在这儿但标 P3**〔**交付时实测订正**：**F14 不在 `BACKLOG.md` 里**，它在
`audit-fixes/MASTERPLAN.md:36` 且早已标成划掉 +「用户自跑」；而且 F14 是**两件事**
（`.bashrc` zcc/bcc 迁移 + wrapper 去轮询），**Z05 只覆盖第一件的文本生成那一步，贴不贴仍归用户**〕：
它对应 **F14**（「`.bashrc` 迁移用户自跑」），
...
| # | 日期 | 改了什么 / 为什么 |
|---|---|---|
| **新9** | 2026-07-30 | **Z05 交付签收（rc 片段一键生成）。account-zero 区能做的到此全部做完**，余下三项全卡 `tabs.ts` 红线。**关键设计决定**：片段**从远端 `cc-acct-iso shellinit` 抓**，不在 TS 里重写一份生成逻辑 —— 后者会多一个跨语言双写点（本区反复在治的病），而它随账号数/默认号/launcher 变化，抄错的表现是**用户贴了个坏 rc**。抓输出则单一来源留在 bash，一处都不用同步。已有 `exec_collect` 可用，`cmd_shellinit` 全是 printf（逐行核过只读）。**fail-closed**：BEGIN 与 END 围栏**都要在** —— 只查 BEGIN 的话，被截断的输出会带着半截片段过关，而**半截片段贴进 rc 会让登录 shell 直接报错**；两种失败给不同的可执行诊断。前端 `invalidReason` 再校验一遍（能显示 ≠ 能贴），组件拒绝时**绝不碰剪贴板**。**★ 撞出 T03 守卫的结构性盲区**：`accounts-section.ts` 原是纯族 B（复制完就完事），Z05 给它加了真·待贴块 ⇒ **两族都属**，而 A/B 二分覆盖不了（塞族 A 被「不得有裸 writeText」打红、留族 B 被「不许有待贴语义」打红）。处置是**新增族 AB + 把判据从「有没有 writeText」换成「处数恰好等于已登记数」**（族 A 那条真正要防的是手搓复制按钮绕开组件；对混合文件而言计数才是可判据）。**这不是放松守卫，是换成能覆盖新形状的判据。****计划两处实测订正**：F14 **不在 `BACKLOG.md`**（在 `audit-fixes/MASTERPLAN.md:36`，早已标「用户自跑」），且它是两件事，**Z05 只覆盖第一件的文本生成那一步、贴不贴仍归用户**。四道既有守卫按设计触发（命令数 119→120 三条 + 待贴块族），顺带订正一处陈旧的测试标题（写着 112、断言早是 119）。四条变异全部成立。vitest 860→**866** / cargo 618→**620** / 命令数 **120** |
