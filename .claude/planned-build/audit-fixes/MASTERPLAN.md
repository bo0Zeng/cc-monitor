...
| `.github/workflows/ci.yml` + `package.json` | F08,F09,F13 | 终态 job：rust/frontend(+lint+coverage)/daemon/vendor-crate/e2e-smoke + 双写点断言 | ✅ F08 落成：双写点断言 + daemon 只读护栏=cargo 测；frontend job 加 eslint/stylelint(advisory `|| true`) + coverage 地板(阻断)。F09 补 vendor-crate/e2e-smoke |
