...
| ID | 功能 | 一句话 | 状态 | 依赖 | 优先级 |
|----|------|--------|------|------|--------|
| F01 | follow-resume 账号安全 + resume 选账号 | pin 现读磁盘(修 B1);resume 入口加账号下拉(含「基座/不隔离」),默认全局当前账号,显式重钉(修 #75) | 实现中 | — | 🔴P0 |
| F02 | kill 白名单 | `kill_remote_tmux` 补 `is_ccm_tmux_name`(与 send-keys 对称)+订正假注释(修 I1) | 待规划 | — | 🔴P0 |
