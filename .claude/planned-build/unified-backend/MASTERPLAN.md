...
## §0.0b 铁律 11：「未实现」≠「未裁定」（2026-08-03 立，因为它失效过 15 轮）

任何要写进文档的**「待裁决」/「阻塞」**，必须**指名 §0/§1 里哪一条没有覆盖它**；
指不出来 ⇒ 它是**活**，不是阻塞。

**立这条的代价**：Phase F 要求「回看并更新主计划」，而实际执行成了**单向写入** ——
每轮只往本文件追加（账本行、变更记录），**§0.0 与 §1.1–§1.5 从头到尾没被完整读过一次**
（每轮都是 grep/sed 定点开）。后果是连着四次把本文件**逐字写着的原则**当成「待用户裁决」上报：

| 被当成待裁决的 | 其实早就在 |
|---|---|
| ccm 与 daemon 谁做哪一半 | §0.0「thin ccm 是 backend 的**客户端**」· §1.3「**它是执行臂，不是宿主**」 |
| §34 Gate 2 要不要搬进后端 | §1.1「**任何改状态的 tmux 命令…一律归 `control/`**」 |
| 有没有 daemonless | §0.0「一份代码、**两种承载**」· §1.2「本机 backend = **安装包的一部分**」 |
| 能不能轮询 | §41 零定时器 + §1.3 已登记**唯一例外**（预信任等信任框） |

并因此给 U10 / U11 / U9b / U12 各挂了一条**假阻塞**，还错判了「接不属于 U10」
（§1.3 的分界是**有没有决策权**，不是进程在哪 —— 接的**决策**归 `control/`，只有 `exec` 留在终端）。

⚠ **同一个病的第二个症状是计划膨胀**：§1.4b「monitor 侧也要有 backend 边界」，
§0.0 的架构图里本来就有 —— 因为没读 §0.0，写了一节重复的原则。
**改本文件前先确认那条不在里面。**

⚠ **执行口**：写进 `STATUS.md` 的恢复入口顶部（loop 每轮唯一必读的地方），
并由 `plan_reread_guard` 机检（见下）—— 散文纪律 = 没有纪律。
...
| S5 | **`common/`**<br>**U2 已交付**：`projects_root`（**5 处不是 4 处** —— 第五处是 `watcher.rs::watch_loop` 里内联的，`grep fn` 找不到）· `mtime_ms`（2 份）。门槛写在 `common/mod.rs`：≥2 **层**用 · **平台无关** · 无域知识。<br>⚠ **「时间换算 · quote」这两项要划掉**：Phase D 审计逐条核过，daemon crate 内**没有可合的逐字副本** —— 时间换算三处语义/单位各不相同（`file_mtime_epoch` 单位是**秒**不是毫秒），quote 在 daemon 内只有一份、多份在 monitor 侧**跨 crate**、`common/` 收不了。<br>⚠ **U3 必须复查**：`mtime_ms` 的两个调用点同属 observe，「≥2 层」按层口径**今天不成立**；`observe/` 一建出来就要重判 | U2 · U3 <br>⚠ **U8c-1 订正：「quote 收不了」这条前提不再成立** —— 共享 crate 家族本身就是那个载体，而 U8c-1 在 `launch-core` 里加了**第三份** monitor 侧 POSIX quote（`launch.rs:30` · `ssh_source.rs:1715` · `launch_core::posix_quote`，加 daemon `control/tmux_hook.rs::sq` 共**四份 Rust 副本**逐字节等价，再加 TS `posixQuote` 与 ccm `sq` 共六份）。~~要么收进 `launch-core` 让三处调它，要么开一条对拍 —— 今天两样都没有。~~ **U8c-2b-0 已收口**：五处全部委托给 `launch_core::posix_quote`，外加零命中守卫 `quote_singleton_guard`（Rust 侧只许一个实现）+ 三入口行为对拍。⚠ **是「五处」不是四处** —— 账本与我摸底都记成四份，`acct_iso_deploy.rs::sq` 那第五份是守卫第一次跑就抓出来的。TS `posixQuote` 与 `shared/ccm::sq` 不在此列（跨语言由黄金串夹具对拍；ccm 那份是 S10 的刻意副本）|
...
| S18 | **账号契约 + 名字安全判据**（U7-3 新登记）<br>**U7-3 已交付**：四条常量 + `is_deceptive_char`（并集）进 `acct-core`；两条守卫**退役**，因为漂移已不可表示 | 唯一定义在 crate 里。**`is_safe_config_dir` / `norm_dir` 刻意不合** —— 那是平台特化不是漂移（本机要认 Windows 盘符且必须允许 `\`） | U7-3 ✅ ·（U8a-2a 补账：同 S17，4 条测试此前不进 CI）<br>⚠ **U8c-1 补账**：U7-3 把并集给了两个**读 manifest** 的地方，**「拼命令」那条路当时漏了**。U8c-1 已把 **POSIX 命令面**接上（`history.rs::validate_config_dir_posix` → `launch_core::config_dir_command_safe`）。**仍在旧表上的两处**：`history.rs::SPOOFABLE`（Windows PS 路，平台特化，留 U4b/U8c-3）· TS `shell-quote.ts::isValidConfigDir`（远端那条路，留 U8c-2/3）。⚠ **这次变严在两种语言之间开了一条新缝**：同一个含 `U+3000` 的 configDir，**远端 TS 拉起放行、本机 Rust 拉起拒绝**。<br>⚠ 顺带查出 `local_accounts.rs` 那句「本模块产出的值不进任何 shell」**已经过期**（它经 `fork-flow.ts` → `resume_history_session` → `bash -lic` 进了 shell），已订正 <br>⚠ **U8c-3-pre 结案（2026-08-03）**：三处（Rust 命令面 / TS `isValidConfigDir` / Windows）**全部接上并集**，`history.rs::SPOOFABLE` 那张 U7-3 之前的旧表**已删除**（不是留着不用）。U8c-1 自己开的那条跨语言缝（同一个含 `U+3000` 的 configDir 本机拒、远端放行）**已收**，判据是 `shell-quote-deceptive-parity.vitest.ts` —— 它**读 Rust 源码解析码位、逐个真喂给 TS 函数**，是行为对拍不是文本对拍。<br>⚠ **U8c-2a 再收一格**：**用量探针**那条路的 configDir 也接上并集了（Rust 侧 `launch_core::config_dir_command_safe`）。TS 侧 `isValidConfigDir` **刻意留着**作前置校验（纵深防御 + 「探测不发起」那 6 条既有判据），所以那条路现在是**两道闸、外松内严** |
...
| S29 | **周期唤醒面**（U7-P 新登记）<br>daemon 侧由 §41 `no_timer_guard` 零容忍钉住，而**前端 + `shared/ccm` 这半一条机检都没有** —— 那条护栏自己的头注写着「要钉那半得单独论证」，而**那个论证到 2026-08-03 没人做过**；两个文件的头注里写着「本文件里不得出现 setInterval / setTimeout 轮询」这类**散文纪律**（`account-usage.ts` · `settings/cc-bus-section.ts`）。<br>**U7-P 已交付**：`polling_registry.rs` —— 登记表（不是禁令，因为前端确有正当周期行为）+ 五条机检。普查实测**四个文件七处**：`main.ts` 10s 账号轮询（data-poll，退役归 **U7e**，而 U7e 按 §1.5 必须排在 U8 之后）· `grid-monitor.ts` 1s 重绘（**ui-clock，不取数**，正当）· **`tabs.ts::awaitExitFor` 1s 拉 `list_remote_tmux`（data-poll，本轮普查新发现 —— 它用递归 `setTimeout` 而非 `setInterval`，此前任何地方都没登记过；退役归 U10）** · `shared/ccm` 两处（预信任等待 = §1.3 登记在案的例外 · **1s 身份轮询 = data-poll，退役归 U9b**）。<br>⚠ **诚实边界**：查不了「没按 `poll` 命名的递归 `setTimeout`」（语法上与一次性延时不可区分）；**monitor 的 Rust 侧刻意不在范围内**（那边 `thread::sleep` 多是等一次性条件而非节拍，逐条论证是另一件事，如实登记为未做） | 三类各自说清：`ui-clock` 说明不取数 · **`data-poll` 必须写明事件源在哪 + 谁退役它** · `wait-for-condition` 说明没有内核事件源。新增一处没登记 ⇒ 红；登记表里的退役了不删 ⇒ 也红 | U7-P ✅ · U7e · U9b · U10 |
...
| S12 | **会话名生成**<br>**U11 摸底已落地为递减棘轮**（`session_name_registry.rs`）：实测**今天 5 个产出点 + 1 个消费点**，`== 2` **钉不上** —— ccm 那份要等 **U9b（⛔）**，硬钉就是本行自己警告的「做完必红」。⇒ 先钉当下：多一处 ⇒ 红（防回潮）· 少一处 ⇒ 也红（提醒往下拧 + 更新本行）。★ 普查两件**新发现**：`launch-requests.ts` 有第二份 `<sid8>-cc` **且不做撞名检查**（F74 存在的理由就是撞名，**可能是真缺陷，登记待查**）· `cc-spawn` 用 `_cc` **下划线**，`is_ccm_tmux_name` 认不出它（**待裁定**）。⚠ 裸查 `-cc` 会撞 CSS 类名，必须收紧成「紧跟收尾引号」 | 两族各一个函数 + 计数守卫 == 2。⚠ **守卫挂 U11 不是 U8**（ccm 到 U9、cc-spawn 到 U11 才收编，挂 U8 是**做完必红**的 DoD） | U8 · U9 · U11 |
...
- **`branch-anywhere/`**：Phase G 已完成、**只剩发版**，下一步是给 aterm 发 `--fork-session` 契约冻结通报。
  ...
  ⚠ **U11 摸底订正（2026-08-03）**：那条待办**其实已完成** —— R-4 那行（本文件 §「两仓 lockstep」）
  里 `--fork-session` 已经在清单上、标着「v3 补」。本文件两行之间的内部不一致，就此收口。
...
| **U8c-1** ✅ | **载荷渲染进共享 crate `launch-core` + 跨语言逐字节对拍** | 2026-08-02 交付。摸底把这件事的形状改了两次：**载荷有六份不是两份**（跨 TS/Rust/shell 三种语言，账本 S28）；**外层 tmux 那半本轮不搬，但不是因为它「没了」**（我第一版这么写、被审计证伪：daemon `launch` 零生产调用方 · 它结构上不 attach · `account_usage.rs` 今天就在 Rust 里拼外层）。`history.rs` POSIX 分支已改调内核，顺带补上 **U7-3 漏掉的命令面并集**（S18） |
| **U8c-2a** ✅ | **用量探针的载荷进内核** | 2026-08-02 交付。`account_usage` 从「收渲染好的串」变成「收结构化账号表态」；`render_payload` 有了第一个生产调用方；**S28 六份变五份**。⚠ 原定的「本机那条先切」摸底证明是**空转**（`resume_history_session` 早就收结构化参数，§36 + R07 已裁决本机不经 IR）|
| **U8c-2b** | **前端发结构化请求**（远端起会话主路，Rust 渲染），TS 渲染器降级为「只供对拍」 | ⚠ **它还要再拆** —— 今天要一次吞下六件：结构化请求 · **U8a-2c** · **U8b 缺的那半（远端 POSIX 拉起）** · 维度注册表搬进 Rust（连着 §35/§37/§38）· ~~`account_usage.rs` 的载荷来源改喂内核~~（**U8c-2a 已做**）· `--resume` 展开与 `sanitizeRemoteLauncher`。<br>⚠ **原来这里建议的「本机那条先切」已被 U8c-2a 摸底否掉** —— `resume_history_session` 早就收结构化参数、本机不经 IR（§36 + R07）。剩下的都是远端那条，**下一刀建议按 plan 形态切**：先 `container:"none"`（`render_payload` 已覆盖），tmux 那两种要连 `session-backend.ts` 一起想 |
| **U8c-3** ⛔ | 删 TS 两个渲染器 + IR，收敛 §33/§35/§36/§37/§38/§39，**重指 S19 第五处** | **2026-08-03 三问已逐条实测（§33b 有表）：①「否」③「未决」⇒ 今天删不得。真前置不是文档，是 U8a-2c 与 U12 两件未做的功能。** ⚠ 删 `session-backend.ts` 前必须先回答三件事（§33b 写死）：① 生产切到 daemon `launch` 没有；② **attach 那条串归谁产**；③ **daemonless 的远端**（U12 未决）还要不要能起会话。牵 E24 |
...
U-1 ─► U0 ─► U1a ─► U2 ─► U3 ─► U1b ─┬─► U4 ─► U5 ─┐
                                      ...
                                      └──────────────┘                                    │
   ┌────────────────────────────────────────────────────────────────────────────────────┘
   └─► U8a ─► U8b ─► U8c-1✅ ─► U8c-2 ─► U8c-3 ─► U8d ─► U7e ─► U9b ─► U10 ─► U11 ─► U12 ─► U13 ─► U14 ─► U15
...
- 2026-08-02 **U8c 拆成 U8c-1/2/3**，U8c-1 交付（载荷渲染进 `launch-core` + 跨语言入库夹具对拍）。
  摸底两次改写形状：载荷是**六份跨三语言**不是两份；外层 tmux 那半本轮不搬**但不是因为它没了**。
  ⚠ **我这轮往耐久文档里写了两句假话，都是审计当场证伪的**：`doc/INVARIANTS.md` §33b 的
  「外层容器那半已经没了」· `launch_payload_parity.rs` 注释里的「monitor Rust 侧没有 nestedEnv 常量」。
  前者会让 U8c-3 的执行者跳过三个必答问题，后者恰好挡住了本可以补上的钉子。两处都已订正并留痕。
- 2026-08-02 账本：**S5 / S18 / S20 改写**（S5 那句「quote 收不了」被本轮**自己证伪** ——
  造出了载体却在里面加了第三份 Rust quote）；**新增 S28**（载荷产出点 + 两套跨语言对拍互不覆盖 +
  夹具机制的结构性盲区：TS 会 throw 的输入根本进不了夹具，那类差异加多少用例都覆盖不到）。
- 2026-08-02 **把「新增共享 crate 要补 CI 三样」从散文变机检**（`shared_crate_registry.rs`）——
  这条纪律已被违反过两次（branch-core 漏两样 / usage-core+acct-core 三样全漏两轮），
  而违反它不会红、只会静默少跑。
- 2026-08-02 **U8c-2a 交付**：用量探针的载荷从 TS 搬进 `launch_core::usage_probe_payload`
  ⇒ **S28 的六份变五份**，`render_payload` 有了**第一个生产调用方**。
  `account_usage` 的 IPC 从「收渲染好的串」变成「收结构化账号表态」（`config_dir: Option<String>`，
  `None` = 账号 0，**不是「不表态」**）。
  ⚠ **行为变化两处，都如实登记**：① `unset` 的顺序从 TS 序变成 Rust 序（语义无关，`unset A B` 等价）；
  ② 探针路径的 configDir 校验变严（并集）。
  ⚠ **原定的「本机那条先切」是空转** —— `resume_history_session` 早就收结构化参数、本机路径
  **不经 TS 渲染器**（INVARIANTS §36），而且 `launch-requests.ts` 里 **R07 明确否决过**「真接上 IR」
  （理由：`plan.action`/`plan.cwd` 恒等于输入，取回来没有信息增量）。摸底当场改计划。
- 2026-08-02 **`args` 盲区闭掉**（U8c-1 登记、U8c-2a 兑现）：`render_payload` 给每个 arg 加白名单，
  含空白/元字符 ⇒ `Err`。**刻意不改成逐个 quote** —— 那会与 TS 的 `join(" ")` 逐字节分家，
  而黄金串对拍正靠字节相等；白名单让会裂/会注入的那一类不可表示，合法输入的字节一个没变。
- 2026-08-02 **U8c-2a-fix**：还上「接线那一行没有判据」的债。审计建议的「让 e2e 由真接线驱动」
  **走不通**（e2e 用 `FAKECLAUDE` stand-in，而载荷里的启动器恒 `claude`，沙箱没有真 claude），
  且「参数有没有被转发」在 Rust 类型里表达不了 —— 换任何一层包装变异只会下移一层。
  ⇒ 用**源码接线守卫**（调用点恰好一个 + 实参含 `config_dir`）。R1/R2 两个变异实测转红，
  外加一条「多一条绕过调用也要红」的自检。**它是约定不是事实，这句写在测试头注里。**
- 2026-08-02 **U8c-2b-0**：账本 S5 收口 —— POSIX 单引号 quote 在 Rust 侧从**五份**收成一份
  （`launch.rs` · `ssh_source.rs` · daemon `tmux_hook.rs` · `acct_iso_deploy.rs` 四处委托给
  `launch_core::posix_quote`），外加零命中守卫 `quote_singleton_guard` + 三入口行为对拍。
  ⚠ **「五份」是守卫数出来的，我摸底和账本都记成四份** —— 第五份 `acct_iso_deploy.rs::sq`
  是它第一次跑就抓到的。五份逐字节相同、从来没红过，靠巧合保持一致。
  ⚠ **我自己写的两条判据被变异复验打回重写**：反向自检有个宽松备选导致**恒真**
  （一条恒绿的自检比没有更坏）；行为对拍只覆盖三个入口里的一个。
- 2026-08-02 **`container:"none"` 那一刀判定为 U8c-2c 的一部分，不是最小刀**：
...
- 2026-08-02 **U8c-2c-2：本工作区第一条真正切过去的生产渲染路径** —— `ccm 调用行`改由 Rust
  渲染（`render_ccm_launch`）。切的是**真在跑的那支**（装了 ccm 时走 CLI 形态）；
  **兜底那支不切** —— 它要 `session-backend.ts`，而 §33b 有三个未答问题 ⇒ U8c-3。
  返回值做成 tagged 而不是 `Result`：「渲染不出来」是**诚实降级**不是错误，
  用 `Err` 表达会和「IPC 真的失败了」混成一件事，而那两件在前端走不同分支。
  ⚠ **我写的判据初版第三次不合格**：`deny_unknown_fields` 那条检查往前扫 220 字符找子串，
  而那些类型的**文档注释里就写着这个词** ⇒ 摘掉真属性照样绿。**散文不是属性。**
  三轮的形状是同一个：**判据的匹配面比它声称的宽**（恒真自检 → 只覆盖三分之一 → 匹配到散文）。
  ⚠ 另有一次我自己造的过头改动：`commands.vitest` 的散文写 121、真断言用 124，
  我顺手把散文一并改了 —— **改了一个与本轮无关、且没查证过的数**，已还原。
- 2026-08-03 **§33b 三问逐条回答，结论：U8c-3 今天做不了**。① 生产没切到 daemon 的 `launch`
  （零生产调用方，U8a-2c 未做）· ② attach 只有一半有答案（装了 ccm 的已是 Rust 产，
  没装的仍靠 `session-backend.ts::attach`）· ③ daemonless 未决（U12 待做）。
  硬删会把「没装 ccm 的远端」与「daemonless 的远端」两类主机的起会话能力直接删掉。
  **三问是有效的 —— 它挡住了一次看起来该做、实际会删功能的改动。**
- 2026-08-03 **账本 S18 结案**：三处（Rust 命令面 / TS / Windows）全部接上 `acct-core` 并集，
