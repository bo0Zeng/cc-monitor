...
| S28 | **「起会话」载荷的产出点**（U8c-1 新登记）<br>同一段 `env 前缀 → cd → argv → wrap` 今天有**六份、跨三种语言**（⚠ 我第一版摸底只数出四份，工程审计补齐）：① TS `launch-render-fallback::renderEnvOps` + `shell-quote::buildEnvPrefix`/`UNSET_CONFIG_DIR_PREFIX` · ② TS `remote-launch::buildUsageProbePayload` · ③ Rust `history.rs` POSIX（**U8c-1 起调内核**）· ④ Rust `history.rs` PowerShell（`$env:`，平台变体不是副本）· ⑤⑥ **`shared/ccm` 的 `--print` 段与 exec 段**（S10 已裁定刻意不合并，靠 `ccm-contract-parity` 差分对拍）<br>**消费方**：`account_usage.rs`（收 TS 传来的 payload、只透传不校验）—— ② 一退役它就要改喂内核，**今天没有落点** | 唯一 Rust 实现在 `launch-core`；跨语言一致性由**入库夹具**保证（`crates/launch-core/fixtures/payload-golden.json`：TS `launch-payload-golden.vitest.ts` 断言「入库 == 现场」、Rust `launch_payload_parity.rs` 断言「Rust == 入库」，两侧各有计数自检，且 Rust 侧 `include_str!` 钉住 TS 那一半还在）。⚠ **绝不能让 Rust 去调 TS 现场生成**（U7-4 的自洽夹具病根）。<br>⚠ **今天有两套跨语言对拍并存且互不覆盖**：`ccm-print-parity`（TS ↔ ccm）与本条（TS ↔ Rust）—— ⑤⑥ 与 ①②③ 之间**没有任何判据**。<br>⚠ **夹具机制有一个结构性盲区**：TS 会 throw 的输入（非法/空 configDir）根本进不了夹具 ⇒ 那一类差异**加多少用例都覆盖不到**，只能靠类型（`render_payload` 回 `Result` 并自己 fail-closed）| U8c-1 ✅ · U8c-2 · U8c-3 · U9b |
...
| **U8c-2** | **前端发结构化请求**（Rust 渲染），TS 渲染器降级为「只供对拍」 | ⚠ **它还要再拆** —— 今天要一次吞下六件：结构化请求 · **U8a-2c** · **U8b 缺的那半（远端 POSIX 拉起）** · 维度注册表搬进 Rust（连着 §35/§37/§38）· `account_usage.rs` 的载荷来源改喂内核（**今天无落点**）· `--resume` 展开与 `sanitizeRemoteLauncher`。<br>建议下一刀：**本机那条先切**（`resume_history_session` 收结构化 plan，POSIX/Windows 都在 Rust 手里、无跨机传输），远端那条单独成件 |
