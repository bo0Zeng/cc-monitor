...
- **当前功能**：U7-1..U7-5 已闭环。**§0.1 分类 ③ 已判定**（见变更记录）：两处性质不同、都不按合流排 —— U7d 是「本机读面在 Linux/macOS 上整体不工作」，U7b 是「旧轮询路径退役了没有」。**U7 整族收尾**（U7-1..5 / U7b / U7d 全闭环）。下一个梯队：**U8 起会话搬进 `control/`**（U8a–U8d）。
