...
- **当前功能**：U7 整族闭环。**U8a 判定已出**。**D1 已裁决**（铁律收窄 + 起进程受管清单落成机检）。**U8a-2 顺序已订正**（入方向通道当时不可达 ⇒ 先接发送端）。**U8a-2a 已闭环** —— monitor 侧发送端接上，入方向通道**在生产里可达了**（「测试连接」的控制通道往返探测是第一个生产调用方；15 条真进程 e2e 进 CI）。下一个：**U8a-2b —— `launch` 命令本体**。
...
### 当前门禁基线（U8a-2a 闭环后实测，2026-08-02）

monitor `cargo test --all` **704 / 3 ignored** · daemon **222** ·
共享 crate `branch-core` / `usage-core` / `acct-core` / `guard-core` 各自 `-p` 跑（**新进 CI**）·
`npm test` **80 文件 1154 例** · `tsc` **0** · 两侧 + 四个共享 crate `cargo fmt --check` 干净 ·
clippy daemon **0** / 共享 crate 各 **0** / monitor **62（与 HEAD 逐条相同，同一计量口径实测对比）** ·
daemon 跨 target Windows `cargo check` 过 · e2e `inbound-frames` **15/15**（新，进 CI 带地板）。

⚠ **clippy 计量口径**：`--message-format=short | grep -c warning` 得 62；
早先记的 **64** 是另一种数法（全格式 `grep -c "^warning"`）。本轮为了断言「零新增」，
**用 `git worktree` 拉了一份 HEAD 用同一条命令量**，两边都是 62 —— 别拿两种口径的数互相比。

⚠ **daemon 从 226 掉到 222 不是删测试**：guard_support 的 7 条通用测试搬进了共享 crate
`guard-core`（那边现在 11 条），daemon 侧留 3 条 daemon 专属 + 1 条新的再导出自检。

⚠ **测试条数对账要用新鲜构建**，且**别手抄**（`8a13ba9` 的 commit message 写 667、
审计在同一 commit 上实测 666）。`cp -a` 还原后必须 `touch`（cargo 按 mtime 判缓存）。
