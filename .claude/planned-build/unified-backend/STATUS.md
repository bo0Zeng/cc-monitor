...
- **当前功能**：U7-1 / U7-2 已闭环。下一个：**U7-3 账号内核抽共享 crate**（§0.1 ② 的另一半）。
...
| U1b · U4b · **U7-3**（账号共享 crate）· U7b/d · U8–U15 | 待做 | — |
...
### 当前门禁基线（U6b-1/2 闭环后实测）

daemon `cargo test` **224** · monitor `--lib` **668 / 3 ignored** · `npm test` **80 文件 1154 例** ·
`tsc` 0 · 两侧 `cargo fmt --check` 干净 · clippy daemon **0** / monitor **64（与 HEAD 逐条相同）**。
门禁脚本里每一步失败都 `exit 1`，不是 `&&` 链上的 echo。

⚠ **测试条数对账要用新鲜构建**，且**别手抄**：`8a13ba9` 的 commit message 写 667，
审计在同一 commit 上实测 666 —— 那个数是我在中间态量的、写进 message 时没重新量。

⚠ **测试条数对账要用新鲜构建**：本轮量到过一次 203/204 且冒出一个不存在的 `zz_dump` ——
那是陈旧的测试二进制（`cp -a` 保留 mtime，cargo 认为文件没变而复用旧产物，
`include_str!` 是编译期烘进去的）。**`cp -a` 还原后必须 `touch`。**
