...
- **下一轮候选**（都要先答一个未决前提，按「答得起」排）：
  ① **U11 分叉 + cc-spawn 收编** —— daemon 侧 `control/fork_write.rs` 已存在，且分叉不碰
     §34 那三道门（它写的是自己刚建的记录），**可能是唯一还有真刀可切的一件**；
  ② U8 剩两刀 / U10 四件 —— 都卡在「ccm 契约谁做哪一半」或「Gate 2 怎么搬进 daemon」；
  ③ U12 daemonless 处置 —— 它是 §1.2 与 S29 里好几条的共同解锁条件。
