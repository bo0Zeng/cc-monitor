...
- **当前功能**：U7-1..U7-5 已闭环。**§0.1 分类 ③ 已判定**（见变更记录）：两处性质不同、都不按合流排 —— U7d 是「本机读面在 Linux/macOS 上整体不工作」，U7b 是「旧轮询路径退役了没有」。下一个：**U7d 的另一半**（要不要给 `is_process_alive` 写 `/proc` 实现 —— 那是功能决定，不是清理）。
...
| **U7d** 文档那一半（本机读面只在 Windows 工作） | ✅ 已写进 ARCHITECTURE + 双语 README | 待提交 |
| U1b · U4b · **U7d 的功能那一半** · U8–U15 | 待做 | — |
