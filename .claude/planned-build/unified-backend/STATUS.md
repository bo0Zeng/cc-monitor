...
- **当前功能**：U7 整族闭环。**U8a 判定已出**。**D1 已裁决**（铁律收窄 + 起进程受管清单落成机检）。**U8a-2 顺序已订正**（入方向通道当时不可达 ⇒ 先接发送端）。**U8a-2a 已闭环** —— monitor 侧发送端接上，入方向通道**在生产里可达了**（「测试连接」的控制通道往返探测是第一个生产调用方；15 条真进程 e2e 进 CI）。**U8a-2b 已闭环** —— daemon 侧长出真正的「远端执行面」（argv 直传不过 shell、`send-into` 一等模式、真 tmux e2e 27 条）。
  ...
  **U9a 已闭环** —— S10「保住清单」从散文变机检：新增 `ccm-contract-parity` **31 条**
  ...
  真正硬的只有「没有决策权威可搬」+ **排期**。
  **下一个：U8c（两个 TS 渲染器退役 + 前端发结构化请求）** —— 审计的原话我接受：
  U9b / U10 / U11 **全在 U8c 下游**，U8c 未做之前从第六梯队里挑任务只会反复得到「阻塞」。
  **不再往下游找软的。**
...
| **U8c** ← **下一个** · U1b · U4b · U8d · **U9b**（阻塞：U8c 的完整载荷权威 + 载荷送得到 ccm）· U10–U15 | 待做 | — |
...
**e2e 17 套全绿**（新增 `ccm-contract-parity` **31**；shellcheck 覆盖 **41 个文件**、error 级零告警）。
