...
⚠ **R1/R2 仍未被钉住，如实登记不假装做完**：抽函数只把接缝从「一整段」缩成
`account_usage` 里**一行、一个 token**（`config_dir.as_deref()`）。要真杀它，得让
usage-probe 的 e2e **由真接线驱动**（两态各发一条场景）—— 审计实测确认了这一点：
它试过的两种纯函数写法都杀不掉 R1/R2。**这条写进 `probe_command_for` 的头注里。**
...
- [x] 过代码审计（D）—— **零阻塞**；抓到一条**我造的覆盖回归**（4 个变异在全绿门禁下存活），
      R3/R4 已补判据、**R1/R2 如实登记未做完**；四处说假话的注释全部订正
