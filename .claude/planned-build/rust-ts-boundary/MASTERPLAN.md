...
- **整体成功标准**：
  ...
  4. 全仓 `import { invoke }` 的直接调用点从 **29 个文件**降到 **1 个**（手写的包装层）。
     **⚠ 这条不松 `tabs.ts` 红线结构性达不到**（该文件 15 处调用点，全仓第二多）。
     未获授权时**如实改写为「降到 1 + `tabs.ts`（等授权）」，不假装达成**（C04 Phase B 实测）。
...
| C04a | **类型化 `invoke` 包装层 + 钉死 119 个命令** | 建机制：手写包装层（`ts-rs` 不生成命令签名）+ 把只覆盖 3/119 的白名单守卫扩到 119；先迁一个模块做样板 | 待规划 | C01 | P1 |
