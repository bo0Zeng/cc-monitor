...
| C04d | **按模块分批迁移** | 每批一模块、一 commit、一次全门禁；`import { invoke }` 文件数逐批下降 | **完成**（八批 11 个 commit；两批等授权如实标注） | C04a | P2 |
...
- 11 — 2026-07-30 — **C04d 八批全交付，本工作区主体完成** —
  `import { invoke }` 的生产文件 **29 → 3**（`ipc/commands.ts` + `tabs.ts` 等授权 + `accounts.ts` 等 Z02）
  ⇒ **§0.1 成功标准 4 达成**。包装层 **109** 条 · 生成物 **67** · **119 个命令全部静态可见**
  （C04a 立的「7 个命令 TS 静态看不见」盲区**归零**，`DYNAMIC_ONLY` 现为 `[]` 且**保留断言**
  以钉「不许再出现新的动态命名调用」）。

  **两处推翻了自己先前的设计/结论**：① 批 6a 推翻 `invokeDynamic` 逃生口——那 3 处「动态」
  查实后都是从**封闭静态集合**里选名字，加 `string` 键后门等于亲手造一个守卫扫不到的洞；
  ② 批 7 被 `VENDOR.md` 的 SS-10 铁律强制——panorama 一族 10 个返回类型住 vendored，
  **不能在副本里改出自己的版本**，故只做名字钉死 + 实参把关，类型生成登记 E38。

  **「守卫范围必须等于性质范围」在本功能被用了三次**，每次都是变异先暴露「没红但应该红」：
  不扫 enum（C04c 遗留）· 派生源手写清单漂了（改成自动发现，批 4 当场还本）· 锚点不认非 pub。
  **衍生第二条纪律**：反向自检的阈值不能挂在被优化的那个量上（`hits > 10` 校准于 29 的年代，
  会在迁移快成功时变假红）。

  **三处「早就写好但一直空转」的检查被接上真实源头**：`describeStage` 的 `never` 兜底 ·
  一条被窄类型逼着挂 `@ts-expect-error` 的测试 · `doWrite` 的 `Record<string, unknown>`。

  **顺带如实登记不修**：E35（`MetadataPatch` 的 `Option<Option<String>>` 让「留空恢复默认」
  清不掉标题，真 bug、修它是行为改动）· E38（vendored 类型，三条备选路径）·
  `Config = Record<string, unknown>`（Rust 自己就不知道形状，结构性无法加固）。
