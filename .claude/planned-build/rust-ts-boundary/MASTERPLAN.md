...
- **生成物落点**：`src/generated/`（新目录，加 `// @generated` 头 + 一条禁止手改的注释）。
...
- **门禁基线**（本工作区开工时）：`cargo test --all` **536** · `cargo test -p code-picture-core` **25** ·
  `npm test` **814 / 53 files** · clippy 0 error · tsc 0 · `npm audit --omit=dev --audit-level=high` rc=0 ·
...
- **生成物不进人工审阅的注意力**：`src/generated/` 加 `// @generated`，
