...
- **整体成功标准**：
  ...
  4. 全仓 `import { invoke }` 的直接调用点从 **29 个文件**降到
     **1 个（包装层）+ `tabs.ts`（等授权）+ 3 个动态派发口** —— 即**实际目标是 5 个，不是 1 个**。
     **这一条就是那句「未获授权时如实改写，不假装达成」的改写本身**
     （C04a Phase D 审计 I4：此前这里只写着*应该*改写，标准正文仍是「降到 1」，
     而 `STATUS.md` 已经声称改写做完了——那是假的，现已改）。两个结构性障碍：
     - **`tabs.ts`**（15 处调用点，全仓第二多）—— 「不碰 `tabs.ts`」是在册红线，不松就摘不掉；
     - **动态命令名**（Phase D 审计新发现，此前任何文件都没登记）——
       ...
       这三个文件的命令名在运行时才定，**不加一个动态派发逃生口就结构性摘不掉** `import { invoke }`。
     **度量已机检**：`generated-boundary-guard.vitest.ts` 用等号钉住「直接 import invoke 的生产文件 == 29」，
     C04d 每迁一个模块必须让它降一次。（此前这个数只活在注释里，而且**写错了**——见变更记录 08。）
