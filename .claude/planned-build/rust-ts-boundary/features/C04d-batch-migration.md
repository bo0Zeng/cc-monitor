...
**强度：低风险**（无逻辑改动，纯调用形式替换；等号守卫全程盯着）⇒
主线程变异验收 + 全门禁 + 真机套件，不开多 agent。

### 变异总账（八批，逐批见 §3a-§3k）

**共 23 条变异，全部有效**（每条都先 diff 确认落位、再按判色规则确认判据来源是本次产的）。
按它们证明的性质归类：

| 性质 | 变异 | 结果 |
|---|---|---|
| 包装层的**键名 ↔ invoke 字面量**对拍 | 批 1 A（抄成邻居真命令）· 批 5b B（同形） | `tsc` **rc=0**（管不了）而守卫红并同时点出两个名字 —— **两次都印证它是唯一的牙** |
| **实参形状**由各命令自己的签名把关 | 批 6b A/B（`sftp_mkdir` 多传 `isDir` · `sftp_rename` 写成 `path`）· 批 6a A · 批 6c A（给本地命令传 `origin`）· 批 7 A（少传 `ranges`） | 全部 `tsc` 精确报错。**这一层此前完全没有把关**（裸 `invoke` 的 `InvokeArgs` 照收任何键） |
| Rust 改动 → `tsc` 报错（成功标准 1/2） | 批 2 A′/B′ · 批 3 A · 批 4 A · 批 5b A · 批 5c A | 全部成立。**批 5c A 最值**：让 `describeStage` 里早就写好的 `never` 兜底第一次真对 Rust 有牙 |
| 守卫**范围**是否等于性质范围 | 批 3 B（`conn_count`）· 批 5a A（非 pub 结构） | **两次都先「假绿」→ 暴露范围缺口 → 修范围后才有牙**。见 §5 |
| 等号计数不是装饰 | 每批 3-5 条等号在改动落地时**自动各红一次** | 全部成立 |
| 盲区集被钉死 | 批 6a B · 批 6c B（往 `DYNAMIC_ONLY` 塞名字） | 成立。**空集也钉**——它现在守「不许再出现新的动态命名调用」 |

**一条作废**（C04c 的，不在 C04d 内但同源）：「真删字段」两轮都没编译过，
而第二轮 `tsc` 变红报的是**上一次变异残留的过期生成物** ⇒ 那条教训写进了判色规则第 ③ 步。

### 门禁与真机（八批逐批复核，不是只跑最后一次）

`cargo test --all` 594 → **603**（每批 +N 个 ts-rs 导出测试）· `code-picture-core` **25 未变**
（⇒ vendored 零改动，SS-10 铁律守住）· `npm test` 837/56 → **837/56**（守卫断言在增，测试文件数不变）·
clippy **0 error / 36+44 warning 逐批未变** · tsc 0 · fmt clean · C05 生成物新鲜度 rc=0 ·
eslint **7 项既有基线逐批未变** · **8 套真机 26/44/12/15/13/21/14/7 = 152 条逐批逐个不变**
⇒ **行为逐字节不变**（本工作区每个 commit 的硬判据）· 默认 tmux socket 每批起降 diff 空。

### ★ 我自己造的错，如实汇总（10 次）

**这些没有一次被守卫或 tsc 漏掉**——它们都是被当场拦住的，但值得记下来因为形状会重复：

| # | 错 | 次数 | 拦住它的东西 | 教训 |
|---|---|---|---|---|
| 1 | **多行注释的锚点**写成 `"  /** 往远端…"`，而实际 `/**` 单独一行 | **3**（批 5b/5c/6c） | python `assert` | 锚点写 `"  /**\n   * …"` |
| 2 | 批量替换 helper **默认预期 1 处**，而实际 2-3 处 | **3**（`sftp_stat`/`sftp_upload` · `origin` 在 history.rs · `update_history_metadata`） | 同一条 `assert` | helper 必须带**显式**预期处数 `rep(a,b,n=1)` |
| 3 | 异常在**写盘前**抛出 ⇒ 只有部分文件被改、中间态不一致 | **2**（批 5c/6b） | `git diff --stat` | 补做前先确认哪个文件动了 |
| 4 | **口算**包装层条目数（54 vs 实际 53） | 1（批 5b） | 等号守卫 | 数字一律用守卫报的实测值 |
| 5 | 量 Rust 参数的正则用 `[^,]+?` ⇒ **被元组里的逗号截断**、漏了 `ranges` | 1（批 7） | 包装层签名 + `tsc` | 参数类型里可能含逗号（元组/泛型） |
| 6 | 临时 `grep -P` **按行匹配** ⇒ 漏了跨行的 `push_public_key` 调用点 | 1（批 5c） | 换 import 后 `tsc` 报 `Cannot find name 'invoke'` | 临时 grep 比守卫弱，别当账本 |
| 7 | 包装层实参名/类型**从调用点猜**而非从 Rust 签名量 | 1（批 5a 订正四处；批 5c `pubKeyPath` 其实是 `Option<String>`） | `tsc` | 一律从 Rust 签名量 |
| 8 | **注释里写了未核实的推断**（「未知 scope 会 `undefined.push` 抛」） | 1（批 5b） | 我自己去读实现才发现（`groupByScope` 有显式三值判断、从不抛） | 写进注释前先读实现 |
| 9 | 生成物清单**手工插入**破坏字母序 | 1（批 3） | `toEqual` 逐项对拍 | 用脚本按 `readdirSync().sort()` 重排 |
| 10 | `git commit -m` 里用反引号 ⇒ **被 bash 当命令替换执行**、吃掉一行主语 | 1 | 事后核对消息发现 | commit message 一律走 heredoc |

**外加一次判断错误（不是操作错）**：E36 我初判「要改 `~/.claude/skills/cc-acct-iso/` 的
`ISOLATE_SET`（红线）+ 迁移现有账号」，调研后发现**完全不必**——`EnvOp` IR 里
`export-model` 已经在做每账号 `ANTHROPIC_MODEL`，加两个窄变体即可。已在 `d0bd950` 订正。

## 5. 工程审计结果（Phase E）

### 账本第 7 行（包装层）：四条形状约束**八批全程守住**

① 扁平 `命令名 → 函数` 映射（109 条，无嵌套、无非命令键）· ② 命令名每条目只出现一次
（机检，两次变异证明它是唯一的牙）· ③ 返回类型按 §5 三桶（批 3 细化了一条：**桶② 只管结构体，
原始类型写 `number`/`string`/`boolean`**）· ④ 每加一条就换掉对应模块的裸 `invoke`
（八批下来 `import { invoke }` 的生产文件 29 → **3**，没有一处两条路并存）。

**并且推翻了账本里预留的一条**：约束 ① 原本预留「动态派发逃生口必须是另一个导出」，
批 6a/6b/6c 查实后**根本不需要逃生口**——那 3 处「动态」都是从封闭静态集合里选名字。
**为一件其实是静态的事加 `string` 键后门，等于亲手造一个守卫扫不到的洞。**

### ★ C04d 给本工作区的最大遗产：「守卫范围必须等于性质范围」被用了三次

这条纪律**每次都是靠变异先发现「守卫没红但应该红」**，再回去修范围（不是修判据）：

| # | 缺口 | 发现方式 | 修法 |
|---|---|---|---|
| 1 | 两条通用性质扫描**只扫 `pub struct`、不扫 `pub enum`**，字段正则还要求 `pub ` 前缀 | C04c 加 `messages.rs` 时 `System.duration_ms` 生成出 `bigint` 而守卫沉默 | 锚点加 `enum`、`pub ` 改可选 |
| 2 | 派生源是**手写清单**，批 2/3 在 7 个新文件里加派生却没同步它 | 变异「删 `conn_count` 的 `ts(type)`」→ **全绿** | **取消清单**，改递归全仓自动发现 + 等号钉住文件数。批 4 当场还本（`logging.rs` 一加派生就自动进范围） |
| 3 | 类型头锚点是 `^pub (struct\|enum)`，**非 `pub` 的命令返回类型看不见** | 批 5a 给非 pub 的 `CcStatusResponse` 加不配 `ts(type)` 的 `u64` → 守卫红（修范围后） | 锚点 `pub` 改可选 |

**衍生出的第二条纪律**：**反向自检的阈值不能挂在「被优化的那个量」上**。
「直接 import invoke 的文件数」那条自检原本写 `hits.length > 10`（校准于 29 的年代），
而 C04d 的目标就是把它降到 3 ⇒ 那个下界会在**迁移快成功时变成假红**，
逼人去改自检而不是看性质。改成断言「扫过的 `.ts` 文件数 > 100」——问「遍历有没有工作」，
不问「命中多少」。

### 迁移的意外收益：把三处「早就写好但一直空转」的检查接上了真实源头

1. **批 5c**：`describeStage` 里有 `const _never: never = st;`，注释写着「未来新增
   `ConnectStage` 变体时编译期即报错」——**但手写类型时它守的只是 TS 自己造的联合**。
   换成生成物后，给 Rust 加一个 variant 才真的红。
2. **批 5b**：一条测试**必须挂 `@ts-expect-error`** 才能构造一个**真实会从线上来的**
   未知 `scope` ——**是类型在逼测试撒谎**，而运行时早就正确处理了。换生成物后抑制变多余、已删。
3. **批 6b**：`doWrite(cmd, args)` 的 `Record<string, unknown>` **照收任何键**，
   thunk 化后三个 sftp 写命令的实参才第一次有把关。

**这类「把已有好意图接上真实源头」比新增一条断言更值**——它兑现的是别人早就写下的承诺。

### 顺带发现、如实登记不修的三条

- **E35（真 bug）**：`update_history_metadata` 的 `MetadataPatch` 是 `Option<Option<String>>`，
  `#[serde(default)]` 下 **JSON `null` 到不了 `Some(None)`** ⇒ `null` = 「不改」。
  而前端「留空恢复默认」传的正是 `null` ⇒ **标题清不掉**。修法一行，但**改它是行为改动**。
- **E38（结构性阻塞）**：panorama 一族 10 个返回类型住 vendored，`VENDOR.md` 的 SS-10 铁律
  禁止在副本里改；「先改上游」要动 `code-picture` 仓（在册红线）。三条备选路径已写进 BACKLOG。
- **`Config = Record<string, unknown>`**（批 3）：`load_config`/`save_config` 的 Rust 签名是
  `Result<serde_json::Value, String>` ——**Rust 自己就不知道形状**，这个边界结构性无法加固。

### 对后续的影响

- **两批解锁后各是一次小改动**：`tabs.ts` 15 处（类型多已生成，主要是换调用点）·
  `accounts.ts` 4 处（3 个类型化调用全来自 `accounts.rs`，需等 Z02 定型）。
- **`rust-ts-boundary` 的机制层已完备**：九条等号守卫 + 自动发现的派生源 + 三桶规则 +
  按量纲分开的大整数论证。后续任何人加一个跨边界类型，守卫会在**加派生的那一刻**就要求他表态。

## 6. 签收

- [x] **全部可做批次完成**（1 · 2 · 3 · 4 · 5a · 5b · 5c · 6a · 6b · 6c · 7 = **11 个 commit**）。
      两批**如实标注等授权**、原因互不相同：4b `accounts.ts`（跨工作区冲突协议，等 Z02）·
      8 `tabs.ts`（等 `tabs.ts` 红线授权）
- [x] 通过代码审计（Phase D：23 条变异全部有效 · 门禁八批逐批全绿 · 真机 152 条逐批不变 ·
      **我自己造的 10 次错全部被当场拦住并如实汇总**）
- [x] 通过工程审计（Phase E：账本第 7 行四条约束全程守住且**推翻了预留的第五条** ·
      「守卫范围 = 性质范围」用了三次 · 三处空转的检查被接上真实源头）
- [x] 主计划已据此更新（§0.1 成功标准 4 **两次改写并达成** · §1 状态列 · 变更记录 11）
