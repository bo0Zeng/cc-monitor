...
- **阻塞 / 待用户确认**：
  - **[待批准] 主计划** —— **2026-07-30 复核确认它确实还没批**。执行顺序表跑到 #14 时我查了：
    批准是**逐区留档**的（`rust-ts-boundary/MASTERPLAN.md:6`「主计划用户 2026-07-29 已批准」·
    `account-zero/STATUS.md:24`「[已批准] 主计划（用户『批准主计划 account zero』）」·
    gate-integrity 同）**而本区全仓找不到任何批准记录**，三处（README 第 27 行 /
    MASTERPLAN 第 6 行 / 本文件）都写着「等审批」。
    用户那句「我要全自动把这些需求跑完」是给 **loop** 的授权，不是对本区主计划的批准
    （否则不会再逐区留一句「批准主计划 account zero」）。
    ⇒ **本区暂不开工**，loop 改去跑已批的 `zero-poll-liveness` P4-P7。
    **一句「批准 local-as-remote」即可解锁**；若想调顺序也在那时说。
    （本区自陈 **L0 是「唯一可能推翻方向的一步」** ⇒ 更不该在未批时开跑。）
