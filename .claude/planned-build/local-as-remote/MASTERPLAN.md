...
| L2 | ~~**Windows 本地进 IR**~~ → **平行世界漂移点守卫** | ~~`planLocal` 复活 + PowerShell 渲染器 honour `plan.env`~~ | **⚠ 原方案三条逐条否决**（`features/L2-parallel-worlds-guard.md`）：一条撞 `INVARIANTS §36` **铁律**（逐字禁止给本地渲染器补读 `plan.env`）、两条撞 R07 已审计的决定（本地借 IR 做校验、不消费输出，因为接了也拿不到新东西）、**收益② 是事实错误**（实证 `lib.rs:124` 的启动期清洗早于 `:161` 的 Builder ⇒ 保护一直都在）、收益① 的兜底渲染器**只服务远端**（加臂 = 加死分支）。**✅ 改做 L2 的真意图**：钉住今天真实存在的跨语言漂移点 —— Rust `adapter` ↔ TS `AGENT_PROFILE`（resume flag / 默认启动器 / 嵌套 env 清单）此前只有一句注释、零门禁；新增 `src/agent-profile-parity.vitest.ts` 6 条断言，三条变异成立 | L1 | P1 |
