...
describe("KR101D2 生产路零解析，而解析层三样都还在盘上", () => {
  ...
  it("★ ③ 退役不是删除：解析器 ＋ 它的 vitest ＋ 冻结夹具，三样都得在盘上", () => {
    for (const rel of [
      ...
      "test/account-usage-parse.vitest.ts",
      "test/__fixtures__/usage-capture-2026-07-31.txt",
    ]) {
    ...
    // 而且它真的在跑那个函数（不是一份空壳）。
    const t = readFileSync(resolve(REPO_ROOT, "test/account-usage-parse.vitest.ts"), "utf8");
