 ...
 * - **a2 总量**：**全仓** `test/**\/*.test.ts` 的行首 `test(` 总数 ≥ `TOTAL_FLOOR`。
...
const NODE_SUITES: readonly (readonly [string, string, number])[] = [
  ["test:diff", "test/cards/diff.test.ts", 17],
  ["test:branching", "test/branching.test.ts", 23],
  ["test:api-error", "test/cards/api-error.test.ts", 5],
  ["test:bash", "test/cards/bash.test.ts", 20],
  ["test:remote-health", "test/remote-health.test.ts", 5],
  // F04b +1：`isValidNewTmuxName` 也禁 `=`（别创建一个主路杀不掉的名字）。
  // `K-R96` +1（`KR96D3`：名字可读、sid 一个片段都不进去 + `@ccm_sid` 必须还在）。
  ["test:remote-launch", "test/remote-launch.test.ts", 44],
  ["test:format", "test/format.test.ts", 10],
  ["test:history-cache", "test/views/history-cache.test.ts", 8],
  ["test:history-prefs", "test/views/history-prefs.test.ts", 18],
  ["test:history-actions", "test/views/history-actions.test.ts", 10],
  ["test:usage-pivot", "test/views/usage-pivot.test.ts", 14],
  ["test:pricing", "test/views/pricing.test.ts", 6],
  ["test:session-backend", "test/session-backend.test.ts", 10], // P3s-Y2 +1（新造名字必须过铸造口）
  ["test:panorama-session-files", "test/panorama/session-files.test.ts", 7],
  ["test:launch-dimensions", "test/launch-dimensions.test.ts", 28],
  ["test:launch-render-cli", "test/launch-render-cli.test.ts", 27], // `K-R53` +1（`KR53D3`：「没探出来」与「真的没装」不许给同一句降级理由）
];
...
function allTestTsFiles(): string[] {
  // 〔src/test 分离〕`*.test.ts` 整体搬到了 `test/` ⇒ 人群的根也跟着搬。
  // ⚠ 别改成同时扫 `src/` —— 那会让「生产段里不许有测试」这件事变成**无法被违反**，
  // 判据就白拿一个恒绿。生产树里出现 `*.test.ts` 应当由布局本身排除。
  const root = resolve(REPO_ROOT, "test");
  const out: string[] = [];
