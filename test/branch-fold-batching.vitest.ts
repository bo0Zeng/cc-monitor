...
import type { BranchRecord } from "../src/branching";

const spy = vi.hoisted(() => ({ compute: 0, lastLen: -1 }));

vi.mock("../src/branching", async (importOriginal) => {
  const real = await importOriginal<typeof import("../src/branching")>();
  return {
    ...real,
...
import { BranchFolder } from "../src/branch-fold";
