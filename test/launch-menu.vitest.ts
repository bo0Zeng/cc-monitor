...
import type { Account, AccountsState } from "../src/accounts.ts";
...
import { enumerateAccountModifiers } from "../src/launch-menu.ts";
...
describe("enumerateAccountModifiers", () => {
  ...
  it("恰好 1 个可选账号 → 只含基座逃生口（无需切到唯一账号）", async () => {
    ...
    expect(await enumerateAccountModifiers("host")).toEqual([{ kind: "base", label: "不指定账号（用远端 ~/.claude 那套凭据，不跟随当前账号）" }]);
  });
