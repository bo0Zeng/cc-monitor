...
距 3.3.0 共 145 个 commit。本版最该知道的一件事在最后的「升级须知」。

### 变更 — 会话变灰不再要等十几秒

- **带外杀掉一个 tmux 会话后，对应 tab 变灰从约 16 秒降到约 0.13 秒。**
  判活整个换成内核事件：进程退出走 `pidfd`、tmux server 生死复活走 socket 目录 inotify、
  会话开关走 tmux hook。daemon 里原有的两条轮询（2 秒判活 / 8 秒 `tmux ls`）**都已删除**。
- **进程身份判定不再是启发式**：原先靠「pid 存在 + 启动时间匹配」推断，现在由 `pidfd`
  绑住进程实例本身——**PID 复用导致的误判在机制上不会再发生**。
- 修复：某个 origin 的**最后一个** tmux 会话被杀时（server 随之退出），灰灯此前会
  一直卡到断连才清。

### 新增 — 账号

- **「没选账号」这个状态本身成了一个可选的账号**（账号 0 = 不设 `CLAUDE_CONFIG_DIR`）。
  它现在会出现在账号列表里、可以显式选中、用量探针也支持它；此前它被当成违规状态。
- **`cc-acct-iso` 新增 `isolate` 迁移能力**：把原本共享的配置项转成某个账号私有。
  顺带修掉一个数据丢失风险——`sync` 过去会把隔离项直接删掉。
- **版本钉 + 声明漂移检测**：Claude Code 换了配置文件位置时能当场发现，而不是静默失效。
- **rc 片段一键生成**：切号需要的 shell 片段可以直接复制，**绝不代写你的 `.bashrc`**。
- 修复：已选中的非默认账号，会被默认号回退**静默覆盖**。

### 新增 — 启动与终端

- **统一启动 CLI `ccm`**：终端里原本 4 套各自为政的起会话实现收敛成一条路径，
  app 与终端从此渲染同一个目标。新增 `--detach` / `--tmux-size` / `--model`。
- **切号入口收敛**：chip 是纯全局切换器，按会话切号走 tab 右键。
- **每账号默认模型**。
- **预信任**能力上提进 `ccm`。
- **plan 窗口用量**探测（懒加载，探不到就如实说探不到，不猜）。
- **一键把 `cc-acct-iso` 部署到远端** + 存在性检测。
- 修复：换号重启会**杀错会话**（`tmux -t` 改精确匹配 `=名:`）。
- 修复：容器路径场景下继承账号会丢。

### 新增 — cc-bus

- **只读驾驶舱**：看 bus 上的 agent 状态、派活、收信、图形化 spawn。
- **`settings.json` 钩子只读诊断** + 生成待贴文本（四态如实呈现，**绝无写入路径**）。
- `cc-spawn` 收编进 `ccm`。

### 新增 — 设置与配置

- **配置面审计视图**：一页看清受管工具都动了哪些文件、在哪一端。
- **待贴文本统一组件**。
- **设置面板一块坏不再整页白屏**（分区块隔离；`panel.ts` 实测后决定**不拆**）。

### 新增 — Linux

- **首次产出 Linux 安装包（`.deb`）**，随 release 一起发布。
  CI 也新增了 Linux 构建 job。
  说明：`.deb` 这条打包流水线**是第一次真正执行**，若本次未产出请以 Windows 产物为准。

### 内部（不影响使用，但决定了以后不容易坏）

- Rust↔TS 边界从人工纪律改成生成物（`ts-rs`）+ 门禁；**121 个命令名全部静态可见**。
- 门禁不再可能「零断言报绿」：13 套真机套件加断言数地板（合计 213 条），
  vendored 的 1348 行 bash 进 shellcheck。
- 新增本地/远端**平价对账表**：121 条命令归成 50 项能力，20 项不对称**逐条带理由**，
  新增命令不登记就红。
- daemon 生产段**零定时器**，有护栏钉住。

### ⚠ 升级须知

- **远端 daemon 需要更新到 `p1r-event-liveness`**——上面那条「变灰 16 秒 → 0.13 秒」
  **只有远端 daemon 也换新才生效**。连上时会自动重部署；也可以在设置页每台机器的卡片上手动重装。
- **本机多账号枚举**（读 `~/.claude-accts/accounts.json`）后端已就绪，但**还没有界面入口**，
  这一版你在 UI 上看不到它。
- POSIX 本地「不走 ssh 的远端」传输层已就绪，**前端尚未接线**，同样暂不可见。
...
### 新增
- **灰灯（idle-tmux 第三态）**：远端 Claude 退出但 tmux 会话还在时，tab 显示灰态（介于 live 与 archived 之间）——可 attach 进那个空 shell，也可右键「杀死会话（kill 空 tmux）」（杀掉后 tab 转归档、即可 Resume）。
- **Resume 就地复用（#75/#76）**：远端 archived/idle 会话 Resume 时复用原会话名 `cc-<sid8>`，不再产生 `cc-<sid8>-N` 残留会话；账号注入正确的 `CLAUDE_CONFIG_DIR`（不带账号 pin 时跟随当前工作账号）。
- **多账号 UX 深化**：当前工作账号语义 + follow 模式（起 / Resume 默认跟随当前账号）、tab 账号徽章「有信息才显」+ live/last 头像层次、账号不一致检测（⚠）+ 一键「对齐」、账号切换的可发现性与快捷键。

### 修复
- **#74 / #41 远端终端「↗ 拉前台」可靠性**：远端从 `@ccm_sid` 派生外层窗口标题，让 HWND 重绑不再漏（甲′）；本地 Windows 终端绑定（丙）已在真机验证 bind 层（find/bind/verify）。
- **#75 follow-resume**：账号 pin 现读磁盘、修内存脏读覆盖，并补「用基座账号」逃生口。
- **误杀会话**：`kill_remote_tmux` 补 `cc-*` 白名单，拒杀非本工具的 tmux 会话。
- **刷新竞态 / 多远端缓存**：修会话列表刷新的竞态与多远端场景的缓存串味。

### 移除
- **「清理孤儿会话」功能**：其主要用途（清理 #76 遗留的 `cc-<sid8>-N` 孤儿堆）随 #76 修复已过时，且原实现会把「无 tab 但正在跑 Claude」的会话误列为孤儿（有误杀别的窗口/实例活会话之虞）→ 直接移除。偶发的残留会话改手动清理（SSH + `tmux kill-session`；相关边界见 issue #81）。

## [3.2.0] — 2026-07-24

### 新增
- **多账号:隔离又同步 + 按会话切账号(#68/#69)**:同一台远端上可管理多个 Claude Code 账号——各自 `CLAUDE_CONFIG_DIR`/`.credentials.json`(两号能同时跑、不互踢),而 skills/memory/history/settings/plugins 实时共享(symlink 到同一共享库)。
- **设置「账号」组**:列出远端各账号(名/邮箱/是否已登录),当前账号有 chip/徽章标识。
- **按会话选账号起 / Resume**:起或 Resume 会话时可指定用哪个账号的 config-dir 启动(远端优先)。
- **换号重启的优雅退出**:切到别的账号需重启该会话——先请求会话优雅退出(`Escape` 打断当前轮 → `/exit` → 有界等待 ~10s → 兜底 kill),再用新账号 config-dir 重起。
- **app 内账号部署向导**:设置「账号」组内置向导,分步跑 `cc-acct-iso` 隔离/同步管线(只读状态查询走 daemon;动凭据的 `--apply`/登录/同步经真实终端窗口),并给每个账号一个「登录终端」按钮。

### 修复(bugfix-sweep)
- **远端 ↗ 拉前不及时(#41)**:刚 attach 就点 ↗ 常弹「未绑定窗口」、要等几秒——现在重绑路加了重试、扫描窗口 1.5s→4s(覆盖标题四跳传播),首点更容易直接成功。
- **从 cc-monitor 起的会话 attach 弹「无 @ccm_sid 标记」警告(#72)**:cc-monitor 自己 Resume(tmux) 建的会话现在会打上会话身份标记,之后能精确 attach、不再误报「可能连到别的会话」。
- **单个 `~` 被当删除线(#71)**:`~/foo~/bar` 这类路径不再被划掉;真·删除线 `~~text~~` 照常。
- **多行 / 含标点的 `$$` 公式不渲染或吞公式(#42)**:块公式配对更稳(`文字：$$⏎…`、`…⏎$$。`、CRLF 都渲染),散文里讨论 `$$` 不再把后面的真公式吃掉。
- **用量视图排序混乱(#67)**:「按天」默认按日期排(最近在上);表头可点切换排序列 + 升/降序。
- **fork 会话显示成同名独立 tab、分不清(#63①)**:fork 出来的会话标题加 `↳` 血缘徽标 + tooltip 标出来源会话。
- **历史「来源」每次启动首开只有本地(#46)**:远端来源列表跨启动持久化,首开即显示上次的远端来源(随即刷新)。

> 注:#41/#72 及多账号只读查询都需远端 daemon 为最新版(build `p1q-accounts`,连上自动重部署,或设置页每台机器卡片手动重装)才生效。

## [3.1.1] — 2026-07-22

### 修复
- **远端 tmux 存活检查刷屏远端 sshd 日志**:cc-monitor 原先每 8s 对每台远端主机新建一条 SSH 连接跑 `tmux ls`(会话存活对账)再断开,把远端 sshd 的登录/登出日志刷屏。现改由远端 daemon 在**本机**跑 `tmux ls`、经既有常驻连接上报,对账链路**不再新建任何 SSH 连接**。**需重装该机器的 daemon**(连上时自动重部署新版,或设置页每台机器卡片手动重装)才生效;daemonless 模式的机器沿用其原有活跃窗口判活。

## [3.1.0] — 2026-07-19

### 新增
- **Codex 会话支持(初步)**:除 Claude Code 外,现在也能在**历史**里浏览本机 Codex CLI 的会话——按工作目录分组成项目、点开渲染对话内容(消息 / 推理 / 工具调用)、**用量视图**也计入 Codex 的 token 消耗。**当前范围**:仅历史浏览 + 用量(读 `~/.codex/sessions/`,与读 `~/.claude` 一样**只读、不改你的 Codex 任何文件**);**实时监视 / 运行状态灯 / resume 尚在推进**(Codex 无 pidfile,判活机制待后续)。**Claude 侧行为完全不变、零回归。**
- **代码全景图(Batch15,融合 code-picture)**:把仓库的代码结构可视化嵌进来——点 🗺 对某仓「建立索引」(tree-sitter 解析,每仓手动开启、索引落 cc-monitor 数据目录不碰你的仓)后,看函数/文件/调用关系的全景图,点会话可高亮它改过的节点,支持节点批注 + 文档漂移面板。
- **用量视图 + 上下文 HUD(F88)**:∑ 顶栏入口打开跨会话 token 用量(按 会话/天/项目/模型 四维汇总,多机加总);状态栏 `ctx N%` chip 显活跃会话上下文占用(≥80% 预警)。**只显已花费 token、不显费用/配额**(本地推不出配额)。
- **多 agent 并排监控(F91)**:▦ 顶栏入口打开跨机器只读 mission-control 状态板,一屏按机器分组看所有会话的实时红绿灯 / 运行中 subagent 数 / 上下文% / 未读,点卡片跳到该会话。只读——不派发/不驱动 agent。
- **键盘命令栏(F84)**:`Ctrl+K` 唤起命令面板,输入过滤 + 方向键选 + 回车执行——开各视图 / 窗口操作 / 切到会话。只读命令(不含 resume/kill 等写动作)。
- **MCP 管理(F87)**:设置→集成→MCP,跨 scope(用户/local/项目)展示已配置的 MCP 服务器,可增改删**项目级 `.mcp.json`**(用户/local scope 只读——绝不改你的 `~/.claude.json`)。
- **点进单个 subagent 看完整记录(F77)** · **远端会话「打开工作目录」进 SFTP(F78)** · **历史页右键起新会话 / 搜索卡片直接 resume(F96/F85)** · **顶栏 SFTP 入口(F83)** · **杀死远端 tmux 会话(F79)**。

- **从历史某一轮创建分支(F62)**:历史查看器里,鼠标移到任意一轮(你的提问 / Claude 的回复)卡片上,右上角出现 `⑂`——点它就把「从开头到这一轮为止」复制成一个**全新会话**,你可以从这一轮岔开、走一条不同的对话线。补上 Claude Code 内置 `/branch` 只能从**当前进度**分叉的缺口(想回到几轮前重新展开时用)。**原会话一字节不改**(只新增一个文件),落盘用 Claude 原生 `/branch` 格式(`forkedFrom` 血缘),建出来的分支在历史列表里带「↳ 分叉自」关系显示。建好后弹提示,点提示即可在新终端 `resume` 这条分支接着聊。仅本地会话(远端会话不显示该按钮)。

### 修复
- **远端 resume / attach 有时接进错误的会话(#63,F74)**:同一目录开了多个 Claude、或用 `/branch` 岔开过分支时,「Resume(tmux)」/「Attach」可能接进**另一个**会话(tab 显示的内容和 attach 进去的对话对不上);从灰掉的会话 resume 也可能落进最新的分支而非原会话。现在靠会话真实身份(`@ccm_sid`)精确匹配,灰会话 resume 保证回到**原会话**,attach 精确接到目标会话。**需在设置页每台机器卡片点一次「装 ccm 助手」重装**才生效(远端 `~/.bashrc` 里的助手要升级);未重装则沿用旧的按目录匹配行为、不会变砖。

### 变更
- **设置搬进独立窗口(F82a)**:点 ⚙(或设置快捷键)不再从右侧滑出抽屉,而是**开一个独立的「设置」窗口**(可移到另一屏、和主窗口并排)。设置里改主题 / 行为 / 快捷键**保存后主窗口即时生效**(跨窗口同步);「取消」不影响主窗口。分组暂时不变(重新分成 连接/外观/远端/集成 是下一步 F82b)。
- **全景图代码索引改为「每仓手动开启」(F69,补既定决策 D20)**:打开某仓的代码全景**不再自动扫描**,而是显示「建立索引」按钮,点了才解析(tree-sitter,大仓较慢)。索引文件也从**你的仓库里**(`<仓>/.codepicture/`)迁到 **cc-monitor 自己的数据目录**(`~/.claude/claudecode-frontend/panorama/`)——**不再在你仓库里凭空生成目录**。**升级须知**:老版本在仓库里建的 `.codepicture/` 会成为无用残留(纯缓存,可手动删),新版本不再读它,重新点「建立索引」即可(已索引仓改了代码会像以前一样自动保持新鲜)。

## [3.0.0] — 2026-07-10

### 新增
- **预览远端 tmux 画面(F60)**:会话 tab 右键——若该会话正跑在远端 tmux 里(与「Attach」同一会话),多一个「预览画面」——弹窗只读展示远端那块 tmux pane 当前的屏幕文本(`tmux capture-pane` 抓一次快照,不接管终端、不影响该会话)。想瞄一眼远端 Claude 现在长啥样、又不想真的 attach 过去时用。弹窗里可「重新抓取」刷新;远端没装 tmux 或会话刚结束会优雅提示。
- **daemonless 降级读取(F59)**:远端主机设置卡新增「daemonless 降级读取(无需 daemon)」勾选——勾上后该台**不部署也不连 daemon**,改用纯 SSH 命令(`find`+`tail`)轮询读会话 jsonl。适合装不了 daemon 的主机(异构架构 / 无写权限 / BSD·macOS)。**能力子集(降级,如实提示)**:无后台(⚙)会话跟踪、无运行状态灯、无拥塞信号、仅显示最近 30 分钟有写入的活跃会话(空闲久的会话 Tab 暂时收起、再有输出即回来);会话内容照常可读。开启后会弹一次「远端降级模式」提示说明。需重启 monitor 生效。
- **本地端口转发管理台(F58)**:远端设置新增「端口转发…」——一个 `-L` 本地转发的图形管理台。填「本地端口 → 远端 host:端口」+ 选一台已配置的主机,即可把远端机(或它能访问的内网)上的服务映射到本机 `127.0.0.1:<本地端口>`(经该主机的 SSH 连接隧道,自动继承多地址竞速与跳板)。列表显示每条转发的状态灯与连接数,可随时停止。常用于本机直连远端数据库/内网 Web 服务等。(当前为即席转发,App 重启后需重新添加。)
- **从 ~/.ssh/config 批量导入 + 智能聚合(F57)**:远端设置新增「批量导入…」——一次导入 `~/.ssh/config` 里所有主机。**智能聚合**:同密钥 + 同用户 + 同基名前缀的多个别名(如 `aya-lan`/`aya-wan`)判为**同一台机的多个地址**,聚合成一台多地址主机(自动填「备用地址」走 happy-eyeballs 竞速),而非建一堆重复主机;`ProxyJump` 自动填「跳板」。导入前有**预览**,可勾选、改名、或把聚合错的一组「拆分」成独立主机。(原「选别名导入单台」仍在。)
- **跳板 ProxyJump(F56)**:远端主机设置卡新增「跳板」字段——填另一台已配置主机的名称,即可通过它作跳板连接内网/堡垒机后的机器(监控数据源走 SSH 隧道,resume/attach 拉起的终端走 ssh `-J`)。跳板配置缺失或连不上时**报错而非静默直连**目标(fail-closed);跳板指向自己(环)也拒绝。当前单跳(跳板的跳板暂不支持)。
- **会话工具卡文件路径 → SFTP 定位(F54)**:远端会话里的文件类工具卡(读/写/编辑文件等)展开后,顶部出现「📂 在 SFTP 打开:<路径>」——一键打开 SFTP 面板定位到该远端文件所在目录并高亮它,就地预览/下载。本地会话不显示(本机路径不走 SFTP)。主机配置被删/改名时优雅提示。
