...
### 新增
- **远端 SFTP 文件面板(F47+F48)**:设置里每台远端主机新增「文件」按钮,打开一个文件面板——浏览远端目录(面包屑导航、目录在前排序、大小显示)、上传/下载(系统文件选择器 + 进度条 + 取消)、拖本地文件进面板即上传、新建目录/重命名/删除(删除二次确认)、目录 Pin 书签、「在此打开终端」一键在该目录起 SSH 终端。走独立连接池(与会话数据源流分离,不影响监控);防误伤守卫拒写 Claude 会话文件(那些用历史浏览器管)。含非 UTF-8 文件名只读保护。
