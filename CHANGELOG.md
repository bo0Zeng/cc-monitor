...
**这是一个只修 bug 的补丁版**，修的是 `v3.8.0` 上两条**真机实测确认过**的缺陷。
🔴 **先读这一句**：`v3.8.0` 装上之后**第一条命令就敲不出来** —— Linux 与 Windows
**两台干净机上同形**。你要是在用 v3.8.0，这一版就是为你发的。

### 修复

- **装上了、能跑，但用户一个名字都敲不到 —— `ccm` 不在 PATH 上**

  症状：装完 `v3.8.0`，`ccm` 真的在盘上（Windows 落在 `%USERPROFILE%\.cc-monitor\bin\`），
  但在终端里敲 `ccm` 是「找不到命令」，敲 `cc` / `cct` 退出码 `127`。
  真机上**三条安装路 × 三种 shell × 三个名字，一个都用不了**。
  根因：**全仓零个 Windows PATH 写入点** —— 安装器把二进制放进那个目录之后，
  没有任何一处把那个目录加到 PATH 上。
  回归性：**不是 v3.8.0 的回归，是从来就没有过这个机制** —— 此前没人在干净机上从头装过一遍。
  修法：
  - 设置面板新增一格「**这台机器的 ccm 命令（用户级 PATH）**」：显示现在的状态
    （现算、不缓存），一个按钮「**加到用户级 PATH**」、一个按钮「**从用户级 PATH 撤掉**」。
    只改**你自己的用户级 PATH**：不需要管理员、不碰系统 PATH、不碰别的用户。
    **撤只摘掉我们自己那一格**（按整格比对，不用子串匹配，你 PATH 上别的东西一个字节都不动）。
    ⚠ **改完要重开终端才生效。**
  - 不想点按钮的人：那一格底下**原样显示按钮会跑的那段命令**，复制进 PowerShell
    自己跑一次，与点按钮是**逐字同一份字节**。
...
- **CJK 代码页的 Windows 上，「终端集成」一直是坏的，而且一声不吭**

  症状：系统 ANSI 代码页是 GBK（`ACP=936`，简体中文 Windows 的默认）的机器上，
  装完「终端集成」之后窗口标题绑定那一套根本没生效，而 PowerShell **一条报错都不报**。
  根因：写进 profile 的那段 PowerShell 是**不带 BOM 的 UTF-8**，而 Windows PowerShell 5.1
  按系统 ANSI 代码页去解它 ⇒ 一行 CJK 注释的字节被解成半个字符，**把它下面那一行整个吃掉**。
  现打：模板里 `$ccmDir` / `$deadline` / `$oldTitle` 三处赋值都被吞掉，而 `parse-errors=0`
  —— 解析器认为这份文件完全正常，所以没有任何报错能让你发现。
  回归性：`v3.8.0` 上就有；**在 GBK 机器上这套功能从来没有真正工作过**。
  修法：写 PowerShell 方言的那一份改成**带 BOM** 落盘。只作用于 PowerShell 那一支 ——
  不能加进通用的写文件那一层，它还写 `.mcp.json` 与一份 `.sh`。

### 受影响用户的应急步骤

**装过 v3.8.0（或更早）、发现 `ccm` / `cc` / `cct` 敲不出来**：装上本版，打开设置里
那一格「这台机器的 ccm 命令（用户级 PATH）」，点「加到用户级 PATH」，然后**重开一个终端**。
不想点按钮就复制那一格底下显示的那段命令自己跑，两者逐字等价。

**你是 GBK（中文）Windows 且装过「终端集成」**：v3.8.0 写进你 profile 的那一段是坏的
（不带 BOM）。装上本版后**重装一次终端集成**，新写进去的那份才带 BOM ——
旧的那段在 BEGIN/END 围栏里，重装会原地换掉它，围栏外你自己写的一个字节都不动。

### ⚠ 本版自己的诚实边界 —— 两条，别读宽

1. 🔴 **上面两条是「修了」，不是「验过了」。**
   证实这两条缺陷的那轮真机测试（干净 Linux 容器 ＋ win11 VM）测的是 **`v3.8.0` 的发版产物**；
   那轮的基线比 `v3.8.0` 多 15 个提交，而 PATH 与 BOM 这两条修复就在那 15 个里
   ⇒ **修完之后的那一版没有上过真机**，而本版发出去的正是它。
2. 🔴 **已知问题，本版没修**：`cc` / `cct` 是那份别名块自己定义的 **shell 函数**，
   所以 `command -v cc` **恒为真**，而真敲下去退出码是 `127`。
   ⇒ **任何拿 `command -v` 去探「装好没有」的诊断，都会对 `cc` / `cct` 报绿，
   而它们实际是坏的。** 要判它到底能不能用，**真敲一下看退出码**，别去探名字。
   这一条已单独立项跟进，**本版不含它的修复**。
...
本版**最该先读的是下面第一节** —— 六条会改变已有行为的；哪几样要重新部署，末尾「升级须知」说清。

主体是三块：**后端收成一个**（`ccm` 不再是一套 bash 脚本、`daemonless` 与 SSH 回落删净、
后端二进制自己带身份）、**「不知道」不再被写成 0**（历史那三个数 · 账号那一格）、
以及**本机与远端走同一条路**（读历史 · 搜索 · 起会话 · cc-bus · 用量探针各自收成一份实现）。

### 六条会改变已有行为的

本版是 `3.8.0` 而不是 `4.0.0`，所以这六条单独摆在最前面：
**它们会让你原来的某些用法直接失效**，不是「优化了一下」。

- **`daemonless`（「不装后端也能看」那个模式）删了。**
  以前一台远端可以在机器卡片里勾 `daemonless`，不装后端、靠 SSH 轮询看会话。
  本版那一档整个没有了 —— **前端就是去调后端，没有「没有后端」这回事**。
  - 你看得见的：机器卡片里那个输入格没有了；**旧配置里那个开关还是 `true` 的机器，
    会在「还差什么」那一栏收到一条有名字的告知**（不是静默吞掉），下一次保存把那个旧键写没。
  - 那台机器要继续用 ⇒ 给它装后端：设置 → 机器 → 那台机器 → 组件。

- **`cch` 这个别名删了；`ccm` 默认也不再替你猜目录。**
  - `cch` 没有了（`cc` / `cct` 留着）。反过来说：`cch` 从「已被我们占用」的名单里退出了，
    **这个名字从此你可以自己拿去用**。
  - `ccm` 不带路径时的默认行为收成**恒等** —— 就在你当前目录起。删掉的两档猜法是：
    ① 在 `$HOME` 里跑就跳 `$CCM_WORKSPACE`；② 往上找得到 `.git` 就跳那个仓的**父目录**。
    **靠过这两档的人，从本版起要自己 `cd` 过去。**
  - `$CCM_WORKSPACE` 从此没有任何消费者，一并删掉。
  - `resume` / `attach` 那两条路的行为**一个字没动**（它们本来就不走这一档）。

- **`ccm` 变成后端的原生命令，`shared/ccm` 那套 1592 行 bash 删掉了。**
  远端 `~/.local/bin/ccm` 现在是个三行转发壳（`exec <后端二进制> ccm "$@"`）；
  argv 解析 · 计划 · `--print` · 执行全部住进后端。
  - 你看得见的：`ccm --print` 与真跑读的是**同一个计划**，结构上不可能分叉；
    以前**静默**的两种情况现在会明说 —— 找不到 `cc-bus`（那条会话不进总线名单）、
    `cc-spawned-record` 没有执行位（那条会话不进 spawn 台账）。
  - **要重新部署才生效**，见末尾「升级须知」。

- **tmux 会话名从 `<sid8>-cc` 改成 `<项目名>-cc`。**
  以前叫 `a1b2c3d4-cc` 这种，现在叫 `my-proj-cc`；撞名是 `my-proj-cc-2`，分叉是 `my-proj-fork-cc`。
  - 你看得见的：`tmux ls` 里的名字变了 —— **按旧名字写死的脚本 / 快捷键要改**。
  - 会话的 `sid` 一格没动（它一直住在 tmux 的 `@ccm_sid` 上，从来不是名字的一部分）。

- **后端二进制自己带身份 ⇒ 每台已经装过后端的远端，连上之后会被判旧并自动重装一次。**
  以前「手上这份二进制是不是我们以为的那一份」只能问它旁边那个 `.build_id` 标记文件 ——
  文件被删、被截成 0 字节、或路径被改到同目录另一个文件名时，那个标记照旧匹配。
  本版把身份烧进二进制自己的字节里，并让 `--ccm-probe` 多吐一行 `build=`。
  - 你看得见的：本版第一次连上每台远端时会多一次自动重装（几秒），之后照旧。

- **桌面侧那两条 SSH 回落（送键 · 杀会话）删净了。**
  以前后端通道不在时，这两个动作会**悄悄换成直接开一条 SSH 去做**，看上去成功了。
  本版只剩后端一条路：**通道不在就明确失败，并告诉你下一步做什么**（本机与远端两句话不同）。
  - 你看得见的：以前「偶尔慢一点但总能成」的操作，现在会直接报错。
    那不是退步 —— 是把一条没人看得见、也不在后端账上的第二条路关掉了。

### 新增 — 用户看得见的

- **列出你在这个会话里说过的每一句，点一下跳过去。** 顶栏多一个「我说过的 N 句」开关，
  展开是一条可点的清单。**历史查看器与实时窗口都有。**
- **加了账号，就能一键拿到对应的命令。** 本机账号清单底下多一块「按账号生成命令」——
  以前加完账号还得自己去改 `.bashrc`。（远端那条路只给指路话：别名是给本机 shell 用的。）
- **裸 `monitor.exe` 自己带上本机后端。** 以前只有装出来的那份有本机后端进程，
  直接双击下载来的 exe 是 0 个。本版按平台内嵌后端字节，需要时自己落盘；
  释放失败会**响亮失败**，不会跟「这份产物压根没带后端」混在一起。
- **本机 POSIX（Linux）第一次摸得到安装入口** —— 装法与远端共用同一份实现。
  另外，手写在 `~/.bashrc` 里的裸 `ccm` 行，本版**查得出来并逐行列给你看**（行号 + 原文），
  但**产品自己不动手删**：要不要清理你说了算。
- **本机具名账号进得了后端那条路**（分叉会话同理）。
- **历史页与分叉起的会话，也进具名 tmux 容器了。** 以前这两条路各漏传了一个参数，
  起出来的会话不在容器里 ⇒ 右键菜单里「杀死会话」「就地 resume」对它一条都给不出来。
- **点用量直接看抓到的那一屏原文**（等宽、保留空白），不再经过一层解析；
  空屏也算抓到了。解析层功能退役、代码留在盘上。
- **搜索的口径收进一份共享实现**，本机与远端从此是同一套；顺带修好两条真缺陷 ——
  预算顺序、以及远端截断不说话。
- **SFTP 面板那一层的写有了自己的守卫**（此前只读守卫认不出 SFTP 写）。
- **远端项目列表带上会话 sid 清单**，历史那几个数从此有真相源可对。

### 修复

- **「探测出错」不再被显示成「没装」** —— 这两件事从本版起分得开。
- **历史项目行里的「收藏数 / 隐藏数 / 有没有在跑」**：远端以前一律报 `0 / 0 / false`，
  而没有任何东西说得出这是「查过了是 0」还是「不知道」。本版这三格转成三态。
- **账号那一格的「未表态（继承）」**：省略 `--account` 是一个**有名字的状态**，
  不再被压成某个具体账号。
- **找不到 `cc-bus` / `cc-spawned-record` 不可执行**：以前是「要了、没做、也不说」，现在会说。

### 改进 — 本机与远端收成同一条路

- 读历史 · 搜索 · 起会话 · cc-bus 发消息 · 用量探针编排 · 按 sid 找会话文件 ——
  这几条以前本机一套、远端一套，本版各自收成**一份实现**。
  用户这一侧的差别是：**本机与远端从此不会给出不一样的答案。**
- 后端补上几条只读原语（抓一屏 · 带看门狗的一次性会话 · 总线名单一次回全），
  抓不到时说得出真实原因。
- 「这些工具要你自己装」（`claude` / `tmux` / `git` / `ssh`）立成一档并**真去查**：
  「查了、确认没有」与「查不动」两格从此分得开。

### 项目管理

- 出货门禁从 11 格扩到 **16 格**：多了 daemon 那棵树的格式检查、量具还原那一跳的检查、
  非 test 构建里的死代码棘轮、hooks 可执行性，以及 `tsc --noEmit` ——
  最后这一格落地当天就现打逮到 6 条类型错，**而在它之前，本地与云端两条路同时断着**。
- 全仓散文里不再说 `daemon`，统一叫「后端」；`evidence/` 与本文件里的历史读数
  **刻意不在射程里** —— 它们是墓碑，改了就是伪造一次读数。

### 升级须知

装新版 app 之外，**这两样要重新部署才生效**：

1. **远端后端** —— 本版身份烧进二进制自己（`BUILD_ID` 到 `p2f-build-stamp`），
   连上远端后会**自动判旧并重装一次**。上面「`ccm` 变成原生命令」「送键 / 杀会话只剩后端一条路」
   「搜索口径」「只读原语」几条都在它里面。
   手工入口：设置 → 机器 → 那台机器 → 组件 → 装后端。
2. **远端 `ccm`** —— 它现在只是个三行转发壳，指向后端二进制。
   **旧机器上那套 bash `ccm` 不会自己消失**，不重装的话你用的还是旧行为。
   同一处入口：设置 → 机器 → 那台机器 → 组件。

**另外两样产品刻意不替你动，要自己来**：

- `~/.bashrc` / `~/.zshrc` 里手写过 `cch` 别名的 —— 本版起它指不到东西了，自己删。
- 按旧会话名（`<sid8>-cc`）写死的脚本 / tmux 快捷键 —— 改成 `<项目名>-cc`。

---

## [3.7.0] — 2026-09-10

距 3.6.0 共 **1130 个 commit / 773 个文件**。本版最该先读的是**「三条会改变已有行为的」**
那一节 —— 其中两条不重新部署就会踩坑，具体怎么部署见最后的「升级须知」。

主体是三块：**多账号 + 第三方 API 中转**（第一次能给每个账号配自己的端点与 key）、
**本机也被当成一台机器**（新装完、一台远端都没配的人第一次打开也有东西可用）、
以及 **`ccm` 与后端之间那条线收敛成一条**（起会话不再有第二条退路）。

### 三条会改变已有行为的

- **`ccm --tmux` 起会话统一走后端；后端不可达就失败，退出码 `4`。**
  以前后端不可达时 `ccm` 会**退回本机 tmux 直起**、而且 `rc=0` —— 看起来成功了，
  但那条会话不在后端的账上。本版把那条退路从 exec 路上删掉了，
  exec 路只剩一条：后端。唯一的失败面是「后端不可达」，退出码 `4`。
  - `--tmux --print` **仍然吐本机那条编排配方**（只打印、不执行）——
    那一格是「未裁」，不是「留了退路」。
  - **要重新部署 `ccm` 才有**（`CCM_VERSION` 由 `3` 升到 `4`；能力列表一个字没变，
    变的是行为）。一台远端上装的是 3 还是 4，决定了「daemon 没部署时那台机器起不起得来会话」。
- **daemon 的构建标识变了**（`p1u-fork-session` → `p2e-dial`）
  ⇒ **每台已经装过 daemon 的远端，连上之后会被判旧并自动重装一次。**
  同时部署决策改了：以前只读二进制同目录那个 `.build_id` 标记，于是
  「版本对不对」与「那个文件在不在」被压成同一个读数 —— 二进制被删掉 / 被截成 0 字节 /
  `daemonPath` 被改到同目录另一个文件名时，标记照旧匹配 ⇒ 判「跳过」⇒
  新字节**永远不会上传**，而 exec 走的是一个不存在的路径。本版把落点单独取样一次：
  **明确没文件 / 是 0 字节 ⇒ 越过版本门控重传**；「在」与「问不出来」一律退回原来的版本门控
  （旧那套 stale 防御一个字节没动）。
- **daemon 起的插件不再继承 daemon 整份环境。**
  daemon 常驻监听口的地址与令牌就在它自己的进程环境里，而起插件那一步从不清环境
  ⇒ 任何被 daemon 起的插件读一读自己的环境，就能回连宿主发全部基础命令 ——
  无协议、无权限模型、无审计。实测：假插件报回的 `inherited-path` 与宿主 `PATH` **逐字相同**。
  修法是先清空环境、再按一张**具名白名单**喂；`cc-bus` 那条适配层改成**显式交办**自己那一族键。
  > **如果你有插件 / skill 依赖「从 daemon 继承来的某个环境变量」，本版起它拿不到了。**
  > 这一刀关的是暗路；「插件怎么正当地调基础命令」是另一件事，本版只写了题面。

### 新增 — 多账号 + 第三方 API

- **HTTP 中转（Rust 重写，只听回环）**：端点与 key 都归中转的路由表，中转在转发时替换
  `Authorization` 头，**客户端一个凭据都不配**。路由键是 `/s/<agent>/<account>/<key>/<真路径>`，
  三段都是不透明串 —— 中转不认识任何 agent 叫什么，也不解释 `<key>` 是什么。
- **每个账号自己的端点与 key**。设置 → 机器 → 账号那一组里多了「中转 API key」一块，
  界面上说得出「这个号在中转表里有没有它那一行」：**没有那一行的号，它的会话走官方直连**。
  （那句话是**后端算的**，前端不自己推账号 id、也不读那份凭据文件 —— 两边各算一遍迟早会漂开，
  漂开那天的症状是「设置里说走中转、起会话时没走」，而两边看起来都没错。）
- **手编那份 JSON 是一等公民**：凭据文件人能手编，程序回写不吃掉手写的；
  文件出生即窄（POSIX `0o600` / Windows DACL）。
- **每行可以有自己的鉴权方式**（订阅 / api-key / 无鉴权），`logged_in` 按 kind 分别判。
  「无鉴权」与「这一行没配 key」**不是一回事**：后者是原样转发客户端那份（订阅那一档要的），
  前者连那份也不转发。
- **上游 URL 的路径前缀真的会到达请求行**（`https://<host>/v1` 这一形）。
  以前那一段被静默丢掉，`<host>/v1` 是**碰巧**对的；现在写错路径会得到一个看得见的 404。
- **本机推理服务接得上**：明文 `http` 上游**只在回环上放行**，非回环的明文一律拒绝并出声
  （出声照发等于「我告诉过你了」，而代价由用户付）。
  `localhost` 算回环靠的是解析器把它解到回环 —— 那是约定不是保证，别的名字一律说「不是」。
- **凭据不泄漏**：明文出口恰好两个（换鉴权头 / 落盘），各自具名、调用点钉死、三棵树交叉对拍；
  记日志走白名单不是黑名单。
  > **诚实边界（原样登记）**：Windows 上的凭据文件权限只到「编得过」，**没跑过**；
  > 远端真机权限没验；金丝雀测试里 panic 那一格没构造出真 panic、502 那条错误支没测。
- **能混不同供应商，但有一条硬边界**：中转是字节级透明代理（查表 · 换鉴权头 · 剥路径前缀 ·
  原样转发），**中间没有任何一层认识 Anthropic 还是 OpenAI 格式**。
  ⇒ 各账号指向完全不同的供应商这一格今天成立，**前提是那些上游都接受 Anthropic 格式**
  （第三方大多提供 Anthropic 兼容端点）；**只有 OpenAI 格式的供应商，今天配上去会失败**。

### 新增 — 本机也是一台机器

- **没有配任何远端的机器，设置面板也列得出自己的账号。**
  以前账号那一节在没有远端时直接早返回，印一句「没有已配置的远端。账号功能在远端 Linux 上
  —— 先在「连接」组配一台远端。」—— **而那台机上本来就有账号**。
- **本机账号改走本机后端**：谁跑 claude，谁的后端管它的账号。
  顺带修掉一处 Windows 恒空：后端那份路径检查第一条是「必须斜杠开头」，
  于是 `C:\Users\…` 恒被判成不安全、列表恒空。判据改成落在**性质**上
  （shell 元字符与视觉欺骗字符是平台无关的安全性质；「是绝对路径」各平台各写各的）。
- **「还差什么」那张清单，本机也进得去了。**
  那张清单的入参本来就含本机，但本机那两格的**唯一写点第一行就早返回**
  ⇒ 每台机器恒定产出至少两条「没测过」⇒ 「全绿就整块不出现」那一支**在结构上走不到**。
  一张既补不齐也消不掉的「还差什么」，比不做这个功能更糟。
- **第一次打开说得出下一步**：那张清单以前只在设置面板深处渲染 ——
  一个刚装完、还没打开过设置的人一个字都看不到。现在主窗口上有一条非模态的指路，见过即不再。

### 新增 — 标签页

- **灰 tab 从主栏挪进归档抽屉**，要用时捞回来。
  一条不能违反的规则：**当前 tab 永远留在主栏** —— 它可能在你正看着它的时候变成已归档
  （会话跑完就归档），无条件分流会让它当场从主栏消失、掉进一个折叠起来的抽屉，
  界面看起来空了而内容还在，那比「灰着但还在」坏得多。
- **栏内纵向拖动排序**，撕离成窗口那条手势一个像素不动 —— 重排走纵轴、撕离走横轴，
  两个动作不抢同一根轴。拖一个交互 tab 时，挂在它下面的后台子串跟着一起走。
- **标签页集合**：纯手动建、手动命名，存进 `config.json`（**不是** localStorage ——
  后者住 WebView2 用户数据目录，跟 cache / cookies 一起被清）。
  解散集合**只去掉分组，一个会话都不关** —— 集合是个视图，不是容器。

### 新增 — 会话与终端

- **打断 Claude 时说的话，终于显示得出来。**
  症状：Claude 还没输出完时敲进去的消息会排队，排完给 Claude 之后 cc-monitor 显示不出来。
  根因不是解析 bug，**是数据里没有**：CC 的 `queue-operation` 有两条命 ——
  `dequeue`（排队消息独立成一轮）会写 `type:"user"` 记录，
  `remove`（被插进正在跑的那一轮）**一条都不写**。实测一个会话因此丢了用户 **16 条**真实输入，
  全是打断时说的。判定规则比预想干净：remove 零产出 user 记录，16/16 无例外。
- **起会话就是起会话，不要 or。** `create-or-attach` 整个消失，模式变成
  「新建 / 送进已有 / 只 attach」三选一。那个 `or` 其实是一段 shell 里的三个符号：
  会话已存在时 `new-session` 的报错被吞掉、`&&` 短路把载荷跳过、而 `; tmux attach` 无条件执行
  ⇒ **「会话已存在时静默接回，且不告诉你载荷没送」**。
- **`ccm` 的显式会话名撞了要响亮失败**：退出码 `3` + 说清是哪个名字，**绝不 attach**。
  以前是幂等建闸，后果是「用户被静默接进别人的会话，而载荷一个字都没送」。
  实测（先占住名字再调）：`rc=3` · 说清是哪个名字 · 占用者一个字没动。
  不影响缺省名那一支（上游已无条件避让），也不影响 `ccm attach <名>`。
- **本机会话补齐了三样**：本机就地 resume（复用空 tmux，不 new-session）·
  本机 tab 也有「杀死会话」（按 `@ccm_sid` 认，**不按名字猜** —— 拿命名巧合当身份是同一类错）·
  attach 只认两种终端（Linux bash / Windows PowerShell + Windows Terminal），本机与远端共用一条路。
  POSIX 上不开终端窗口时如实说「本机不开终端窗口，命令已复制」并给出命令 —— **这不是失败**。
- **远端会话的 subagent 也能展开了**（daemon 只列候选，挑选留本侧）。
  > **诚实边界**：远端那条读法继承了 30 秒超时，而本机那条没有。
  > subagent 通常短命、文件很小，但一个长跑的 subagent 可能撞上那 30 秒。如实登记，没顺手重构。
- **resume 命令从一条自由文本变成预设列表**。预设是**放大器** ——
  手打错一次只错一次，存成预设是把错误固化成一键 ⇒ 点远端预设走同一条启动器诊断。
- **本机也能查「某会话跑在哪个账号下」**：扫本机 pidfile → pid → 进程环境，
  **只抠两个写死的键**（`CLAUDE_CONFIG_DIR` 与 `CCM_LAUNCH_ID`），绝不回传整个环境快照 —— 那里面有用户全部的密钥类环境变量。
  键名**不是参数**，所以这条查询不是「任意环境变量读」原语。
  分叉一个**正跑着的**本机会话不再白弹一次追问小窗。Linux 有、Windows 没有，界面说清楚。
- **新会话的 sid 从起会话那一刻就回填得上**：起会话方铸出来的身份 token 交给调用方，
  用它回填新会话的 sid（事件驱动，零新增调度点）。此前 5 处起会话方**没有一处**在起新会话时知道 sid。
  身份从数据通道读回来（不写盘、不轮询、与 tmux 无关，也与窗口标题无关），并带冒名检查；
  「换号重启」的成因判断第一次真的拿它做决定。

### 修复 — 灰灯与判活（issue #60 闭合）

- **带外杀掉一个会话之后 tab 不变灰。** 根因：daemon 起得比 tmux server 早就**永远失明**
  （零 server 起步时它没挂上任何耳朵）。修好后灰灯出来了。
- **永久灰点、归档不了。** 根因：先 retire 后更新账本，顺序反了。改成先摘账再 retire。
- 全链台架 **3 过 / 0 败** —— 这条 issue 立件以来第一次全绿。
  排除路上另有六条**台架自己的**成因逐条修掉（陈旧 pidfile / 目录被换 inode /
  被测二进制被静默换掉 / 探针不带 argv / 标记文件名 / 门禁人群）。

### 修复 — 静默失效那一族

- **非 UTF-8 客户端下，tmux 的 TAB 分隔输出整条被改写。**
  实测（容器内 tmux 3.4，`od -c` 读字节）：`#{pid}\t#{socket_path}` 打出来的是
  `9_/tmp/tmux-1000/pmcheck` —— **TAB 变成了 `_`**。
  ⇒ 按 `\t` 拆分不出两段 ⇒ 落进一条什么都不打的臂 ⇒
  **pid watcher 与 tmux hook 一次都不调、socket 目录的 inotify 一次都不装**
  —— 「零轮询判活」那一整套有几条根本没装上，而且**是静默的**。
  六处同病，按调用点形态分治装上 UTF-8 口径（脚本串那一形挂 env；argv 直传与跨 SSH 那两形
  用 `-u` 旗，且旗必须排在子命令之前）。
  > **不是 tmux 3.4 独有** —— 3.6a 一模一样地犯。原来的题名说「3.4 上那一处」，两半都错。
- **`send-keys` 那道跨 SSH 的门，在通道脏时放行。**
  远端用 TAB 吐两个字段、本地再 `cut` 拆开；而 `cut` 在**没有分隔符**时把整行原样吐出来
  （实测 GNU coreutils 9.4）⇒ 取出来的 sid 是整行 ⇒ 非空 ⇒ 那条 guard 恒真
  ⇒ **远端核验那一半被静默绕过**。隔壁 kill 那条的拒绝是**碰巧**（它多看了一个字段）。
  改成 shell 原生拆：不引入任何新的对端假设（那条命令串本来就要求对端是 POSIX sh），
  反而去掉了对 `cut` 这个外部工具的依赖；拆不出就 fail-closed。
- **关掉历史视图之后，那条每秒问所有远端的链终于停了。**
  全文搜索在索引中时会挂一个 1 秒定时器重跑自己，而关闭视图时没人拨它的代际号
  ⇒ 用户关掉了面板，后台还在替他每秒问所有远端（每轮对每台远端各一条 SSH），
  并把结果写进已经 detach 的 DOM。
- **取不到账号时不再报成「账号 0」** —— 一个 `None` 装了三件事，而它在生产路上。
- **起后端那一跳，分得开「这台机器起不来」与「这一刻恰好撞上了」**：
  以前 `spawn()` 的错误一个桶装所有失败，而且当场放弃、不再试。

### 改进 — 后端常驻与自愈

- **本机后端真脱离了。** 以前那个「独立进程」是普通的管道子进程，宿主一退读端就断，
  它在 **153 毫秒**内自己 broken-pipe 退出 ⇒ 「不主动 kill」的真实语义是「让它自己死」，
  不是「后台常驻」。本版：真脱离 + 监听口 + 起时认已有实例。
- **界面如实说「无人监护」。** 勾掉「monitor 退出时结束它」并且真脱离了的那一档，
  界面逐字说「monitor 退出后它继续跑，无人监护：崩了不会自动重起；下次开 monitor 会接上它，
  接不上才起一个新的」。这一区的用户可见文案**一个都不许**出现
  「后台常驻 / 继续运行 / 一直跑 / 保持运行」——做成机检，不靠评审。
- **后端死了会记一笔账**：三维证据（退出状态 · 有没有说过话 · 读端怎么结束的）
  分得开四件事 —— 从来没起来 / 被拒了 / 崩了 / 读坏了。账落 monitor 的滚动日志，
  界面另起一行画读数，三档如实呈现（含「没观测到」这一档）。非 Linux 上也说得出口。
  > **诚实边界**：立件时去数「它到底崩过没有」的受益者，现打结果是 **0** ——
  > 这一块是**预防，不是治疗**。远端那条路的退出状态在别人机器上、拿不到，本版不接。

### 新增 — cc-bus、插件与其它面

- **收件箱**：命令面板（`Ctrl+K`）开一个 overlay，一个文本框 + 保存。
  可写的位置由后端一处围栏说了算，前端不自己判路径。
- **`cc-broadcast` / `cc-kill` 两条命令进界面**，并把危险程度做进 UI：
  广播的确认框**带数字**（一个不带数字的「确定吗」等于没问 —— 实测本机名册 86 行）；
  `cc-kill` 两步确认，第二步逐字回显 id（一屏几十个 agent，别让人杀错那一个）。
- **cc-bus 可以装到本机**（只读铁律的一条例外，四个配套逐条落地：用户显式动作 ·
  独立白名单 · 幂等 · 可撤销）。装两次，第二次**一个字节都不写、也不留备份**
  （否则「可撤销」会变成「攒垃圾」）。状态三档刻意不合并：没装 / 已是最新 / 版本不符 ——
  「没装」并进「不是最新」，用户以为点一下是更新，其实是第一次装。
- **插件面：marketplace 的只读枚举**，并在页面上**明说**不知道装了 / 启用了哪些。
  原计划是列「已装插件」，而实测**那个集合今天在盘上没有真相源** ——
  一个写着「已装 39 个插件」的界面会在说假话。三条出口刻意不合并：
  没有 / 读不到 / 某条数不出（是「不知道 + 理由」，**不是 0**）。
- **全景多跳调用子图与影响面**（深度 1–3，每层渲染条数有上界，**截断必须说清**）。
  刻意不做图布局引擎：分层列表对「谁调了谁」的可读性不输点线图，而点线图在几十个节点上就糊了。
- **顶栏那七个图标改成不依赖字体的 SVG。**
  用户在 Linux 上实测报告「右上角那些不显示 emoji，反而显示一些奇怪的东西，根本不知道是什么」。
  成因是两类字符混在一起：其中五个不是 emoji 而是数学 / 几何符号（靠正文字体覆盖），
  另两个才是真 emoji（要 emoji 字体）—— 换成「另一个符号」只是换一个赌注，
  所以改成零字体依赖的方案。
- **设置 → 改动足迹多一节：数据面漂移记账。**
  CC 自己的会话记录格式在变，而这件事此前**不可观测**：盘上记的是 7 种未知记录类型 / 8,774 条
  （2026-07-16），**17 天后复测是 10 种 / 27,747 条**，新增 `started` · `result` ·
  `fork-context-ref`（后者说明 CC 侧长出了第二套分叉记账，与本工具仿的那套不是同一套）——
  **是人手工扫语料才发现的，那正是问题所在**。
  本节**零行为变化**：不改任何白名单、不新增告警、不新增轮询，解析输出逐字节不变。
- **MCP 的工作目录做成可浏览的清单**，不再只喂自动补全。
  实测本机 12 个项目里只有 4 个真有 `.mcp.json` ⇒ 以前只能猜一个路径、点进去、
  发现是空的、再猜下一个。
- **SFTP 头部加「新建文件」—— 同名时拒绝，不覆盖。**
  旁边的「新建目录」由服务端报错，天然毁不了东西；而写文件那条**零存在性检查**：
  在一个已有 `notes.md` 的目录里点「新建文件」、手滑打了同一个名字，
  那份文件当场变成 0 字节，没有提示，不可撤销。两个按钮并排、叫法一样，
  而危险程度差一个量级 —— 那正是最容易出事的形状。

### 改进 — 不再偷偷刷屏 / 不再白算

- **报错提示按标题合流** —— 一条都没少报，只是不再刷屏。
  全仓 153 个提示调用点没有一个在循环里：刷屏不是「一个调用点连发」，
  是「多个来源同时各发一条」（后端逐台报错事件 + 健康检查的节流键只防同一台重复弹）。
- **实时会话的主线重算与 tab bar 刷新改成帧末合批。**
  以前 live 分支每来一条记录就跑一次全量拓扑，变了还要走一次全量 DOM 重建 ⇒
  N 条记录是 O(N²)；而它的契约只要求「每条 1 帧内反映折叠状态」——帧内算一次就满足。
- **历史视图那三个放大器是相乘的，不是相加的**（已收）。
- **那圈 10 秒账号轮询**：串行扇出 / 无重入锁 / 无可见性门控，三条一起修。
- **`↗` 拉到前台失败时的归因按实况分档。**
  以前只有一句「未绑定窗口（远端会话需在远端启用 ccm wrapper）」——
  它对「用户直接跑 claude 而不是 ccm」是对的，但对**没有交互终端撑着**的会话
  就是把用户引向一个不存在的问题（去装 ccm 也不会好）。
  现在只在**失败路径上**补查一次 tmux，分四档，其中一档明说装 ccm 不会好；成功路径零开销。

### 改动 — 其它

- **`ccm` 的身份（`@ccm_sid`）必须走 daemon，没有 daemon 就响亮失败（退出码 `2`）。**
  那条**每会话一条、与会话同寿、跑在远端机器上的每秒轮询**整条删掉，
  **不留任何轮询退路**。接班的是 daemon：它本来就在盯 sessions 目录，
  看到 pidfile 的那一刻 pid 与 sid 同时在手，零新增节拍、零新增探测进程。
  `/clear` 与 `/branch` 原地换 sid 时 pidfile 被重写，跟着重打，旧行为不丢。
  逃生口仍在：`CCM_NO_DAEMON=1` 明示放弃身份（那时**照样往 stderr 说一句**，不闷声）。
- **`ccm` 的账号解析改走 daemon**（能力 token 多一个 `account-via-daemon`）。
  daemon 侧零改动 —— 它本来就有那条命令。
- **删掉机器详情页里的 origin 选择器**：留一个能绕过页面上下文的入口，
  等于把「进去 = 选定一个实例」这条地基判据降级成约定。
  cc-bus 驾驶舱那个下拉**保留且必须保留** —— 它住在顶层视图，那儿没有页上下文，
  选择器本身就是上下文。
- **「不指定账号」那句文案在主路径上是假的**（本版只订正文案，不改行为）：
  这一档经账号维度恒发 `--base`，而 `ccm` 收到 `--base` 是 `unset CLAUDE_CONFIG_DIR`；
  远端 shell 里若有 `export CLAUDE_CONFIG_DIR=<某账号>`，
  「不注入」会继承它、「unset」落回 `~/.claude` —— 两者落到的是**不同的账号**。

### 项目管理

- 出货门禁从七格扩到九格（两套 `ccm` e2e 进门），每一格配上分母；
  门禁红了说得出**为什么**红、诊断吐得出失败测试的名字、门禁自检补上「这一条判定还在判」。
- 弱网台架进仓：两个容器 + 自建网络造四维网况，每维给出改前 / 改后两个读数，含一条真 SSH 连接。
- daemon 协议内部去掉 agent 名（additive，线上字节逐字节不变）。
- 多轮「零源码改动」的摸底把几条撑着结论的前提推翻并留下量具，其中三条的结论是**不值得做**
  （第五张网 / 住址描述词那道闸 / 协议面五条候选），按读数记账而不是按直觉。

### 升级须知

装新版 app 之外，**这两样要重新部署才生效**：

1. **远端 daemon** —— 本版 `BUILD_ID` 已 bump（`p1u-fork-session` → `p2e-dial`），
   连上远端后会**自动判旧并重装**。上面「后端常驻 / 死亡账 / 中转 / 插件环境 / UTF-8 口径」
   几条都在它里面。手工装的入口：设置 → 机器 → 那台机器 → 组件 → 「安装 daemon」。
2. **`shared/ccm`** —— 上面四条 `ccm` 行为变更都在它里面（统一走后端 `exit 4` ·
   身份必须走 daemon `exit 2` · 显式名撞了 `exit 3` · 账号解析走 daemon）。
   设置 → 机器 → 那台机器 → 组件 → 「安装 ccm」。
   装对了的话探测回来那行是 `version=4`，能力列表里有 `account-via-daemon`。
...
- **任意对话节点分叉，两条都活着**：每条 user / assistant 消息旁多了一个 `⑂`。点它把
  `[根 … 这一条]` 复制成一份**全新的会话文件**，然后**直接把新会话起起来** —— 原会话
  一个字节都不改、也不会被杀掉，两条从此各走各的。
  - 与 CC 自己的 `/branch` **不是一回事**：那个是同一份 pidfile 原地换 sid（原会话就没了）。
  - 落盘格式与官方 `/branch` **一致**（拿两份真机原生 fork 的算法指纹比对过：它跳过了
    562 / 52 条旁支 ⇒ 排除线性切片 ⇒ 官方与本工具同为「按 `parentUuid` 上溯到根」）。
    所以分出来的会话 `claude --resume` 直接能接着聊。
- **远端会话也能分叉**：远端那份 jsonl 在另一台机器上，所以这件事交给那台机器上的 daemon 做
  （新命令 `--fork-session`）。monitor 只传**会话 id**、不传路径。
  > **要重新部署 daemon 才有**：本版 daemon 的 `BUILD_ID` 已 bump，连上远端后会自动判旧并重装。
- **实时会话里也有入口**：不再只有历史查看器。
- **被 ESC 回退掉的那些消息**：**保留分叉入口，但呈现区分**（悬停提示会说清「这条属于被 ESC
  回退掉的分支，分叉会把你当初放弃的那条对话复活成一个新会话」）。那段数据在 CC 的 TUI 里
  已经没有任何出口，删了就是删了 —— 缺的是信息，不是限制。

### 改动

- **新会话继承原会话的参数**（账号 / 工作目录 / 是否在 tmux 里），**查不出来的那几格会问一次**，
  不猜。已退出的会话账号是真的还原不出来（记账号的 pidfile 随进程消失、各账号的 `projects/`
  是同一个共享目录的软链、会话文件里 44 个字段没有一个记账号）—— 与其静默用错身份跑一条对话，
  不如多问一次。追问小窗里账号那一格的默认值是**账号 0（不注入）**，不是「当前账号」。
- **分叉出来的 tmux 会话另取名**（`<原名>-fork-cc`，撞名再排 `-2`/`-3`）。同名会让新会话
  attach 进原会话那个窗口，正好毁掉「两条都活着」。

### 修复

- **拉前经常失败（「未绑定窗口」）**：`ccm` 把 tmux 的 `set-titles-string` 设成一次性字符串，
  而 Claude 自己会持续改写 pane 标题把那个 marker 冲掉；`ccm` 每 20s 才重设一次，而 monitor
  的按需扫描窗口只有 4s ⇒ 首次绑定命中率约 1/5。改成让 marker 由 `@ccm_sid` 现场合成、**常驻**。
  > **要重新部署 ccm 助手才有**（设置里每台机器卡片可一键装）。
- **Windows 上「换个账号 resume」被判成非法路径**：路径合法性检查照搬了 POSIX 的
  「必须斜杠开头且不含反斜杠」，于是 Windows 的账号目录恒被拒。改成按 shell 分开判。
- **本机「账号 0」可能静默落到别的账号上**：账号 0 原来渲染成「什么都不加」，而本地拉起会加载
  shell 配置文件，里面很可能有一句 `export CLAUDE_CONFIG_DIR=<默认账号>` ⇒ 落到那个号上，
  而界面上写着「不注入」。改成显式 `unset`（远端那条路一直是这么做的）。
- **远端 daemon 的错误看不见**：远端一次性命令此前只收 stdout，失败原因（写在 stderr 上）
  与退出码都被丢掉 ⇒ 「命令失败」与「查询成功但结果为空」不可区分。分叉这条路改成三样全收，
  失败会带着 daemon 给的原因弹出来。
- **v3.4.0 / v3.5.0 连续两版 CI 是红的**（Release 绿、CI 红，没人处理）。四条全修：Windows 上
  一条路径断言用了斜杠后缀（在主平台从来没绿过）· vendored `cc-acct-iso` 的三个脚本 vendor 进来时
  丢了可执行位（294 条断言里 184 条连锁失败）· 两套 e2e 分别是「命名约定改了测试没跟」和
  「等一个再也不会来的事件」。**本版起 CI 七个 job 全绿。**

### 项目管理

- 分支记录的变换逻辑提成共享 crate `branch-core`，monitor 与远端 daemon 共用一份。
- daemon 的写盘能力被两层护栏钉死在**一个**模块上（必须 `O_EXCL` 新建，禁删除 / 改名 / 截断 /
  追加 / 覆盖写；本版又加严了「每个写句柄都必须配一个 `O_EXCL`」与递归扫描）。
- 平价对账表里「远端历史会话不能分支」这条欠账**还清**。

---

## [3.5.0] — 2026-07-31

距 3.4.0 共 31 个 commit。主体是**设置面板整个重做**，另有三个真 bug 的修复。
**升级后需要重新部署两样东西**，见最后的「升级须知」。

### 变更 — 设置面板按「被设置的对象」重新组织

- **顶层从「连接 / 外观 / 账号 / 集成」四组换成「应用 / 机器 / 改动足迹」三页**，左侧导航。
  原来那四个名字不在同一抽象层（「连接」是动作、「外观」「账号」是切面、「集成」是其它），
  于是新东西一律往「集成」里塞 —— 14 个设置块有 8 个挤在那一个折叠组里。
  现在的判据是「**每个顶层 = 一类被设置的对象；进去 = 选定这类里的一个实例**」。
- **机器成了中心对象**：本机是机器列表的第一行（不可删），点进去是那台机器自己的一页，
  分「连接 / 组件 / 账号 / 工具」四栏。**daemon、启动器、多账号这三处部署第一次同屏**
  ——以前分别藏在三个不同的折叠组里。
- **列表行上直接显示每台机器的状态**（连接 / daemon / ccm / 多账号 / 账号数），
  一律写成「3 分钟前」这种**上次动作的结论 + 多久以前**，不伪装成实时探测
  ——打开设置面板**不会**因此多发任何一次 SSH。
- **机器页顶部多了「还差什么」清单**：列出这台机器缺哪些件、缺了的后果是什么。
  「**确认缺**」与「**还没测过**」严格分开显示 —— 一个刚装好什么都没点过的人
  不该看到一屏红叉。
- **每台机器可以有自己的 resume 命令**。以前是全局单值，于是「A 机装了 ccm、B 机没装」
  这种情况表达不出来；留空 = 沿用全局默认，**没填过的人行为一字不变**。
- **cc-bus 驾驶舱移出设置**，成为顶层运营视图 —— 它回答「现在谁在跑、给谁发消息」，
  跟「字体多大」不是一类东西。入口是**命令面板 Ctrl+K**（顶栏已有 6 个图标，不再加第 7 个）。
- **per-account 的 plan 窗口用量并进顶层「用量」视图**，不再单开一处。
  按需读取（点了才探，一次探测要在远端起 tmux 跑 `/usage`），读不到时**说读不到并附原始屏**，
  不给一个看起来像真的数字。
- **「需重启才生效」从三处静态提示收敛成窗口底部一条状态条**：只在**真有**改动待生效时出现，
  并列出改了什么。刻意**没有**「知道了」按钮 —— 改动没生效这件事不会因为点一下就不成立。
- **UI 上不再出现「基座」这个词**，五处改成「**不指定账号**」。
- **Linux / macOS 上不再显示「PowerShell 集成」那一块**（换成一行说明为什么不适用）。

### 修复

- **`/branch` 之后原 tab 永久变灰、还杀不掉。**
  症状：在会话里跑 `/branch`（或 `/clear`），原来那个 tab 变灰且点「关闭」无效。
  根因：`/branch` 是**同一个 pidfile 原地换 sid**，旧 sid 并没有死、只是被顶替了；
  而 monitor 判断「灰还是归档」时去查自己缓存的那份 `tmux ls` 原文，那份快照对这个场景**恒错**。
  修法：daemon 在 `session_removed` 帧上明说 `cause`（`gone` 真没了 / `superseded` 被顶替），
  monitor 收到 `superseded` 直接归档、**根本不查快照**。缺字段视为 `gone`，旧 daemon 行为不变。
  回归性：v3.4.0 起。
- **账号用量读不到 —— 显示「认不出格式」。**
  症状：点「刷新用量」后拿不到 plan 窗口百分比。
  根因：两个缺陷叠在一起。① 探针的稳定判据是「连续两次抓屏一致」，
  而 `/usage` 面板渲染出来**之前**屏幕本来就是静止的 ⇒ 抓回一张压根没有面板的屏；
  ② 解析器把桶标签硬编码成三条枚举，而真机第三块是 `Current week (Fable)`
  （括号里是**会变的模型名**）⇒ 整块被静默丢掉。
  修法：探针改成「屏幕变过 **且** 静止满 3 秒」才判定渲染完成；解析器改成结构性识别，
  以后加窗口 / 改模型名都不用动代码。
- **`--account-trust-zero` 这个参数从来没被分发过。**
  症状：远端 daemon 上该参数不生效。根因：`main` 的分发分支里漏了它。随 v3.4.0 发出去的真 bug。

### 变更 — 终端里的 `cc`

- **在已有 tmux 会话的路径下敲 `cc`，现在是新建一个会话，而不是 attach 进去。**
  以前的行为是「这个路径已经有会话 ⇒ 接进去」——但你既然新开了一个终端并敲了 `cc`，
  意图就是再要一个。显式指定 `--tmux=<名字>` 时仍然幂等（同名就接进去），没变。
- **tmux 会话命名从 `cc-<路径>` 改成 `<路径>-cc`**，长路径下前缀不再把名字挤没。
  **老会话不会失管**：两种写法都认。
...
1. **远端 daemon** —— 上面那条 `/branch` 灰点修复靠 daemon 在帧上说 `cause`。
   不重新部署的话**那个 bug 照旧**。设置 → 机器 → 那台机器 → 组件 → 「安装 daemon」。
   装对了的话 `BUILD_ID` 是 `p1t-removal-cause`。
2. **`shared/ccm`** —— 上面「终端里的 `cc`」两条行为变更在它里面。
   设置 → 机器 → 那台机器 → 组件 → 「安装 ccm」。
