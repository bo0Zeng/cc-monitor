...
>
> English · [中文](./README.md) | License: MIT | Platform: Windows 10/11 · Linux (.deb) | Current: v3.8.1

Renders the real-time conversation written by Claude Code CLI to `~/.claude/projects/*.jsonl` with a modern UI: Markdown / LaTeX / syntax highlighting / collapsible tool-call cards / auto multi-tab management / history browsing & resume / **branch from any past turn**. **Read-only with respect to Claude's own data, zero intrusion** (does not **modify** Claude Code's jsonl / pidfiles; the only two explicit user writes against them are deleting a session and branching from a turn — the latter only **adds** a new session file, leaving the original untouched). ⚠ **This does not cover your own files**: the SFTP panel, the planned-build inbox, the `~/.bashrc` helper install and `.mcp.json` are all writes performed under an explicit user gesture, each registered in `src/bridge/src/write_site_registry.rs` (local) and `remote_write_registry.rs` (remote).

**Project status**: Stable & in use. backend + remote-daemon + vendor `code-picture-core` tests + frontend node pure-function & vitest+jsdom DOM tests + an e2e suite, strict tsc type-check, **CI green** (**counts are deliberately not duplicated here** — run them; see `src/`src/doc/DEVELOPMENT.md` and the floor lines in `.github/workflows/ci.yml`) (Rust `cargo test` [incl. `-p code-picture-core`] + frontend `npm test` [+ advisory eslint/stylelint + a coverage floor] + remote-daemon `cargo test` + an e2e-scripts health smoke [shellcheck/py_compile] — `npm test` gates only the frontend). current release **v3.8.1** (**Multi-account + third-party API relay** — every account can carry its own upstream endpoint and key; a loopback-only HTTP relay swaps the `Authorization` header on the way out, so the client is configured with zero credentials, and an account with no row in the relay table still goes straight to the official endpoint; **your own machine is now a machine too** — a fresh install with no remote configured has something usable on first open; **⚠ two things break unless you redeploy**: `ccm --tmux` now always starts sessions through the backend (the local-tmux fallback is gone from the exec path; an unreachable backend fails with exit code `4`; `CCM_VERSION` 3 → 4, so redeploy `ccm`), and the backend's build id changed while backend-spawned plugins no longer inherit the backend's whole environment (every remote that already has a backend is judged stale and reinstalled once on connect); v3.6.0: **fork any conversation node, both branches stay alive** — the `⑂` next to each message copies `[root … this message]` into a brand-new session file and starts it, leaving the original untouched; remote sessions fork too (done by the backend on that machine); v3.5.0: **settings rebuilt around the object being configured** — App / Machines / Change-footprint pages, machines become the central object, and the cc-bus cockpit moves out of settings into a top-level view; v3.4.0: **liveness switched to kernel events** (grey-out latency ~16s → ~0.13s) **and the first Linux `.deb`**; v3.3.0: **Multi-account: isolated yet synced + per-session account switching + in-app account deploy wizard (#68/#69)**; earlier **Batch 14: SSH/SFTP/tmux remote integration (F41-F60)** — one-click remote resume (launches a terminal) / multi-address failover (happy-eyeballs racing) / SFTP file panel (browse·up/download·edit) / one-click public-key push / tmux attach & right-click screen preview / ProxyJump jump hosts / bulk import & aggregate from ~/.ssh/config / local port-forward console / daemonless fallback reads / "Claude finished a turn" system notification / tool-card file paths → SFTP locate; v2.22.2: **⚙ mislabel fixed** — bg-spare claiming the parent sid could demote an interactive session into a mis-mounted gear tab; kind conflicts now resolve deterministically; **remote stream-mode degradation fixed** — installers since v2.19 shipped without the backend identity manifest, silently hiding bg sessions & re-introducing congestion; manifest embedded + hello-driven self-heal + visible degradation toast; v2.22.0: **message-stream virtualization** #35 — long sessions no longer lag (off-viewport layout/paint skipped + precise height estimates), history viewer opens a 37MB session in 1.1s (was 65.5s), cold start 24s→4s, live tabs load earlier messages on scroll-up; **right-click Resume on archived tabs**; `cc` first-bind race fixed — new shells no longer stall 800ms); v2.21.0: (**customizable resume command** (cc/cct), sidebar-drag / h-scroll / remote-↗ / ccm-installer fixes; v2.20.0: **vertical left tab bar** — drag-resize / narrow-window collapse, tabs no longer slide under the top-right icons; **background-clone sessions flagged in history** with a ⚙ badge so you resume the real session; + v2.19.1 fix: queued input messages no longer misfolded as ESC-rewinds #36); v2.19.0: (**remote congestion eliminated** — history via side-channel snapshots + a dedicated real-time tail, 46MB ≈ 4.6s with zero overflow (verified E2E); **newest messages load first**; **remote session status lights** now match local; correct skeleton/bg/focus rebuild after F5), now covering **SSH remote mode** (aggregate local + multiple remote machines in one window, #15/#17/#18/#20/#30/#31) — incl. **backend auto-deploy + one-click install/uninstall** (embedded musl binaries pushed over SFTP #29; per-machine cards install/uninstall the backend & ccm helper with install-location hints), **remote full-text search** (#28), **remote history delete / one-click resume** (since F41: tab right-click / history ↺ launches the remote terminal directly, falling back to clipboard copy), **history grouped/collapsible by machine** (#30/#31), **version negotiation + congestion toasts** (#32/#33), **session status lights** (#23), **local session auto-revive on resume** (crash/exit → archived, `/resume` restores without F5), visible AskUserQuestion options / API errors (#21), single-key shortcuts + tab tear-off, and more. See [CHANGELOG](CHANGELOG.md) + [src/`src/doc/ARCHITECTURE.md`](src/`src/doc/ARCHITECTURE.md`).
...
> SSH + backend path (see "SSH remote mode" below) works fully.
> Details in `src/`src/doc/ARCHITECTURE.md`.
...
- A remote backend streams sessions back over SSH live; **auto-reconnect** on drop (exponential backoff 2→30s, #17), seq-dedup catch-up on reconnect
- **Backend auto-deploy** (#29): cc-monitor embeds cross-compiled aarch64/x86_64 musl backend binaries and pushes the right one over SFTP on connect (build_id version-gated) — zero manual deploy
- **Remote full-text search** (#28): the top-bar full-text search covers remote session content via a backend server-side `--search`, hits tagged `[host]`
- **Remote history delete** (SFTP remove, double-confirm) / **one-click resume**: right-click a tab / history `↺` launches a remote terminal running `claude --resume` (wt.exe first, falls back to copying the command to the clipboard on failure)
  - ⚠ **On Linux / macOS no terminal window is opened** — the command is copied to your clipboard instead. This is a **deliberate design decision, not an unfinished feature**: POSIX has no "the terminal", and picking one for you (gnome-terminal / konsole / alacritty / …) is a decision that would be wrong on someone else's machine. The session container is tmux anyway, so paste and run it in your own terminal. **Local** resume is unaffected (no window either — the command just runs and the session waits in tmux)
- **Per-machine cards** in settings: one-click **install / uninstall backend** and **install / uninstall the `ccm` helper** (into the remote `~/.bashrc`), with **install-location hints** (backend → `~/.cc-monitor/bin/`, ccm → `~/.bashrc` marker block); cards collapse to the machine name
- **Version negotiation** (#33) + **slow-consumer overflow signal** (#32): a backend/client build_id mismatch or a congested pipe surfaces a remote-health toast
- Remote tabs can also ↗ raise their terminal (#18)
- Deployment: [src/`src/doc/REMOTE-PHASE0-DEPLOY.md`](src/`src/doc/REMOTE-PHASE0-DEPLOY.md`) (auto-deploy + manual fallback)
...
| **T** | Toggle Task panel |
| **Ctrl+K** | Open the command bar (command palette) |
| **Esc** | Close topmost overlay (read-only viewer → history view → settings panel) |

> **Defaults are all single keys** — cc-monitor is a read-only monitor window, no modifier needed. When an editable field (search / rename input) is focused, shortcuts yield to typing. Every chord is customizable in Settings → Shortcuts; **6 actions are unbound by default** (behavior toggles + **account** switch/align — "align current session to the account" is destructive, so it deliberately has no default single key) and can be assigned a key there.
>
...
|---|---|---|
| [src/`src/doc/ARCHITECTURE.md`](src/`src/doc/ARCHITECTURE.md`) | New contributors | Data flow + module map + design layering |
| [src/`src/doc/IPC-PROTOCOL.md`](src/`src/doc/IPC-PROTOCOL.md`) | Protocol changers | Cross-process file IPC + sessions/status + remote wire schemas + handshake timing |
| [src/`src/doc/REMOTE-PHASE0-DEPLOY.md`](src/`src/doc/REMOTE-PHASE0-DEPLOY.md`) | Remote deployers | SSH remote backend auto-deploy (#29) + manual deploy runbook (issue #15) |
| [src/`src/doc/INVARIANTS.md`](src/`src/doc/INVARIANTS.md`) | Everyone | Global invariants (zero intrusion / encoding / ACL / ordering) |
| [src/`src/doc/CONTRIBUTING.md`](src/`src/doc/CONTRIBUTING.md`) | Contributors | Checklists + cookbook (add IPC / jsonl type / setting / hotkey) |
| [src/`src/doc/DEVELOPMENT.md`](src/`src/doc/DEVELOPMENT.md`) | Developers | Dev environment + port conflict / debugging |
| [src/`src/doc/BUILDING.md`](src/`src/doc/BUILDING.md`) | Releasers | Production build + packaging + Code Signing |
| [src/`src/doc/RELEASING.md`](src/`src/doc/RELEASING.md`) | Releasers | Release SOP + how to write the CHANGELOG |
