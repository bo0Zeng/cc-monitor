...
# 地板 = 首跑实测值（PASS=28 FAIL=0 SKIP=0）。判定表 25 行 + 抽取器自检 1 + 两个补充场景 2。
# 表加一行 ⇒ 这里也要 +1（CI 的 `assert-pass-floor.sh daemon-gate2 28` 同理，**两处同一个数**）。
[ "$pass" -ge 28 ] || { echo "通过数低于地板 28 —— 用例被静默跳过了？（SKIP=$skip）"; exit 1; }
[ "$fail" -eq 0 ]
