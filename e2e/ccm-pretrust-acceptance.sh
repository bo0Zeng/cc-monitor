...
# ⚠ `e2e/fake-daemon.sh` 里的 `tmux` 走 **PATH** ⇒ 落在上面那个 `-L $SOCK` 的 shim 上，
...
#      本套件要测的预信任面因此干净。
# `FAKE_DAEMON_TMUX_SOCK` 与本套件的 `-L $SOCK` **给同一个名字**：那份假后端自带选择器
# （fail-closed，见它的头注），而 PATH 上的 shim 会再插一个 —— tmux 取最后一个，同名即同 socket。
