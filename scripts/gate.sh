...
#     ⚠ **现值 09-03 `D3` 起是 `ccm-cli 240` · `ccm-contract-parity 68`**（下面那两行 `run_e2e` 同值）。
...
#   ⚠ **`D3`（生产接线）之后是 328**（12 + 8 + **240** + 68）—— 同样量于本拍那趟沙箱门禁的
#     四格读数，**不是从 291 加出来的**。
#   ⚠ 别把这一格读大成另一个方向，两条：
...
# ★★ **`K-P2` `D` 阶段第三拍（09-03，生产接线）：`ccm-cli` 再棘 `203` → `240`**。
#   那 37 条是 `WIRE/launch` 一节：**`D2` 那 30 条盖的是 `--resolve` 那一跳，一条都没盖住
#   `launch`**。棘之前那一趟是**切完 8 刀、补完判据之后**的那一趟（`§19 裁八`），
#   沙箱 `ccmon-devbox:latest` 现打 `PASS=240 FAIL=0`。
run_e2e ccm-cli               240
