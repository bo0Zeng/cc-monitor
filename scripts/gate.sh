...
#     `ci.yml:559` `assert-pass-floor.sh ccm-cli 126` · `:574` `… ccm-contract-parity 68`。
...
#   ⇒ **那个「只对 20 条成立」的读数到此作废**，今天四套合计 **214 条**（12 + 8 + 126 + 68）。
#   ⚠ 别把这一格读大成另一个方向，两条：
...
# 与 `ci.yml:559` / `:574` 同值 —— 详见本文件头注那一节，改地板要三处一起改。
