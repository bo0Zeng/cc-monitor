...
#       （559 → 571、574 → 586）。**按名字指，不写行号**（`e2e/ccm-cli.test.sh` 头注同一条纪律）。
#     ⚠ **现值 09-03 起是 `ccm-cli 202` · `ccm-contract-parity 68`**（下面那两行 `run_e2e` 同值）。
...
#   ⚠ **09-03 `K-P2` `D` 阶段之后这个合计是 290**（12 + 8 + **202** + 68）—— 量于本拍那趟
...
# ★★ **`K-P2` `D` 阶段（09-03）：`ccm-cli` 棘 `126` → `202`**（`ccm-contract-parity` 本拍没动）。
#   那 76 条分两拍买的：`D1` 的 `JSONENC` 47 条（真 JSON 编码器，与 `jq -Rs .` 逐字节对拍）
#   ＋ `D2` 的 `WIRE` 29 条（真发请求那一半：落盘式假 daemon，「发了」与「发对了」分两族判）。
#   量于 `7d93775`、同一份 devbox 镜像，`PASS=202 FAIL=0`。
...
run_e2e ccm-cli               202
run_e2e ccm-contract-parity   68
