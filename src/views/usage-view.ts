...
export class UsageView {
  ...
  private renderList(): void {
    ...
    for (const h of [
      ...
      { t: "合计" },
      {
        // F88d-fix(round2)：「▼」标出这是排序键——表按本列降序排（哪儿最烧在最上）；否则用户扫更显眼的
        // 「合计」列会看到一列乱序数字却不知按什么排。
        t: "等效∑ ▼",
        // F88d：相对成本折算，非绝对 $。F88d-fix：表已按本列降序排（哪儿最烧在最上）+ 跨模型告警。
        title:
          "等效 input token = Σ(各档 × 相对系数：input1 / cache写1.25 / cache读0.1 / output5)。反映相对成本（哪儿最烧），非绝对 $。▼ 表按本列降序排。" +
