 ...
 * **前提退役了，这条路没退役 —— 它另有消费者。**
 *
 * 🔴 **`K-R105` 09-13：这里原来写着两个数（「3 个」「2 个」），撤了。**
 * 那是**尺子A** 的读数，而读它的人一律读成**尺子B**（「有三条生产路在跑它」）——
 * 现打尺子B：那三个消费者文件里**只有 `remote-launch-run.ts` 有生产调用方**。
 * 一个数不写清它的尺子就是半句假话；⇒ 这里只留住址，数让机器现算。
 * 两条依据各有一条会红的判据，住 `launch_wire.rs` 的
