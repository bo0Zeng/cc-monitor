...
// ——**那个目录永不被创建**，它只是被 `../` 抵消掉的一级。所以从 `src/bridge/` 跑测试时：
//   `src/bridge/` + `bindings/` + `../../src/generated` = `<repo>/src/generated/` ✓
// 基准是**测试二进制的 cwd**（`std::env::current_dir()`，不是 `CARGO_MANIFEST_DIR`），
// 而 cargo 会把它设成 package root ⇒ `cargo test` 与
// `cargo test --manifest-path src/bridge/Cargo.toml` 都落对（审计双向实测过）。
// **直接跑测试二进制则会落到仓库外**（审计实测落在 cwd 上两级）——CI 安全，
// 因为 `ci.yml` 用 `working-directory: src/bridge`。
// 记这一段是因为：**我第一次写成 `../src/generated/`，落到了 `src/bridge/src/generated/`**，
...
#[cfg(test)]
#[path = "../../../tests/bridge/data_paths_tests.rs"]
mod tests;
