...
//!
//! 09-12 PM 现打：`cargo tree -i russh --manifest-path src/bridge/Cargo.toml` 只有一条边
...
//!
//! 🔴 **家变成空集那一刻，正是二值旗该翻面的那一刻** —— `russh` 从 `src/bridge/Cargo.toml`
...
mod tests {
    ...
    ///
    /// `(相对 `src/bridge/src/` 的路径, 生产段里的锚点处数, 它是什么, 解锁条件)`
    ///
        ...
        if seen != want {
            return Err(format!(
                 ...
                 **少了**：那一处搬走了或没了 ⇒ 把这一行的处数改小；改到 0 就把整行删掉，\
                 并同拍去看 `russh` 还能不能从 `src/bridge/Cargo.toml` 里走。"
            ));
        }
        Ok(total)
    }
    ...
    fn the_interface_side_ssh_debt_is_one_number_and_it_is_printed() {
        ...
        eprintln!(
            "〔界面侧 SSH 债 · 09-12 立〕过程那个数 = **{truth}** 处拨号还没搬走\
             ...
             用 `use` 别名或宏藏起来的调用点。\n\
             〔终点那面二值旗〕`src/bridge/Cargo.toml` 里 `russh` 家族的直接依赖：{deps:?}\
             —— 空了才算 `R32` 裁定一那句话成真。"
        );
