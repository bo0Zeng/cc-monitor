...
//! **`src/bridge/Cargo.toml` 当时没有 `[workspace]` 表**：六个 crate 只是 path 依赖，
//! `--all` 覆不到，只能一个个手工列。
...
//! | # | 要补哪儿 | 漏了会怎样 | 谁在钉 |
//! |---|---|---|---|
//! | 1 | `src/bridge/Cargo.toml` 的 `[workspace] members` | **静默少跑**（`--workspace` 覆不到非成员，那个 crate 的测试从门禁里消失，不是失败是不存在） | `every_shared_crate_is_a_workspace_member` |
...
mod tests {
    ...
    /// ★ 每个共享 crate 都必须在 `src/bridge/Cargo.toml` 的 `[workspace] members` 里。
    ///
    ...
    fn every_shared_crate_is_a_workspace_member() {
        ...
        assert!(
            ...
            "`src/bridge/Cargo.toml` 的 `[workspace]` 没了 —— 六个共享 crate 会退回\n\
    ...
    fn every_ci_run_step_is_classified_as_local_or_unrunnable() {
        ...
        const STEPS: &[(&str, bool, &str)] = &[
            // ── job rust
            ("cargo fmt --check（整个 workspace）", true, "`cd src/bridge && cargo fmt --all --check`"),
            ...
            ("cargo build (full app binary, not --lib)", true, "`cd src/bridge && cargo build` —— 它编的是 bin，`--lib` 那条盖不住"),
            // ── job e2e-smoke
