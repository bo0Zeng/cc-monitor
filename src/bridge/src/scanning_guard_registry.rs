...
//! | **扫的树不含自己** | 2 | `frame_cadence_guard` 只扫 `.md`（自己是 `.rs`）· `shared_crate_registry` 扫 `crates/` 与 `Cargo.toml`（自己在 `src/bridge/src`） |
//!
...
//! - **射程只有 `src/bridge/src` 一棵树**（`scan_tree!` 的实参就写在那儿）：
...
//! 09-06 现打（量具 `tests/evidence/K-R36-per-guard-census.py`，读数与逐条判词在
...
//!
//! 上面「⚠ 它没有买到什么」的**第二条**（射程只有 `src/bridge/src`）**已经不再成立**：
//! `every_registry_guard_keeps_its_reverse_half` 今天走**逐子树循环**，实参两棵
//!（形状照本模块 [`raw_walkers`]，它从一开始就是这么写的）。
//!
//! ## 🔴 它买到的红是 **0** —— 这一句必须写在最前面
//!
//! `D1` 现打（量具 `tests/evidence/K-R37-daemon-tree-census.py`，读数与**逐条判词**在
...
//! 只看总数的地板同样接不住（`src/bridge/src` 那一百多份自己就顶过去了）。
//! ⇒ 地板逐子树各一个（抽取器自检⑤）。
...
//! `D1②` 现打（量具 [`ratchet_history`] 与 `tests/evidence/K-R38-ratchet-history-census.py`，
