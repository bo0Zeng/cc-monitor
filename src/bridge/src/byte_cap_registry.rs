...
mod tests {
    ...
    const NOT_A_SIZE_CAP: &[(&str, &str)] = &[
        ...
        // ⚠〔`K-H1` 回修轮之五 08-25，D3 `阻-2(D3)`〕tee 的写从「持锁同步写」改成
        //   「投进有界队列 + 专属写线程」时带进来的。
        (
            "TEE_QUEUE_LINES",
            "**条数**不是体量：tee 队列能排多少**行**（`sync_channel` 的界）。与本表第一条 \
             `CHANNEL_CAPACITY` 同族 —— 量的是队列深度，不是任何一段用户数据的字节数。\
    ...
    const CAPS: &[(&str, &str, u64, &str, &str)] = &[
        ...
        //      （那两处随后被改动顶走）。⇒ 改成点**函数名**：`server.rs::handle` 里那两个调用点。
    ...
    fn the_cross_crate_twins_are_machine_checked_not_hand_copied() {
        ...
        //
        // 原文：`src/bridge/src/search.rs` 与 `src/backend/observe/search_query.rs`
    ...
    const UNCAPPED_STREAM_READS: &[(&str, &str, &str)] = &[
        // 🔴 **`K-R104`（09-13）：`src/bridge/src/account_usage.rs` 这一行删了，欠账真的没了。**
    ...
    fn every_uncapped_stream_read_has_an_owner() {
        ...
        //    `src/bridge/src/account_usage.rs` 那一处 `read_to_end`（远端探针的 stdout）
    ...
    fn the_drop_and_report_semantics_is_honoured_at_every_over_limit_arm() {
        let root = repo_root();
        let raw = std::fs::read_to_string(root.join("src/bridge/src/ssh_source.rs"))
