...
mod tests {
    ...
    /// 原来那四条是新集合的子集 ⇒ 只加不减（铁律 13）。
    ///
    /// # 为什么**刻意不翻成枚举式白名单**（Q4d 的反例，量过才敢这么写）
    ///
    /// 白名单形态会是「扫出每一处 `8s` 提及 → 要求它带历史限定词」。实测这条路走不通：
    /// 全扫描面 47 处 `8s`/`8 秒` 里 **41 处落在节拍语境**，而它们几乎全是**历史叙述**
    ///（这个仓花了很大篇幅记「那个 ticker 已删」）。试跑那条规则今天**误红 7 处**，
    /// 全是合法文本：限定词写成「替」「改为」的，以及**引用这句被禁的话本身**的四处。
    /// ⇒ 「带没带限定词」是**自然语言判断**，机检只能近似；把限定词表调到今天全绿，
    /// 就是把判据调成这批文本的曲线拟合（判据成了文本的第二份副本，那等于没有判据）。
    ///
    /// ★ **白名单有两个前提，不是一个**：人群同质 **且 施加的性质可机判**。
    /// 本条人群其实同质（都在讲那个已删的节拍），倒在第二个前提上。
    fn forbidden() -> Vec<String> {
        ...
        let units: Vec<String> = bases
            .iter()
            .flat_map(|u| [u.clone(), format!("~{u}")])
            .collect();
        let verbs = ["推", "推帧", "推送", "推一帧", "发帧"];
        let mut out = Vec::new();
        for u in &units {
            for v in verbs {
                out.push(format!("每 {u} {v}"));
                out.push(format!("每 {u}{v}"));
                out.push(format!("每{u}{v}"));
            }
            out.push(format!("帧最长 {u} 陈旧"));
            out.push(format!("帧 ≤{u} 陈旧"));
            out.push(format!("推帧间隔({u})"));
            out.push(format!("推帧间隔 {u}"));
        }
        out.sort();
        out.dedup();
        // 反向自检：派生塌了（单位表或说法表被掏空）⇒ 本护栏静默变成「什么都不禁」，
        // 而它会**照常绿**——那正是本轮在别处反复逮到的静默失效形状。
        assert!(
            out.len() >= 100,
            "只派生出 {} 条禁用说法 —— 单位表或说法表被掏了（08-06 实测 114 条）",
            out.len()
        );
        out
    }
    ...
    fn scanned_sources(root: &Path) -> Vec<PathBuf> {
        ...
        for base in ["src/bridge/src", "src/backend", "src/doc"] {
