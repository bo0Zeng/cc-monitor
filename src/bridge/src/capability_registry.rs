...
//!
//! 那个前提由 `src/bridge/capabilities/default.json` 一个文件决定。
...
mod tests {
    ...
    const CARGO_CFG_DIRS: &[&str] = &[".", "src/bridge", "src/backend"];
    ...
    /// ② 判据这一头 —— 同一形状（键换成 `runner`）放进真格位 `src/bridge/.cargo/config.toml`
    ...
    /// ⇒ 两边各有一形，08-29 各现打了一次（放进真格位 `src/bridge/.cargo/config.toml`、只跑本条）：
    ...
    /// | 面 | 今天 | 谁在管 |
    /// |---|---|---|
    /// | `src/bridge/build.rs` | 起 `sh`/`git`、往 `OUT_DIR` 写 | **08-08 刚并进** `write_site_registry`（第 82 件） |
    ...
    ///   用户本机那份瘦身配置搬进 `src/bridge/.cargo/config.toml` 之后，
    ...
    ///   `CARGO_CFG_DIRS` 的 `src/bridge` 换成 `e2e` ⇒ **红**，报文逐字「本条的人群**逐格**变了。」
    ...
    /// 和「`src/bridge/` 的 `config.toml`」读成同一格。
    /// ⚠ **还有一个例外要说清，别把这句读成「格外的文件本条一概不管」**：
    /// 那 6 格里的 `include` **能把格外的文件拉进来**，而那一形本条逮的是
    /// **那句 `include` 本身**，**不是**去读被拉进来的那份文件（所以它的理由是「看不见」）。
    /// 08-29 现打：`src/bridge/.cargo/config.toml` 写 `include = ["elsewhere.toml"]`、
    /// 被拉进来的那份里写 `[build]` + `rustflags = []`
    /// ⇒ 报文逐字 `["src/bridge/.cargo/config.toml：设了 [\"include\"] —— 本条看不进它拉进来的文件"]`，
    ...
    /// `git ls-files` **703** 个跟踪文件，去掉 `src/bridge/vendor/` 的 **33** 个 ⇒ **670**）：
    ...
    /// 「`src/bridge/` 那一格上**真生效**的 `runner` 会让本条**自己不被执行**」（对形 2 假）；
    ...
    /// `src/bridge/.cargo/config.toml`（`runner` 指向那个 skip-runner），
    /// 拿环境变量 `CARGO_TARGET_<TRIPLE>_RUNNER` 换一个透明代理**让判据跑起来**
    /// ⇒ 当场红，报文逐字 `["src/bridge/.cargo/config.toml：设了执行面的键 [\"runner\"]"]`。
    ...
    /// `D3` 那一刀是只把 `CARGO_CFG_DIRS` 里的 `src/bridge` 换掉（格数仍是 6）——
    ...
    /// **形 2s** —— 一份 `src/bridge/.cargo/config.toml` 就够（08-29 现打：`rc=0` · 包数 **8** ·
    ...
    fn the_build_time_execution_surface_stays_registered() {
        ...
        let want_slots: &[&str] = &[
            "./.cargo/config.toml",
            "./.cargo/config",
            "src/bridge/.cargo/config.toml",
            "src/bridge/.cargo/config",
