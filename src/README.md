...
| **sftp/panel.ts** (B14-F48/F49) | SFTP 文件面板 overlay（每 host 从设置卡「文件」入口开）：浏览（面包屑/列表/排序）+ 传输（下载/上传 dialog picker + 进度 Channel + 取消）+ 拖入上传 + 写（新建目录/改名/删除 + 确认）+ 目录书签 + 「在此打开终端」+ 小文件编辑（F49：面板内浮层 textarea + 字符/字节数 + 覆盖确认 + 失败保留）。消费 F47/F49 `sftp_*` 命令，独立 overlay 不碰 TabManager | `openSftpPanel(cfg)` |
