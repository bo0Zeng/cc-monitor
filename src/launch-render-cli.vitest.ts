...
describe("CLI 渲染只有一个入口", () => {
  ...
  // ⚠⚠ **本条第一版被变异证伪**〔08-08〕：我写成「文件里有 ``return { ok: false, reason: `维度`` 」，
  // 而那种返回**有两处**（能力闸门 + `cliFlags` 返回 null 那条）⇒ 把能力闸门改成 `continue`，
  // 正则命中另一处、判据照样绿。**比的是任意一处，不是那一处**（坏法②，本会话第 N 次，
  // 而且这次犯在我自己刚立的元判据所点名的那一类上）。
  // ⇒ 改成：两条闸门**各按自己的措辞**钉，且**各自恰好一处**。
  it.each([
    ["能力闸门", "需要远端 ccm 能力", "能力不足时**拿不到命令**，而不是静默跳过这个维度"],
    ["cliFlags 闸门", "无法用 CLI 语法表达", "`cliFlags` 返回 `null` 时**拿不到命令**，而不是丢掉这个修饰"],
  ])("%s：走「返回 ok:false」而不是静默跳过", (_name, needle, why) => {
    const n = code.split(needle).length - 1;
    expect(
      n,
      `\`${needle}\` 在生产段出现 ${n} 次（应恰好 1 次）。\n` +
        `  0 次 = 那道闸门没了 —— ${why}；\n` +
        "  ≥2 次 = 有第二条同措辞的路，本条会比到别处去（08-08 第一版正是栽在这上面：\n" +
        "  两处「维度 …」返回，改掉一处另一处仍在，判据照样绿）。",
    ).toBe(1);
    expect(
      code,
      `\`${needle}\` 那一处不再以 \`ok: false\` 返回 —— 头注说的「同一次遍历里直接变成\n` +
        "  `{ ok: false }`：**拿不到命令**」没了，渲染器退回改造前那个丢修饰的形状。",
    ).toMatch(new RegExp(`return \\{ ok: false, reason: \`[^\`]*${needle}`));
  });
});
