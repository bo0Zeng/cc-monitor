...
///
/// ⚠ 字段名与减法**不在这里** —— 它们住 `codex_token_core::codex_delta`（唯一权威源）。
...
pub fn last_token_delta(v: &Value) -> Option<codex_token_core::CodexDelta> {
    let usage = unwrap_envelope(v)?.1.get("info")?.get("last_token_usage")?;
    Some(codex_token_core::codex_delta(usage))
}
