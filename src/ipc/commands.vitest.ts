...
describe("C04a 命令名钉死", () => {
  ...
  it("包装层：键名 ⊆ Rust 集，**且每个条目的键名 == 它传给 invoke 的字面量**", () => {
    ...
    expect(keys.length, `包装层今天覆盖 ${keys.length} 个`).toBe(111); // Z05 +1；L1 +1
  });

  // 标题里的数原先写着 112，而断言早就是 119 了（Z05 起 120，local-as-remote L1 起 121）——**标题也是记录**，
  // 一并订正，免得下一个人拿标题当依据。
  it("TS 侧字面量命令名 ⊆ Rust 集，唯一名数 == 121，动态名盲区逐字钉死", () => {
    ...
    // ★★ **C04d 批 6c 到 119 —— 这是里程碑：命令全部静态可见。**（Z05 +1 ⇒ 120；L1 +1 ⇒ 121）
