...
describe("C04a 命令名钉死", () => {
  it("Rust 侧「声明 = 注册」，且计数恰好 140", () => {
    ...
    expect(declared.size, `期望恰好 140 个命令，实得 ${declared.size}`).toBe(140); // U8c-2c-2 +1（render_ccm_launch）；U8a-2c-pre +1（render_launch_payload）；P3t-Y2b +1（local_tmux_names）；**P4c +2（cc_bus_broadcast / cc_bus_kill，#77/#78）**；**P8a +1（list_plugin_marketplaces，#70）**
  ...
  it("TS 侧字面量命令名 ⊆ Rust 集，唯一名数 == 137，动态名盲区逐字钉死", () => {
    ...
    expect(used.size, `期望恰好 140 个字面量命令名，实得 ${used.size}`).toBe(140); // devbench F03 +3（skill 接入面三条） // U8c-2c-2 +1；U8a-2c-pre +1；P3t-Y2b +1（local_tmux_names）；**P4c +2**；**P8a +1（list_plugin_marketplaces）**
