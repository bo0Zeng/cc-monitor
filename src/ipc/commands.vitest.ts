...
describe("C04a 命令名钉死", () => {
  it("Rust 侧「声明 = 注册」，且计数恰好 128", () => {
    ...
    expect(declared.size, `期望恰好 128 个命令，实得 ${declared.size}`).toBe(128); // U8c-2c-2 +1（render_ccm_launch）；U8a-2c-pre +1（render_launch_payload）
  });
  ...
  it("包装层：键名 ⊆ Rust 集，**且每个条目的键名 == 它传给 invoke 的字面量**", () => {
    ...
    expect(keys.length, `包装层今天覆盖 ${keys.length} 个`).toBe(118); // U8a-2c-1 +1（daemon_send_into）； Z05 +1；G6 远端分叉 +1、list_remote_tmux 进包装层 +1；U8c-2c-2 +1（render_ccm_launch）
  });

  // 标题里的数原先写着 112，而断言早就是 119 了（Z05 起 120；local-as-remote L3a 起 121）——**标题也是记录**，
  // 一并订正，免得下一个人拿标题当依据。
  it("TS 侧字面量命令名 ⊆ Rust 集，唯一名数 == 128，动态名盲区逐字钉死", () => {
    const rust = rustCommands();
    ...
    expect(used.size, `期望恰好 128 个字面量命令名，实得 ${used.size}`).toBe(128); // U8c-2c-2 +1；U8a-2c-pre +1
