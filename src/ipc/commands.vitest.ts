...
describe("C04a 命令名钉死", () => {
  it("Rust 侧「声明 = 注册」，且计数恰好 121", () => {
    ...
    expect(declared.size, `期望恰好 125 个命令，实得 ${declared.size}`).toBe(125); // U8c-2c-2 +1（render_ccm_launch）
  });
  ...
  it("包装层：键名 ⊆ Rust 集，**且每个条目的键名 == 它传给 invoke 的字面量**", () => {
    ...
    expect(keys.length, `包装层今天覆盖 ${keys.length} 个`).toBe(115); // Z05 +1；G6 远端分叉 +1、list_remote_tmux 进包装层 +1；U8c-2c-2 +1（render_ccm_launch）
  });
  ...
  it("TS 侧字面量命令名 ⊆ Rust 集，唯一名数 == 121，动态名盲区逐字钉死", () => {
    ...
    expect(used.size, `期望恰好 125 个字面量命令名，实得 ${used.size}`).toBe(125); // U8c-2c-2 +1
