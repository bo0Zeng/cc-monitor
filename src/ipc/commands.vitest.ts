...
describe("C04a 命令名钉死", () => {
  it("Rust 侧「声明 = 注册」，且计数恰好 136", () => {
    ...
    expect(declared.size, `期望恰好 131 个命令，实得 ${declared.size}`).toBe(136); // U8c-2c-2 +1（render_ccm_launch）；U8a-2c-pre +1（render_launch_payload）
  });
  ...
  it("TS 侧字面量命令名 ⊆ Rust 集，唯一名数 == 136，动态名盲区逐字钉死", () => {
    ...
    expect(used.size, `期望恰好 131 个字面量命令名，实得 ${used.size}`).toBe(136); // devbench F03 +3（skill 接入面三条） // U8c-2c-2 +1；U8a-2c-pre +1
