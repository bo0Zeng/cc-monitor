...
- **测试**：后端 cargo + vendor code-picture-core + 远端 daemon + 前端 node 纯函数 + vitest（jsdom）+ e2e 脚本，CI job 全绿（`rust` / `frontend` / `daemon` / `linux-app-build` / `e2e-smoke` / `e2e-tmux` / `e2e-tmux-rust`；eslint/stylelint 是顾问式基线，覆盖率有地板棘轮）。
