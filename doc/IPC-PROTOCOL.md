...
- 响应**逐块透传绝不缓冲**；同一批字节里的 SSE 事件抄一份到**本进程的 stdout**
  ...
  **一格都不带**。⇒ **去修一句假话，换来一句射程更大的假话。**
  ...
  ⚠⚠ **谁看着这一格 —— 两个人群，别混成一句**（上一拍写成了没有射程的全称，`D4` 逮到）：
  - **甲 · 读这份文档的判据**：分母 = 全仓 `.rs` 里把 `IPC-PROTOCOL.md` 当**字符串字面量读进去**
    的文件，现打**恰好 6 个** —— `include_str!` 那种 **4 个**（`protocol_doc_guard.rs` ·
    `doc_claim_registry.rs` · `daemon_kill.rs` · `profile_installer.rs`）＋**运行时按文件名读**
    那种 **2 个**（`doc_copy_registry.rs` · `control/launch.rs`）。
    六个里 `relay` / `tee` / `__meta__` 命中**各 0 处**。
    ⚠ 这个分母的**完整性**另核过一次（就是被上一次数错逼出来的）：全仓 `.rs` 的 `read_dir`
    落点里**没有**任何一处遍历 `doc/` 收 `.md`（`doc_copy_registry` 自己那个 `collect` 收的是
    `.rs` / `.ts`）⇒ 读这份文档**只有「按文件名点它」这一条路**，上面那 6 个就是全部。
    非空对照：`protocol_doc_guard.rs`
    （1266 行）里 `inbound|REGISTRY` = **14 处** ⇒ 那道判据的人群是 daemon 的**命令 / wire 面**，
    结构上不含中转这一族，不是碰巧没扫到。
    ⇒ **甲的结论要说准**：没有任何判据把**这一格的内容**与 `tee.rs` 对拍；
    但这**不等于**「这份散文没人管」—— `doc_copy_registry` 就读它的措辞，
    只是它买的是**别的性质**（散文里不许出现「只该住在一处的事实」的**数量形态**副本）。
    🔴 **这个分母我数错过一次，把经过记下来**：上一拍我把尺子取成「`include_str!` 那一种读法」
    ⇒ 数出 **4 个**，正好把 `doc_copy_registry` 这种**运行时按文件名读**的漏在人群外，
    于是写出了「这份散文本身没有人对拍」。**本拍它当场把我逮了** —— 我在这一格里写下各套判据的
    条数，`doc_copy_registry::tests::no_quantity_form_of_a_pointer_only_fact_appears_in_prose`
    立刻红，判定行逐字：「散文里又出现了这些事实的**数量形态**副本：`doc/IPC-PROTOCOL.md`……
    改法同 `E12`：删掉这个数、只留指针」。⇒ **那句话被它自己的反例证伪了。**
    ⚠ 与 `E` / `D4` 逮到的那两次是**同一形**：**尺子的作用域对不上要说的那句话。**
    ⚠ 也别把分母取成「提过 `IPC-PROTOCOL` 的文件」—— 那个作用域现打是**全部跟踪文件 30 个 /
    只数 `.rs` 20 个**，其中多数只是散文里提一句、并不读它。
  - **乙 · 看着 tee 行字段名的判据**：分母 = `cargo test relay` 那一趟，**79 条**。
    现打变异：把 `tee.rs` 那两处字段名 `account` 改成 `acctMUT`（**只改名**，零编译风险）
    ⇒ **78 passed / 1 failed**，红在 `server.rs:2340`，判定行是人话（逐字「tee 行里没有账号那一格」
    ＋整条流转储）。⇒ **`account` 那一格是有牙的** —— 79 条里**恰好 1 条**看着它。

  ⇒ 准确的说法是：**「内容对拍」这件事甲乙都不买；乙买的是「字段名」那一格。**
  「今天没有任何判据看着这一格」那句**缺了射程**，而且**连甲那一半也说宽了**
  （散文的**措辞**确实有判据看着，只是它看的不是「这一格跟 `tee.rs` 对不对得上」）。
  ⚠ 乙买的是「**tee 行的字段名**变了会红」，**不买**「这份散文与 tee 行对得上」：
  本拍现打 —— **只改这一格、代码一个字没动 ⇒ 全量门禁各道读数逐个不变、`GATE: OK`**
  （具体条数**不抄在这里**：那是 `doc_copy_registry` 的 `POINTER_ONLY` 管着的「只留指针」
  那一族事实，它们的家在 `scripts/gate.sh`）。
