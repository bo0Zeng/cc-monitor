...
//!
//! ⚠ **为什么自己写 SHA-256 而不是加一条依赖**（[`sha256_hex`]）：本 crate 的
//! `Cargo.toml` 里逐字记着一条今天仍成立的约束 —— 依赖树钉在一个 **yanked** 的
//! `crypto-bigint 0.7.3` 上，**凡是让 cargo 重走解析的事（含「加或改任何一条依赖」）
//! 都会撞墙**。⇒ 加一条哈希依赖 = 让这个 crate 当场在断网门禁上编不过。
//! 代价如实记：这是一份手写的密码学原语，接住它的是 [`tests`] 里那三条 NIST 向量
//! （空串 / `abc` / 跨块的 56 字节串）—— **三条都对不上任何一条本模块自己算出来的数**，
//! 它们来自标准，不是来自这份实现。
//!
//! # 三 · 唯一失败面
//!
...
///
/// ⇒ 两条路，本轮选后者：① 去那张表上登记一行（越界）；
/// ② **上限跟着真正会去下载的那一层走** —— 那一层本轮不存在。
/// 选 ② 还有一条独立理由：本层是纯判定，它**执行不了**任何上限；
/// 一个自己管不住的数写在这里，是一句好看的空话。
/// ⚠ **代价如实记**：今天这条路上的上限**没有住址**。接线那一拍要同轮做两件事 ——
/// 把那个数定在下载那一层，并**去 `byte_cap_registry` 上登记它**（那时写区必须包含那棵树）。
...
impl Face {
    ...
    pub fn sentence(&self) -> String {
        match self {
            ...
            Face::Oversize(n) => format!("取代码全景 sidecar 时对面给的东西有 {n} 字节，超过了上限——收到的多半不是那个资产本身（错误页 / 重定向）。"),
            ...
            Face::LandingIo => "代码全景 sidecar 落盘时撞上 IO 错（磁盘满也算这一格）。".to_string(),
