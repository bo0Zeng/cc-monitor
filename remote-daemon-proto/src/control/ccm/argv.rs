...
///
/// ⚠ **`--account` 那一格今天是「不给 = 落 manifest 默认号」，而它是一个已知病灶。**
/// `shared/ccm:1001-1012` 那段逐字写着「把调用方选中的号**静默换掉**」——
/// 本实现**逐条照搬了那个语义**（见 [`Opts::account`] 与 `plan::resolve_account`），
/// 但**没有裁它**：改不改是产品决定，PM 09-11 明说「这一格归待问用户」。
/// ⇒ **这里只是把它搬到一处、并写明它是什么**，不是批准它。
