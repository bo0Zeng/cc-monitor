...
//! ⇒ 退而求其次是**跨语言对拍**，住在 `test/turn-notify.vitest.ts`：它**从本函数的合取项派生**
//! 人群，新加一条而不在那张登记表里说明 TS 侧怎么办 ⇒ 当场红。
//! ⚠ 这不是多余的礼节：报告 §4.1 记着这两份**已经漂过一次**（daemon 有 `!isApiError`、TS 没有），
//! 而 08-07 实测「daemon 加第五个合取项」时，改前的对拍表两侧共 17 条判据**一条都不会红**。
