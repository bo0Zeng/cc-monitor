...
/// | 键 | 为什么非它不可 |
/// |---|---|
/// | `PATH` | 被起的那个东西自己还要去找别的命令。`tests/e2e/daemon-cc-bus.sh` 的 `[10]` 逐字写着「`PATH` 清空是不行的 —— 那样它自己的 `awk` 也没了」；而且本层 [`super::discover`] 的兜底档本来就按 `PATH` 找插件本体 ⇒ 找得到却跑不动是自相矛盾 |
...
///   **没有一处**要子进程知道这件事；反过来，`tests/e2e/daemon-cc-bus.sh` 的 `[13]` 逐字要把这两个键
