...
///
/// - `TMUX` / `TMUX_PANE`：它们说的是「宿主此刻在哪个窗格里」。今天**没有任何调用方**要子进程
///   知道这件事；反过来，`e2e/daemon-cc-bus.sh` 的 `[13]` 逐字要把这两个键**摘掉**才测得出正确
///   行为（不摘的话，被起的那个东西会把**宿主的**窗格身份当成发信人）⇒ 给不出论据，不进。
/// - `TZ`：说得出的用途只有「时间戳两边同一个时区」，而今天经这一处口起出来的东西**没有一条**
///   把时间戳交回宿主 ⇒ 论据是想出来的，不是量出来的，不进。
/// - `USER` / `LOGNAME` / `SHELL` / `TERM`：现打，今天的被调方**一处都没读**它们；
...
pub(crate) const INHERITED_ENV_KEYS: &[(&str, &str)] = &[
    ("PATH", "被起的那个东西还要去找别的命令；本层找插件本体也走它"),
    ("HOME", "插件按 ~ 定位自己的装机位与数据目录"),
    ("TMPDIR", "子进程写临时文件的落点；清掉它写面反而变大"),
    ("TMUX_TMPDIR", "决定裸调的复用命令连到哪一台服务端；清掉会回落到用户那一台"),
