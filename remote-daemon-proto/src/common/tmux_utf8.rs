...
mod tests {
    ...
    /// `(&'static str, &'static str)`（一个**完全合法、语义零变化**的写法）就当场红，
    ...
    /// # ⚠ 锚点**刻意以非标识符字符收尾**（`:` 或 `(`）
    ///
    /// 第三版写成 `const <名>`（收在标识符上），当场被自己的变异证伪：把家里那个 env
    /// 改名成 `UTF8_CLIENT_ENVX` 之后，裸 `contains` **照样命中**（前者是后者的前缀）
    /// ⇒ 两条断言一起绿。那正是本仓「**匹配单位比事实小**」那一族（`find_pinned` 的头注
    /// 逐字记着 `remote-daemon-proto` 被 `remote-daemon-proto-X` 撑大那个活体）。
    ...
    /// （实测：第三版就是这样，把锚点末尾那个 `:` 去掉，反向自检**照样绿**）。
