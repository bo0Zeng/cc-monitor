...
//! | `crate::agents::…` | 本文件第一句逐字写着「**它不懂任何 agent 的语义**」。这条今天**承重**：`run(home, args)` 的 `home` 是**入参**（`main.rs` 的 `--relay` 臂递进来），不是中转自己去 `agents/` 里问的 |
//! | `crate::plugin::…` | 与调用口是**两条互不相干的基础设施**。中转认识它 = HTTP 线程上长出一条起进程的路（全 crate 唯一起进程口就在那儿） |
//!
//! ## 许引
//!
