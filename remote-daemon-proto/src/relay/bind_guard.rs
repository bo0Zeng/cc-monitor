...
//! ⚠ 订正〔回修轮之四 08-25，承接 D2 `建议-3`：这一行先前**与它上面三行自相矛盾**〕：
//! 先前这里写的是「两条各自单断过：**拼出来的**通配地址只红行为那条」——
//! 而上面 `:20-21` 刚逐字写完「审计实测…换成**字面量**…**一个字都没拼**」。
//! **实测过的是「字面量」那一形**（审计 `CT2`，D2 重打确认：射程 1/389，只红行为那条、本守卫绿）；
//! **「拼出来的」那一形至今没有人打过**。
//! ⇒ 今天准确的说法（分母 = 我数得出来的这 2 形）：
//!   · **字面量通配地址**（`Ipv4Addr::new(0,0,0,0)`）：**实测**只红行为那条，本守卫绿。
//!   · **本守卫单独会红**的那一形：needle 表里那 4 个拼写之一**出现在生产段**（不必被用到）。
//! 「拼出来的通配地址」与「具体非回环地址」这两形**都没人打过**，是从 needle 表推的。
