...
- 🔴 **这七处不是全部** —— 收窗口现打又逮到两处（两份 README「此刻自称的版本」），
  **本件实际改的是九处**。第 8/9 处与它的新判据住 `§K`，PM 的裁定住 `DECISIONS.md#R79`。
...
**每一趟 `--revert` 之后 `git status --short` 都核过，没有一刀留下残留**（前八刀那几趟只剩一行 `?? evidence/K-R120-deathvalue.md`；`d9`/`d10` 那两趟剩的是收窗口这一拍**还没提交**的那几份，逐份都是我自己写的）。
⚠ **`d6` 的射程随收窗口那一块一起变了**：它的两端 `BLOCK_HEAD..BLOCK_TAIL` 框的是「**本轮插进 `doc_claim_registry.rs` 的全部**」⇒ 补了第二块之后，`d6` 会把**两块一起**摘掉。它的 md5 自证仍然成立（摘完就是基点那一份），但**上面那一行 `d6` 的读数量于只有一块的时候**，如实标出来。
...
| `d9` | 收窗口 ①〔`R79`〕 | `README.md` 的 `当前发布 **v` ＋ `README.en.md` 的 `current release **v`，**各 1 次**（`3.8.0` → `3.7.0`，另外七处不动） | `GATE: FAIL —— cargo（退出码 101）` | `1478 passed; 1 failed`，唯一那条是`the_docs_self_reported_release_is_the_version_we_ship`，诊断**逐处点名**：`README.md 发布沿革段抬头（README.md）：3.7.0` / `README.en.md 发布沿革段抬头（README.en.md）：3.7.0` | **15** | ✅ **已知答案回测** —— 这一形收窗口那一刻真的在盘上，而当时 **16 格全绿** · 最小面 |
| `d10` | 收窗口 ② | 第二块两端锚点各 **1 次**（摘 101 行，刀自证「摘完那条判据的名字在整份文件里 **0** 处」）＋ 那两处各 1 次 | `GATE: OK —— 16 格全绿`（cargo **1614** —— 回到加它之前） | 无 | 16 | ✅ **阴性对照成立：一条都不红** |
...
| `README.en.md` | 1 行 | 同上 |
| `README.md` · `README.en.md` **收窗口那一拍** | 各 1 行 | `当前发布 **v` / `current release **v` 之后：`3.7.0` → `3.8.0`（`§K`） |
| `src-tauri/src/doc_claim_registry.rs` **收窗口那一拍** | **纯插入** 101 行 | hunk 头逐字 `@@ -2082,0 +2083,101 @@ mod tests`；挖掉之后整份 md5 `17568399b96af74c0d9e1ca3481dce15` == 上一个提交那一份（两边都是 3225 行）⇒ 那一块之外一个字节没动 |
...
9. 「产品面 112 条」里哪几条「用户可见」是**人裁的**，给不出判别式（`K-R118` 交回时逐字写过）。
   ⇒ 那一节挑了哪几条，**没有机检在守**。
10. 🔴 **「该改哪几处」今天仍然没有一个够得着全部的人群。** `§K` 补的是**这两处**，
    不是「所有自称版本的地方」—— 本仓别处（`doc/` · 界面文案 · 安装脚本里的 banner）
    有没有第三处自称版本，**我没查**（我查的是两份 README ＋ 那条判据的人群）。
    下一次发版前值得再人眼扫一遍，**而那正是本轮逮到这两处的唯一办法**。
11. 收窗口新加的那条判据靠 `当前发布 **v` / `current release **v` 两个**带 markdown 记号**的锚点。
    排版一改（去掉加粗）它当场红在「锚点命中 0 次」上 —— 那是有意的（fail-closed），
    但**它确实会因为一次纯排版改动而红**，写在这里别让下一个人当成误报去放宽它。

---

## §K 收窗口补的那两处 —— **判据在，而它的人群不含这一处**（`DECISIONS.md#R79`）

### §K1 现打逮到的是什么

本件按 `KR120D1` 把「七处」一次改齐、门禁 16 格全绿之后，**PM 收窗口复核现打**：

```
README.md:15      当前发布 **v3.7.0**
README.en.md:9    current release **v3.7.0**
```

⇒ **这棵树那一刻会带着「当前发布 v3.7.0」把 `3.8.0` 发出去，而 16 格一格都不红。**

🔴 **成因不是「改漏了」**：`KR120D1` 的「七处」＝
`the_release_version_is_the_same_in_all_six_places` 那条判据的人群 ＋ `Cargo.lock`，
而现打 `grep -c '当前发布\|current release' src-tauri/src/doc_claim_registry.rs` = **0** ——
**判据在，而它的人群不含这一处**。
...
1. **那两处改到 `3.8.0`**（落刀前锚点各断言恰好 1 次）：

   | # | 谁 | 文件 | 逐字锚点 | 行 | 改前 | 改后 | 锚点命中 |
   |---|---|---|---|---|---|---|---|
   | 8 | 发布沿革段抬头 | `README.md` | `当前发布 **v` | 15 | 3.7.0 | **3.8.0** | 1 |
   | 9 | 发布沿革段抬头 | `README.en.md` | `current release **v` | 9 | 3.7.0 | **3.8.0** | 1 |

   ⇒ **本件实际改的是九处，而 dod 的字面是「七处」** —— 差的那两处是 dod 口径的问题，
   PM 已裁（`DECISIONS.md#R79`），不是实现面的漏。

2. **新判据 `the_docs_self_reported_release_is_the_version_we_ship`** 把它们收进人群，
   与 `the_changelog_top_section_is_the_version_we_ship` 同一条路：
   跟 `env!("CARGO_PKG_VERSION")` 比，**不抠第二份权威源锚点**。

### §K3 🔴 射程刻意很窄 —— 为什么不把整段收进人群

那两行是**发布沿革**段，现打**各含 11 个** `vX.Y.Z`（量法
`sed -n '15p' README.md | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sort -u`）：
`v2.19.0` `v2.19.1` `v2.20.0` `v2.21.0` `v2.22.0` `v2.22.2` `v3.3.0` `v3.4.0` `v3.5.0` `v3.6.0`
＋ 自称的那一个。**其中 10 个是历史沿革，本来就该停在旧号上。**

⇒ 本条只钉**紧跟那两个锚点之后**的那一个。**把整段收进人群 = 下次发版红一片**，
那不是守，是拆（`testing.md` 判据硬规则 4：扫描面按**语义**划）。
同理 `README.md:164` 与 `README.en.md:136` 那两句「v3.6.0 与 v3.7.0 的实际产物是 `en-US`」
是 09-10 落的**历史订正**，**刻意不在射程里** —— 交回这一刻现打，
两份 README 里剩下的 `v3.7.0` **恰好就是这两处**，一处不多。

### §K4 这一族的口径纪律（写下来，下一次发版的人直接用）

**「该改哪几处」≠「判据今天数得到哪几处」。** 前者是事实，后者是人群 ——
把两者等同起来，就等于让「人群漏了谁」这件事**永远不会被发现**：
漏掉的那一处不在任何判据里，于是它不红；不红就没人去补人群。
🔴 **这个环只能从外面打断** —— 本轮打断它的不是判据，是**收窗口那一次人眼复核**。
