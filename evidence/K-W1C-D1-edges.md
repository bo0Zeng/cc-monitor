...
| 2 | 忘 | SidHwndCache | `forget` | `lib.rs:699` 逐字 `cache_for_emitter.forget(&sid);` | 本机 diff 报 removed（`Gone` 或 `Superseded`） | **0**；PM 把这一行换成 `apply_local_removal` 之后，由 `a_local_session_that_is_gone_gets_forgotten_in_memory_and_on_disk` + `a_local_session_that_was_superseded_gets_forgotten_too` 盖住 |
| 3 | 建 | RemoteHwndCache | `try_bind` | `lib.rs:809` 逐字 `if cache.try_bind(&sid) {` | 远端 daemon 帧报 session_added | **0** |
| 4 | 忘 | RemoteHwndCache | `forget` | `lib.rs:878` 逐字 `remote_cache_for_emitter.forget(&sid);` | 远端 removed 且 `classify_removed` 裁 `Archive` | **0**；PM 挪成 `apply_remote_disposition` 之后，由 `a_remote_session_classified_as_archive_gets_forgotten` + `a_remote_session_that_only_went_idle_keeps_its_binding` 盖住 |
| 5 | 建 | RemoteHwndCache | `try_bind_with_retry` | `lib.rs:2011` 逐字 `cache.try_bind_with_retry(` | 用户点 ↗ 且缓存没命中 | **0** |
| 6 | 忘 | RemoteHwndCache | `forget` | `lib.rs:2025` 逐字 `cache.forget(&session_id);` | 用户点 ↗ 且旧绑定 verify 失败 | **0** |
| 7 | 建 | RemoteHwndCache | `try_bind_with_retry` | `lib.rs:2029` 逐字 `cache.try_bind_with_retry(` | 同 5（verify-fail 重绑路） | **0** |

**内部转调 4 处**（不算边）：`bind.rs` 里 `self.forget(&removed.sid);` · `self.forget(sid);` ·
两处 `if self.try_bind(sid) {`。

**入口尚未接线 2 个**：`apply_local_removal` · `apply_remote_disposition` ——
本波新抽的两个入口，接线那一行住 `lib.rs`，**不在本波写区** ⇒ 归 PM。
⇒ 所以边 2 / 边 4 的判据栏今天如实写 **0**，不是「已经盖住了」。

## 与 PM 那把粗尺子对账（PM 量于 `4eb271f`，我量于本树尖）

| | PM 的粗尺子 | 本尺子 | 差在哪 |
|---|---|---|---|
| `.forget(` 全仓 | 5 处 | —— | 我在本树重打也是 **5 处**，行号也逐个对上（`bind.rs:836` 测试 · `lib.rs:699` · `:878` · `:1710` · `:2025`）⇒ 那个读数**没馊** |
| 与拉前有关的生产 `forget` 点 | 3 | 3（边 2 · 4 · 6） | 一致 |
